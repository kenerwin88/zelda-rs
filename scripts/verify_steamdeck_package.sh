...
grep -q 'ZELDA3_VIEWPORT_SCALE' "$PACKAGE_DIR/run-zelda3.sh" || fail "wrapper does not set ZELDA3_VIEWPORT_SCALE"
...
grep -q 'ZELDA3_RENDER_VIEWPORT_LOG' "$PACKAGE_DIR/verify-on-deck.sh" || fail "on-Deck verifier does not log renderer viewport"
...
grep -q 'steamos-add-to-steam' "$PACKAGE_DIR/install-to-desktop-mode.sh" || fail "installer does not request SteamOS shortcut registration"
grep -q 'ZELDA3_SKIP_STEAM_SHORTCUT' "$PACKAGE_DIR/install-to-desktop-mode.sh" || fail "installer cannot skip SteamOS shortcut registration"
grep -q 'Non-Steam Game' "$PACKAGE_DIR/README.txt" || fail "package README does not document Steam registration"
