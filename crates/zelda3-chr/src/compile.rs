...
fn rebuild_items(
    ...
    for (index, donor_item) in donor_items.iter().enumerate() {
        ...
        let donor_raw = if index < uncompressed_prefix {
            donor_item.clone()
        } else {
...
mod tests {
    ...
    // A synthetic donor: sprite pack 0 raw (prefix), pack 1 compressed; bg pack 0
    // compressed, bg pack 1 uncovered (pass-through).
    fn synthetic() -> (Vec<DecodedSheet>, Vec<u8>, Vec<u8>) {
        ...
        // Donor sprite container: both items are within the uncompressed prefix,
        // so both are stored raw planar (the real donor stores items 0..11 raw).
        let spr_items = vec![
            encode_planar_tiles(&spr0, 3).unwrap(),
            encode_planar_tiles(&spr1, 3).unwrap(),
        ];
        let bg_items = vec![
        ...
        // Two sheets: one carries sprite pack 0 + bg pack 0, one carries sprite pack 1.
        // bg pack 1 is intentionally uncovered.
        let sheet_a_tiles: Vec<[u8; 64]> = spr0.iter().chain(bg0.iter()).copied().collect();
        let sheet_a = DecodedSheet {
            name: "a".to_string(),
            tiles: sheet_a_tiles,
            blocks: vec![
                block("a.DAT1", "sprite", 0, 3, 0, 4),
        ...
        let sheet_b = DecodedSheet {
            name: "b".to_string(),
            tiles: spr1.clone(),
            blocks: vec![block("b.DAT1", "sprite", 1, 3, 0, 4)],
        };
    ...
    fn single_edit_touches_only_its_item_and_re_decodes() {
        ...
        // Edit one tile of sprite pack 1 (sheet b, block b.DAT1).
    ...
    fn edited_raw_sprite_stays_raw_when_0x600() {
        // Sprite pack 0 with 64 3bpp tiles encodes to exactly 0x600 bytes.
        let spr0: Vec<[u8; 64]> = (0..64).map(|i| tile(i as u8)).collect();
        let donor_spr = pack_arrays(&[encode_planar_tiles(&spr0, 3).unwrap()]);
        let donor_bg = pack_arrays(&[vec![0xFFu8]]); // trivial bg (uncovered)
        let mut sheet = DecodedSheet {
            name: "a".to_string(),
            tiles: spr0.clone(),
            blocks: vec![block("a.DAT1", "sprite", 0, 3, 0, 64)],
        };
