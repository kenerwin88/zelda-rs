...
pub enum Snes9xCpuQuiescentCheckpointError {
    ...
    #[error("source CPU checkpoint vertical IRQ timer {actual:?} does not match {expected:?}")]
    IrqTimerOwnershipMismatch {
        actual: Option<u64>,
        expected: Option<u64>,
    },
...
impl Snes9xColdCpuExecutor {
    ...
    fn validate_quiescent_machine(
        ...
        if machine.irq_timer_at != expected_irq_timer {
            return Err(
                Snes9xCpuQuiescentCheckpointError::IrqTimerOwnershipMismatch {
                    actual: machine.irq_timer_at.map(CpuMasterTimestamp::master_cycles),
                    expected: expected_irq_timer.map(CpuMasterTimestamp::master_cycles),
                },
            );
        }
    ...
    fn publish_due_vertical_irq(&mut self) {
        ...
        {
            // S9xMainLoop reschedules the next occurrence before asserting
            // CPU.IRQLine at this instruction boundary.
            self.reschedule_vertical_irq(false);
            self.machine.snes.cpu.irq_wanted = true;
            self.machine.snes.in_irq = true;
        }
    }

    fn enter_native_interrupt(
        &mut self,
        vector_address: u32,
    ...
    fn execute(
        ...
        match opcode {
            ...
            0x10 => self.branch(!self.machine.snes.cpu.n, accesses)?,
            0x15 => {
            ...
            0x1d => {
                let address =
                    self.absolute_indexed_read_address(self.machine.snes.cpu.x, accesses)?;
                let operand = self.read_by_m(address, WordWrap::None, accesses)?;
                self.ora_accumulator(operand);
            }
            0x20 => {
            ...
            0x3a => {
                self.add_cycles(ONE_CYCLE)?;
                let value = if self.machine.snes.cpu.mf {
                    let value = (self.machine.snes.cpu.a as u8).wrapping_sub(1);
                    self.machine.snes.cpu.a = (self.machine.snes.cpu.a & 0xff00) | u16::from(value);
                    u16::from(value)
                } else {
                    self.machine.snes.cpu.a = self.machine.snes.cpu.a.wrapping_sub(1);
                    self.machine.snes.cpu.a
                };
                self.set_zn(value, self.machine.snes.cpu.mf);
            }
            0x3b => {
                self.add_cycles(ONE_CYCLE)?;
                self.machine.snes.cpu.a = self.machine.snes.cpu.sp;
                self.set_zn(self.machine.snes.cpu.a, false);
            }
            0x3f => {
            ...
            0x46 => {
                ...
                self.machine.snes.cpu.c = value & 1 != 0;
                let result = value >> 1;
                self.add_cycles(ONE_CYCLE)?;
            ...
            0x54 => {
                ...
                self.machine.snes.cpu.db = destination_bank;
                let source_bank = self.immediate8(true, accesses)?;
                let source_address =
                    (u32::from(source_bank) << 16) | u32::from(self.machine.snes.cpu.x);
                let destination_address =
                    (u32::from(destination_bank) << 16) | u32::from(self.machine.snes.cpu.y);
                ...
                if self.machine.snes.cpu.xf {
                    self.machine.snes.cpu.x = (self.machine.snes.cpu.x & 0xff00)
                        | u16::from((self.machine.snes.cpu.x as u8).wrapping_add(1));
                    self.machine.snes.cpu.y = (self.machine.snes.cpu.y & 0xff00)
                        | u16::from((self.machine.snes.cpu.y as u8).wrapping_add(1));
                } else {
                    self.machine.snes.cpu.x = self.machine.snes.cpu.x.wrapping_add(1);
                    self.machine.snes.cpu.y = self.machine.snes.cpu.y.wrapping_add(1);
                }
                self.machine.snes.cpu.a = self.machine.snes.cpu.a.wrapping_sub(1);
                if self.machine.snes.cpu.a != 0xffff {
                    self.machine.snes.cpu.pc = self.machine.snes.cpu.pc.wrapping_sub(3);
                }
                self.add_cycles(TWO_CYCLES)?;
            }
            0x58 => {
            ...
            0x59 => {
                let address =
                    self.absolute_indexed_read_address(self.machine.snes.cpu.y, accesses)?;
                let operand = self.read_by_m(address, WordWrap::None, accesses)?;
                self.eor_accumulator(operand);
            }
            0x5a => {
            ...
            0x88 => {
                ...
                let value = if self.machine.snes.cpu.xf {
                    self.machine.snes.cpu.y =
                        u16::from((self.machine.snes.cpu.y as u8).wrapping_sub(1));
                    self.machine.snes.cpu.y
                } else {
                    self.machine.snes.cpu.y = self.machine.snes.cpu.y.wrapping_sub(1);
                    self.machine.snes.cpu.y
                };
                self.set_zn(value, self.machine.snes.cpu.xf);
            }
            0x89 => {
                let operand = self.immediate_by_m(accesses)?;
                let accumulator = if self.machine.snes.cpu.mf {
                    self.machine.snes.cpu.a & 0x00ff
                } else {
                    self.machine.snes.cpu.a
                };
                self.machine.snes.cpu.z = accumulator & operand == 0;
            }
            0x8c => {
            ...
            0x8a => {
                self.add_cycles(ONE_CYCLE)?;
                let value = if self.machine.snes.cpu.mf {
                    let value = self.machine.snes.cpu.x as u8;
                    self.machine.snes.cpu.a = (self.machine.snes.cpu.a & 0xff00) | u16::from(value);
                    u16::from(value)
                } else {
                    self.machine.snes.cpu.a = self.machine.snes.cpu.x;
                    self.machine.snes.cpu.a
                };
                self.set_zn(value, self.machine.snes.cpu.mf);
            }
            ...
            0x90 => self.branch(!self.machine.snes.cpu.c, accesses)?,
            0x95 => {
            ...
            0x98 => {
                self.add_cycles(ONE_CYCLE)?;
                let value = if self.machine.snes.cpu.mf {
                    let value = self.machine.snes.cpu.y as u8;
                    self.machine.snes.cpu.a = (self.machine.snes.cpu.a & 0xff00) | u16::from(value);
                    u16::from(value)
                } else {
                    self.machine.snes.cpu.a = self.machine.snes.cpu.y;
                    self.machine.snes.cpu.a
                };
                self.set_zn(value, self.machine.snes.cpu.mf);
            }
            0x99 => {
                ...
                self.add_cycles(ONE_CYCLE)?;
                let address =
                    address.wrapping_add(u32::from(self.machine.snes.cpu.y)) & 0x00ff_ffff;
                self.store_accumulator(address, accesses)?;
            }
            0x9d => {
                let address = self.absolute_address(false, accesses)?;
            ...
            0x9b => {
                self.add_cycles(ONE_CYCLE)?;
                let value = if self.machine.snes.cpu.xf {
                    self.machine.snes.cpu.x & 0x00ff
                } else {
            ...
            0xbb => {
                self.add_cycles(ONE_CYCLE)?;
                let value = if self.machine.snes.cpu.xf {
                    self.machine.snes.cpu.y & 0x00ff
                } else {
                    self.machine.snes.cpu.y
                };
                self.machine.snes.cpu.x = value;
    ...
    fn direct_indexed_x_address(
        ...
        let indexed = if self.machine.snes.cpu.e && self.machine.snes.cpu.dp as u8 == 0 {
            (address & 0xff00)
                | u16::from((address as u8).wrapping_add(self.machine.snes.cpu.x as u8))
        } else {
            address.wrapping_add(self.machine.snes.cpu.x)
        };
        self.add_cycles(ONE_CYCLE)?;
        Ok(u32::from(indexed))
    }
    ...
    fn direct_indirect_indexed_y_address(
        ...
        let pointer_wrap = if self.machine.snes.cpu.e {
            if self.machine.snes.cpu.dp as u8 == 0 {
                WordWrap::Page
            } else {
                WordWrap::Bank
            }
        ...
        self.machine.snes.open_bus = (pointer >> 8) as u8;
        if write
            || !self.machine.snes.cpu.xf
            || u16::from(pointer as u8).wrapping_add(u16::from(self.machine.snes.cpu.y as u8))
                >= 0x100
        {
            self.add_cycles(ONE_CYCLE)?;
        }
        Ok(
            ((u32::from(self.machine.snes.cpu.db) << 16) | u32::from(pointer))
                .wrapping_add(u32::from(self.machine.snes.cpu.y))
                & 0x00ff_ffff,
        )
    }
    ...
    fn source_read_semantic(&mut self, address: u32) -> Result<u8, SourceCpuError> {
        ...
        if (bank & 0x7f) < 0x40 && adr == 0x4211 {
            let asserted = self.machine.snes.cpu.irq_wanted;
        ...
        if (bank & 0x7f) < 0x40 && (0x4214..=0x4217).contains(&adr) {
            return Ok(match adr {
                0x4214 => self.machine.snes.divide_result as u8,
                0x4215 => (self.machine.snes.divide_result >> 8) as u8,
                0x4216 => self.machine.snes.multiply_result as u8,
                0x4217 => (self.machine.snes.multiply_result >> 8) as u8,
                _ => unreachable!(),
            });
        }
    ...
    fn source_write_semantic(&mut self, address: u32, value: u8) -> Result<(), SourceCpuError> {
        ...
        if (bank & 0x7f) < 0x40 && adr == 0x4200 {
            ...
            let was_v_irq_enabled = self.machine.snes.v_irq_enabled;
            self.machine.snes.v_irq_enabled = value & 0x20 != 0;
            if was_v_irq_enabled != self.machine.snes.v_irq_enabled {
                self.reschedule_vertical_irq(true);
            }
            if !self.machine.snes.v_irq_enabled {
                self.machine.snes.cpu.irq_wanted = false;
                self.machine.snes.in_irq = false;
            }
        ...
        if (bank & 0x7f) < 0x40 && matches!(adr, 0x4209 | 0x420a) {
            ...
            if self.machine.snes.v_timer != old {
                self.reschedule_vertical_irq(true);
            }
            return Ok(());
        }
        if (bank & 0x7f) < 0x40 && matches!(adr, 0x4204..=0x4206) {
            match adr {
                ...
                0x4205 => {
                    self.machine.snes.divide_a =
                        (self.machine.snes.divide_a & 0x00ff) | (u16::from(value) << 8);
                }
                0x4206 => {
                    if value == 0 {
                        self.machine.snes.divide_result = 0xffff;
                        self.machine.snes.multiply_result = self.machine.snes.divide_a;
                    } else {
                        self.machine.snes.divide_result =
                            self.machine.snes.divide_a / u16::from(value);
    ...
    fn sbc(&mut self, operand: u16) {
        ...
        let rhs = operand & mask;
        let carry = u32::from(self.machine.snes.cpu.c);
        let (result, overflow) = if self.machine.snes.cpu.d {
        ...
        self.machine.snes.cpu.c = result > i32::from(mask);
        if !self.machine.snes.cpu.d {
            self.machine.snes.cpu.c = result >= 0;
        }
        let narrowed = result as u16 & mask;
        self.machine.snes.cpu.v = overflow;
        self.machine.snes.cpu.a = (self.machine.snes.cpu.a & !mask) | narrowed;
        self.set_zn(narrowed, eight_bit);
    }
...
fn bcd_sub(lhs: u16, rhs: u16, carry: u32, digits: usize) -> (i32, bool) {
    let width_mask = if digits == 2 { 0x00ff } else { 0xffff };
    let sign = if digits == 2 { 0x0080 } else { 0x8000 };
    let complemented = (!rhs) & width_mask;
    let mut result = 0i32;
    let mut carry = carry as i32;
    let mut overflow = false;
    for digit in 0..digits {
        let shift = digit * 4;
        let place = 1i32 << shift;
        let lower_mask = place - 1;
        let digit_mask = 0x0f * place;
        let lower = result & lower_mask;
        result = i32::from(lhs) & digit_mask;
        result += i32::from(complemented) & digit_mask;
        result += lower;
        result += carry * place;
        let upper = place << 4;
        if digit + 1 == digits {
            overflow = ((lhs ^ complemented) & sign) == 0 && ((lhs ^ result as u16) & sign) != 0;
        }
        if result < upper {
            result -= 6 * place;
        }
        carry = i32::from(result > upper - 1);
    }
    (result, overflow)
}
...
mod tests {
    ...
    fn checkpoint_v5_roundtrips_vertical_irq_timer_and_rejects_v4_without_mutation() {
        ...
        assert!(restored.machine.snes.v_irq_enabled);
        assert_eq!(restored.machine.snes.v_timer, 144);
        assert_eq!(restored.machine.irq_timer_at, source.machine.irq_timer_at);
    ...
    fn malformed_or_mixed_quiescent_checkpoint_never_mutates_restore_target() {
        ...
        malformed.snes.v_irq_enabled = true;
        malformed.snes.v_timer = 144;
        assert!(matches!(
            target.restore_quiescent_checkpoint(malformed),
            Err(
                Snes9xCpuQuiescentCheckpointError::IrqTimerOwnershipMismatch {
    ...
    #[test]
    fn vtime_and_nmitimen_schedule_the_source_vertical_irq_deadline() {
        let rom = synthetic_rom(&[
            0x8d, 0x09, 0x42, // STA $4209
            0x9c, 0x0a, 0x42, // STZ $420a
            0x8d, 0x00, 0x42, // STA $4200
        ]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.a = 0x0090;

        let low = cpu.step().unwrap();
        assert_source_transaction_shape(
            &low,
            &[
                (
        ...
        assert_eq!(cpu.machine.snes.v_timer, 0x0090);
        assert_eq!(cpu.machine.snes.open_bus, 0x90);
        assert_eq!(cpu.machine.irq_timer_at, None);

        let high = cpu.step().unwrap();
        assert_source_transaction_shape(
            &high,
            &[
                (
        ...
        cpu.machine.snes.cpu.a = 0x00a1;
        let enable = cpu.step().unwrap();
        assert_source_transaction_shape(
            &enable,
            &[
                (
        ...
        assert!(cpu.machine.snes.v_irq_enabled);
        assert!(!cpu.machine.snes.h_irq_enabled);
        assert!(cpu.machine.snes.nmi_enabled);
        assert!(cpu.machine.snes.auto_joy_read);
        assert_eq!(cpu.machine.snes.open_bus, 0xa1);
        assert_eq!(
            cpu.machine.irq_timer_at,
            Some(CpuMasterTimestamp::new(
                CpuFieldTiming::NON_INTERLACE_EVEN
                    .master_cycles_at(0, crate::CpuRasterPosition::new(144, 10))
            ))
        );
    }

    #[test]
    fn due_vertical_irq_enters_native_vector_after_the_active_instruction() {
        let mut rom = synthetic_rom(&[0x18]); // CLC
        rom[0x0100] = 0x78; // first handler opcode at $8100
        rom[0x7fee] = 0x00;
        rom[0x7fef] = 0x81;
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
    ...
    #[test]
    fn timeup_read_clears_only_the_asserted_irq_line() {
        let rom = synthetic_rom(&[0xad, 0x11, 0x42]); // LDA $4211
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        ...
        assert_eq!(cpu.machine.snes.cpu.a, 0x55c2);
        assert_eq!(cpu.machine.snes.open_bus, 0xc2);
        assert!(!cpu.machine.snes.cpu.irq_wanted);
        assert!(!cpu.machine.snes.in_irq);
        assert_eq!(cpu.machine.irq_timer_at, deadline);
    }

    #[test]
    fn mode7_product_read_publishes_ppu_and_cpu_open_buses_at_source_boundaries() {
        let rom = synthetic_rom(&[0xad, 0x35, 0x21]); // LDA $2135
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.a = 0x5500;
        cpu.machine.snes.ppu.m7_matrix[0] = 0x1234;
        cpu.machine.snes.ppu.m7_matrix[1] = 0x5600;
    ...
    #[test]
    fn mode7_product_event_observes_ppu_publication_before_cpu_open_bus() {
        let rom = synthetic_rom(&[0xad, 0x35, 0x21]); // LDA $2135
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.a = 0x5500;
        cpu.machine.snes.ppu.m7_matrix[0] = 0x1234;
        ...
        assert_eq!(
            ...
            Some(CpuSynchronousCompletion::Read(0x1d))
        );
        assert!(cpu.is_poisoned());
    }

    #[test]
    fn cpu_division_registers_publish_quotient_and_remainder_in_source_order() {
        let rom = synthetic_rom(&[
            0x8d, 0x04, 0x42, // STA $4204
            0x9c, 0x05, 0x42, // STZ $4205
            0x8c, 0x06, 0x42, // STY $4206
            0xad, 0x14, 0x42, // LDA $4214
            0xad, 0x16, 0x42, // LDA $4216
        ]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.a = 0x0034;
        cpu.step().unwrap();
        cpu.step().unwrap();
        cpu.machine.snes.cpu.y = 4;
        let divide = cpu.step().unwrap();
        assert_source_transaction_shape(
            &divide,
            &[
                (
        ...
        assert_eq!(cpu.machine.snes.divide_a, 0x0034);
        assert_eq!(cpu.machine.snes.divide_result, 0x000d);
        assert_eq!(cpu.machine.snes.multiply_result, 0);
        assert_eq!(cpu.machine.snes.open_bus, 4);

        let quotient = cpu.step().unwrap();
        assert_source_transaction_shape(
            &quotient,
            &[
                (
        ...
        assert_eq!(cpu.machine.snes.cpu.a, 0x000d);
        assert_eq!(cpu.machine.snes.open_bus, 0x0d);
        cpu.step().unwrap();
        assert_eq!(cpu.machine.snes.cpu.a, 0);
        assert_eq!(cpu.machine.snes.open_bus, 0);

        cpu.source_write_semantic(0x00_4206, 0).unwrap();
        assert_eq!(cpu.machine.snes.divide_result, 0xffff);
        assert_eq!(cpu.machine.snes.multiply_result, 0x0034);
    }

    #[test]
    fn cpu_division_semantic_commits_before_a_failing_outer_write_drain() {
        let rom = synthetic_rom(&[0x8d, 0x06, 0x42]); // STA $4206
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.divide_a = 0x0034;
        cpu.machine.snes.cpu.a = 4;
        cpu.machine.snes.open_bus = 0x5a;
        install_hmax_smp_failure(&mut cpu, 1_334);
    ...
    #[test]
    fn rtl_native_pulls_return_word_then_bank_and_increments_target() {
        let rom = synthetic_rom(&[0x6b]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.sp = 0x01fc;
        cpu.machine.snes.ram[0x01fd] = 0x34;
        cpu.machine.snes.ram[0x01fe] = 0x92;
        cpu.machine.snes.ram[0x01ff] = 0x7e;
        cpu.machine.snes.open_bus = 0x5a;
        ...
        assert_eq!(receipt.accesses[1].address, 0x00_01fd);
        assert_eq!(receipt.accesses[2].address, 0x00_01ff);
        assert_eq!(cpu.machine.snes.cpu.sp, 0x01ff);
        assert_eq!(cpu.program_address(), 0x7e_9235);
        assert_eq!(cpu.machine.snes.open_bus, 0x5a);
    }

    #[test]
    fn rtl_emulation_pulls_bank_from_page_two_then_repairs_stack_high_byte() {
        let rom = synthetic_rom(&[0x6b]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.sp = 0x01fd;
        cpu.machine.snes.ram[0x01fe] = 0x34;
        cpu.machine.snes.ram[0x01ff] = 0x92;
        cpu.machine.snes.ram[0x0200] = 0x7e;
        cpu.machine.snes.ram[0x0100] = 0x11;
        cpu.machine.snes.open_bus = 0x5a;
        ...
        let reads = receipt
            ...
            .filter_map(|access| match access.kind {
                SourceCpuBusAccessKind::Read { .. } => Some(access.address),
                _ => None,
            })
            .collect::<Vec<_>>();
        assert_eq!(reads, [0x00_01fe, 0x00_0200]);
        assert_eq!(cpu.machine.snes.cpu.sp, 0x0100);
        assert_eq!(cpu.program_address(), 0x7e_9235);
        assert_eq!(cpu.machine.snes.open_bus, 0x5a);
    }
    ...
    #[test]
    fn rtl_bank_failure_keeps_pulled_pc_and_incremented_stack_but_defers_bank() {
        let rom = synthetic_rom(&[0x6b]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.sp = 0x01fc;
        ...
        cpu.machine.snes.ram[0x01ff] = 0x7e;
        cpu.machine.snes.open_bus = 0x5a;
        install_hmax_smp_failure(&mut cpu, 1_326);
        ...
        assert_eq!(
            cpu.machine.pending_completion(),
            Some(CpuSynchronousCompletion::Read(0x7e))
        );
        assert!(cpu.is_poisoned());
    }

    #[test]
    fn lda_direct_indirect_long_m8_reads_pointer_word_bank_then_data() {
        let rom = synthetic_rom(&[0xa7, 0x10]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.ram[0x10] = 0x34;
        cpu.machine.snes.ram[0x11] = 0x12;
        cpu.machine.snes.ram[0x12] = 0x7e;
        ...
        assert_eq!(receipt.accesses.len(), 5);
        assert_eq!(receipt.accesses[2].address, 0x00_0010);
        assert_eq!(receipt.accesses[3].address, 0x00_0012);
    ...
    #[test]
    fn lda_direct_indirect_long_m16_uses_none_wrap_for_target_data() {
        let rom = synthetic_rom(&[0xa7, 0xff]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        cpu.machine.snes.cpu.dp = 0x0101;
        cpu.machine.snes.ram[0x0200] = 0xff;
        cpu.machine.snes.ram[0x0201] = 0xff;
        cpu.machine.snes.ram[0x0202] = 0x7e;
        cpu.machine.snes.ram[0xffff] = 0x34;
        cpu.machine.snes.ram[0x1_0000] = 0x92;
        cpu.machine.snes.cpu.a = 0x0055;
        cpu.machine.snes.open_bus = 0x5a;
        ...
        assert_eq!(receipt.accesses.len(), 6);
        assert_eq!(receipt.accesses[2].address, 0x00_0200);
        assert_eq!(receipt.accesses[3].address, 0x00_0202);
        assert_eq!(receipt.accesses[4].address, 0x7e_ffff);
        assert_eq!(receipt.accesses[5].address, 0x7f_0000);
    ...
    #[test]
    fn lda_direct_indirect_long_bank_failure_keeps_pointer_high_publication() {
        let rom = synthetic_rom(&[0xa7, 0x10]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.ram[0x10] = 0x34;
        cpu.machine.snes.ram[0x11] = 0x12;
        ...
        cpu.machine.snes.cpu.z = true;
        cpu.machine.snes.open_bus = 0x5a;
        install_hmax_smp_failure(&mut cpu, 1_324);
    ...
    #[test]
    fn lda_direct_indirect_y_native_x8_skips_cycle_without_page_crossing() {
        let rom = synthetic_rom(&[0xb1, 0x10]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.db = 0x7e;
        cpu.machine.snes.cpu.y = 1;
        cpu.machine.snes.ram[0x10] = 0x00;
        cpu.machine.snes.ram[0x11] = 0x02;
        cpu.machine.snes.ram[0x0201] = 0x80;
        cpu.machine.snes.cpu.a = 0xab55;
    ...
    #[test]
    fn lda_direct_indirect_y_native_x16_always_adds_cycle_before_word_data() {
        let rom = synthetic_rom(&[0xb1, 0x10]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        ...
        cpu.machine.snes.cpu.db = 0x7e;
        cpu.machine.snes.cpu.y = 1;
        cpu.machine.snes.ram[0x10] = 0xfe;
        cpu.machine.snes.ram[0x11] = 0x02;
        cpu.machine.snes.ram[0x02ff] = 0x34;
        cpu.machine.snes.ram[0x0300] = 0x92;
    ...
    #[test]
    fn lda_direct_indirect_y_emulation_wraps_pointer_inside_direct_page() {
        let rom = synthetic_rom(&[0xb1, 0xff]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.db = 0x7e;
        cpu.machine.snes.ram[0x00ff] = 0x00;
        cpu.machine.snes.ram[0x0000] = 0x02;
        cpu.machine.snes.ram[0x0100] = 0x7f;
        ...
        assert_eq!(cpu.machine.snes.cpu.a, 0x005a);
        assert_eq!(cpu.machine.snes.open_bus, 0x5a);
    }

    #[test]
    fn lda_direct_indirect_y_index_cycle_failure_keeps_pointer_and_alu_state() {
        let rom = synthetic_rom(&[0xb1, 0x10]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.xf = false;
    ...
    #[test]
    fn lda_direct_x_m8_emulation_wraps_index_inside_the_direct_page() {
        let rom = synthetic_rom(&[0xb5, 0xff]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.x = 2;
        cpu.machine.snes.cpu.a = 0xab55;
        cpu.machine.snes.ram[0x0001] = 0x80;
        cpu.machine.snes.ram[0x0101] = 0x5a;
    ...
    #[test]
    fn lda_direct_x_m16_native_adds_full_index_after_nonzero_dp_cycle() {
        let rom = synthetic_rom(&[0xb5, 0x10]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        cpu.machine.snes.cpu.xf = false;
        cpu.machine.snes.cpu.dp = 1;
        cpu.machine.snes.cpu.x = 2;
        cpu.machine.snes.ram[0x13] = 0x34;
        cpu.machine.snes.ram[0x14] = 0x92;
    ...
    #[test]
    fn lda_direct_x_index_cycle_failure_precedes_data_and_accumulator() {
        let rom = synthetic_rom(&[0xb5, 0x10]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        ...
        cpu.machine.snes.cpu.n = false;
        cpu.machine.snes.cpu.z = true;
        cpu.machine.snes.ram[0x11] = 0x80;
        ...
        assert_eq!(cpu.machine.snes.open_bus, 0x10);
        assert_eq!(cpu.machine.snes.cpu.a, 0xab55);
        assert!(!cpu.machine.snes.cpu.n);
        assert!(cpu.machine.snes.cpu.z);
        assert!(cpu.is_poisoned());
    ...
    #[test]
    fn cmp_direct_x_m8_emulation_wraps_index_inside_the_direct_page() {
        let rom = synthetic_rom(&[0xd5, 0xff]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.x = 2;
        cpu.machine.snes.cpu.a = 0xab80;
        cpu.machine.snes.ram[0x0001] = 0x80;
        cpu.machine.snes.ram[0x0101] = 0x5a;
    ...
    #[test]
    fn cmp_direct_x_m16_native_adds_full_index_after_nonzero_dp_cycle() {
        let rom = synthetic_rom(&[0xd5, 0x10]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        cpu.machine.snes.cpu.xf = false;
    ...
    #[test]
    fn cmp_direct_x_index_cycle_failure_precedes_operand_and_flags() {
        let rom = synthetic_rom(&[0xd5, 0x10]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.x = 1;
        cpu.machine.snes.cpu.a = 0xab55;
    ...
    #[test]
    fn cmp_direct_m8_charges_nonzero_dp_and_sets_equality() {
        let rom = synthetic_rom(&[0xc5, 0x10]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.dp = 1;
        cpu.machine.snes.cpu.a = 0xab80;
        cpu.machine.snes.ram[0x11] = 0x80;
    ...
    #[test]
    fn cmp_direct_m16_reads_bank_wrapped_word_and_sets_sign() {
        let rom = synthetic_rom(&[0xc5, 0x10]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        cpu.machine.snes.cpu.dp = 1;
        cpu.machine.snes.cpu.a = 0x1234;
        cpu.machine.snes.ram[0x11] = 0x34;
        cpu.machine.snes.ram[0x12] = 0x92;
    ...
    #[test]
    fn cmp_direct_read_failure_preserves_flags_after_address_publication() {
        let rom = synthetic_rom(&[0xc5, 0x14]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.dp = 0x0101;
        cpu.machine.snes.cpu.a = 0xab55;
        cpu.machine.snes.cpu.c = true;
    ...
    #[test]
    fn cmp_absolute_x_m8_skips_internal_cycle_without_page_crossing() {
        let rom = synthetic_rom(&[0xdd, 0x00, 0x01]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.db = 0x7e;
        cpu.machine.snes.cpu.x = 2;
        cpu.machine.snes.cpu.a = 0xab80;
    ...
    #[test]
    fn cmp_absolute_x_m16_always_adds_cycle_and_reads_none_wrapped_word() {
        let rom = synthetic_rom(&[0xdd, 0xfe, 0xff]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        cpu.machine.snes.cpu.xf = false;
    ...
    #[test]
    fn cmp_absolute_x_page_cross_failure_keeps_operand_and_flags() {
        let rom = synthetic_rom(&[0xdd, 0xff, 0x01]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.db = 0x7e;
        cpu.machine.snes.cpu.x = 1;
        cpu.machine.snes.cpu.a = 0xab55;
    ...
    #[test]
    fn sta_direct_indirect_long_y_m8_reads_pointer_then_writes_indexed_target() {
        let rom = synthetic_rom(&[0x97, 0x10]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.ram[0x10] = 0x00;
        cpu.machine.snes.ram[0x11] = 0x01;
        cpu.machine.snes.ram[0x12] = 0x7e;
        cpu.machine.snes.cpu.y = 2;
        cpu.machine.snes.cpu.a = 0xab80;
        cpu.machine.snes.ram[0x0102] = 0x5a;
        ...
        assert_eq!(receipt.accesses[2].address, 0x00_0010);
        assert_eq!(receipt.accesses[3].address, 0x00_0012);
        assert_eq!(receipt.accesses[4].address, 0x7e_0102);
        assert_eq!(cpu.machine.snes.ram[0x0102], 0x80);
        assert_eq!(cpu.machine.snes.open_bus, 0x80);
    }

    #[test]
    fn sta_direct_indirect_long_y_m16_crosses_target_bank_low_then_high() {
        let rom = synthetic_rom(&[0x97, 0x10]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        cpu.machine.snes.ram[0x10] = 0xfe;
        cpu.machine.snes.ram[0x11] = 0xff;
        cpu.machine.snes.ram[0x12] = 0x7e;
        cpu.machine.snes.cpu.y = 1;
        ...
        assert_eq!(cpu.machine.snes.ram[0x1_0000], 0x92);
        assert_eq!(cpu.machine.snes.open_bus, 0x92);
    }

    #[test]
    fn ora_direct_indirect_long_m8_preserves_b_after_pointer_and_data_reads() {
        let rom = synthetic_rom(&[0x07, 0x10]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.ram[0x10] = 0x34;
        cpu.machine.snes.ram[0x11] = 0x12;
        cpu.machine.snes.ram[0x12] = 0x7e;
        cpu.machine.snes.ram[0x1234] = 0x80;
        cpu.machine.snes.cpu.a = 0xab01;
        cpu.machine.snes.open_bus = 0x5a;
        ...
        assert_eq!(cpu.machine.snes.cpu.a, 0xab81);
        assert!(cpu.machine.snes.cpu.n);
        assert!(!cpu.machine.snes.cpu.z);
        assert_eq!(cpu.machine.snes.open_bus, 0x80);
    }
    ...
    #[test]
    fn ora_direct_m8_charges_nonzero_dp_and_preserves_accumulator_b() {
        let rom = synthetic_rom(&[0x05, 0xff]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.dp = 0x0101;
        cpu.machine.snes.cpu.a = 0xab01;
        cpu.machine.snes.ram[0x0200] = 0x80;
    ...
    #[test]
    fn ora_direct_m16_reads_bank_wrapped_word_and_publishes_high_byte() {
        let rom = synthetic_rom(&[0x05, 0xff]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        cpu.machine.snes.cpu.dp = 0x0101;
        cpu.machine.snes.cpu.a = 0x0001;
        cpu.machine.snes.ram[0x0200] = 0x00;
        cpu.machine.snes.ram[0x0201] = 0x80;
        cpu.machine.snes.open_bus = 0x5a;
        ...
        assert_eq!(receipt.accesses[2].address, 0x00_0200);
        assert_eq!(cpu.machine.snes.cpu.a, 0x8001);
        assert!(cpu.machine.snes.cpu.n);
        assert!(!cpu.machine.snes.cpu.z);
        assert_eq!(cpu.machine.snes.open_bus, 0x80);
    }
    ...
    #[test]
    fn ora_direct_operand_failure_precedes_accumulator_and_flag_mutation() {
        let rom = synthetic_rom(&[0x05, 0xff]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.dp = 0x0101;
        cpu.machine.snes.cpu.a = 0xab01;
        ...
        assert_eq!(cpu.program_address(), 0x00_8002);
        assert_eq!(cpu.machine.snes.cpu.a, 0xab01);
        assert!(!cpu.machine.snes.cpu.n);
        assert!(cpu.machine.snes.cpu.z);
        assert_eq!(cpu.machine.snes.open_bus, 0xff);
        assert!(cpu.is_poisoned());
    }

    #[test]
    fn ora_immediate_m8_preserves_accumulator_b_and_publishes_operand() {
        let rom = synthetic_rom(&[0x09, 0x80]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.a = 0xab01;
    ...
    #[test]
    fn ora_immediate_m16_uses_one_word_operand_and_publishes_high_byte() {
        let rom = synthetic_rom(&[0x09, 0x00, 0x80]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        cpu.machine.snes.cpu.a = 0x0001;
    ...
    #[test]
    fn ora_immediate_failure_defers_pc_open_bus_accumulator_and_flags() {
        let rom = synthetic_rom(&[0x09, 0x80]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.a = 0xab01;
        cpu.machine.snes.cpu.n = false;
    ...
    #[test]
    fn ora_direct_x_m8_emulation_wraps_index_inside_the_direct_page() {
        let rom = synthetic_rom(&[0x15, 0xff]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.dp = 0x1200;
        cpu.machine.snes.cpu.x = 2;
        cpu.machine.snes.cpu.a = 0xab01;
        cpu.machine.snes.ram[0x1201] = 0x80;
        cpu.machine.snes.open_bus = 0x5a;
        ...
        assert_eq!(receipt.accesses[2].address, 0x00_1201);
        assert_eq!(cpu.machine.snes.cpu.a, 0xab81);
        assert!(cpu.machine.snes.cpu.n);
        assert!(!cpu.machine.snes.cpu.z);
        assert_eq!(cpu.machine.snes.open_bus, 0x80);
    ...
    #[test]
    fn mvn_x8_moves_one_byte_then_rewinds_with_low_index_increments() {
        let rom = synthetic_rom(&[0x54, 0x7e, 0x7e]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.xf = true;
        cpu.machine.snes.cpu.x = 0xabff;
        cpu.machine.snes.cpu.y = 0xcdff;
        cpu.machine.snes.cpu.a = 1;
        cpu.machine.snes.cpu.db = 0x12;
        cpu.machine.snes.ram[0xabff] = 0x5a;
        cpu.machine.snes.open_bus = 0x34;
        ...
        assert_eq!(receipt.accesses[3].address, 0x7e_abff);
        assert_eq!(receipt.accesses[4].address, 0x7e_cdff);
        assert_eq!(cpu.machine.snes.ram[0xcdff], 0x5a);
        assert_eq!(cpu.machine.snes.cpu.db, 0x7e);
        assert_eq!(cpu.machine.snes.cpu.x, 0xab00);
        assert_eq!(cpu.machine.snes.cpu.y, 0xcd00);
        assert_eq!(cpu.machine.snes.cpu.a, 0);
    ...
    #[test]
    fn mvn_x16_finishes_on_accumulator_underflow_without_rewinding() {
        let rom = synthetic_rom(&[0x54, 0x7e, 0x7f]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.xf = false;
        cpu.machine.snes.cpu.x = 0xffff;
        cpu.machine.snes.cpu.y = 0x0010;
        cpu.machine.snes.cpu.a = 0;
        cpu.machine.snes.ram[0x1ffff] = 0xa5;
        ...
        assert_eq!(receipt.accesses[3].address, 0x7f_ffff);
        assert_eq!(receipt.accesses[4].address, 0x7e_0010);
        assert_eq!(cpu.machine.snes.ram[0x0010], 0xa5);
        assert_eq!(cpu.machine.snes.cpu.db, 0x7e);
        assert_eq!(cpu.machine.snes.cpu.x, 0);
        assert_eq!(cpu.machine.snes.cpu.y, 0x0011);
        assert_eq!(cpu.machine.snes.cpu.a, 0xffff);
        assert_eq!(cpu.program_address(), 0x00_8003);
        assert_eq!(cpu.machine.snes.open_bus, 0xa5);
    }

    #[test]
    fn mvn_final_cycle_failure_retains_committed_move_and_rewound_registers() {
        let rom = synthetic_rom(&[0x54, 0x7e, 0x7e]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.x = 0x0010;
        cpu.machine.snes.cpu.y = 0x0020;
        cpu.machine.snes.cpu.a = 1;
        cpu.machine.snes.ram[0x0010] = 0x6c;
        cpu.machine.snes.open_bus = 0x34;
        install_hmax_smp_failure(&mut cpu, 1_320);
        ...
        assert_eq!(cpu.machine.snes.ram[0x0020], 0x6c);
        assert_eq!(cpu.machine.snes.cpu.db, 0x7e);
        assert_eq!(cpu.machine.snes.cpu.x, 0x0011);
        assert_eq!(cpu.machine.snes.cpu.y, 0x0021);
        assert_eq!(cpu.machine.snes.cpu.a, 0);
        assert_eq!(cpu.program_address(), 0x00_8000);
        assert_eq!(cpu.machine.snes.open_bus, 0x6c);
        assert_eq!(cpu.machine.pending_completion(), None);
        assert!(cpu.is_poisoned());
    }

    #[test]
    fn ora_direct_x_m16_native_adds_full_index_after_nonzero_dp_cycle() {
        let rom = synthetic_rom(&[0x15, 0xff]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        ...
        cpu.machine.snes.cpu.x = 2;
        cpu.machine.snes.cpu.a = 1;
        cpu.machine.snes.ram[0x0202] = 0;
        cpu.machine.snes.ram[0x0203] = 0x80;
        cpu.machine.snes.open_bus = 0x5a;
        ...
        assert_eq!(receipt.accesses[2].address, 0x00_0202);
        assert_eq!(cpu.machine.snes.cpu.a, 0x8001);
        assert!(cpu.machine.snes.cpu.n);
        assert!(!cpu.machine.snes.cpu.z);
        assert_eq!(cpu.machine.snes.open_bus, 0x80);
    ...
    #[test]
    fn ora_absolute_x_m8_skips_internal_cycle_without_page_crossing() {
        let rom = synthetic_rom(&[0x1d, 0x00, 0x01]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.db = 0x7e;
        cpu.machine.snes.cpu.x = 2;
        cpu.machine.snes.cpu.a = 0xab01;
    ...
    #[test]
    fn ora_absolute_x_m16_always_adds_cycle_and_reads_none_wrapped_word() {
        let rom = synthetic_rom(&[0x1d, 0xfe, 0xff]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        cpu.machine.snes.cpu.xf = false;
    ...
    #[test]
    fn ora_absolute_x_page_cross_failure_precedes_operand_and_alu_state() {
        let rom = synthetic_rom(&[0x1d, 0xff, 0x00]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.db = 0x7e;
        cpu.machine.snes.cpu.x = 1;
        cpu.machine.snes.cpu.a = 0xab01;
        ...
        assert!(!cpu.machine.snes.cpu.n);
        assert!(cpu.machine.snes.cpu.z);
        assert_eq!(cpu.machine.snes.open_bus, 0x00);
        assert_eq!(cpu.machine.pending_completion(), None);
        assert!(cpu.is_poisoned());
    }

    #[test]
    fn ora_direct_indirect_long_m16_reads_the_full_none_wrapped_operand() {
        let rom = synthetic_rom(&[0x07, 0x10]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        ...
        cpu.machine.snes.ram[0x11] = 0x01;
        cpu.machine.snes.ram[0x12] = 0x7e;
        cpu.machine.snes.ram[0x0100] = 0x00;
        cpu.machine.snes.ram[0x0101] = 0x80;
        cpu.machine.snes.cpu.a = 0x0001;
        cpu.machine.snes.open_bus = 0x5a;
    ...
    #[test]
    fn ora_direct_indirect_long_data_failure_retains_pointer_bank_and_alu_state() {
        let rom = synthetic_rom(&[0x07, 0x10]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.ram[0x10] = 0x34;
        cpu.machine.snes.ram[0x11] = 0x12;
        ...
        cpu.machine.snes.cpu.z = true;
        cpu.machine.snes.open_bus = 0x5a;
        install_hmax_smp_failure(&mut cpu, 1_316);
        ...
        assert!(!cpu.machine.snes.cpu.n);
        assert!(cpu.machine.snes.cpu.z);
        assert_eq!(cpu.machine.snes.open_bus, 0x7e);
        assert_eq!(
            cpu.machine.pending_completion(),
    ...
    #[test]
    fn sta_absolute_long_x_m8_adds_index_without_an_internal_cycle() {
        let rom = synthetic_rom(&[0x9f, 0x00, 0x01, 0x7e]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.x = 2;
        cpu.machine.snes.cpu.a = 0xab80;
        cpu.machine.snes.ram[0x0102] = 0x5a;
        ...
        assert_eq!(receipt.accesses[2].address, 0x7e_0102);
        assert_eq!(receipt.accesses[2].timestamp, CpuMasterTimestamp::new(230));
        assert_eq!(cpu.machine.snes.ram[0x0102], 0x80);
        assert_eq!(cpu.machine.snes.open_bus, 0x80);
    }

    #[test]
    fn sta_absolute_long_x_m16_crosses_bank_with_low_then_high_writes() {
        let rom = synthetic_rom(&[0x9f, 0xfe, 0xff, 0x7e]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        cpu.machine.snes.cpu.x = 1;
        cpu.machine.snes.cpu.a = 0x9234;
        cpu.machine.snes.ram[0xffff] = 0xa5;
        cpu.machine.snes.ram[0x1_0000] = 0x5a;
        cpu.machine.snes.open_bus = 0x7c;
    ...
    #[test]
    fn lda_absolute_long_x_m8_crosses_bank_without_an_internal_cycle() {
        let rom = synthetic_rom(&[0xbf, 0xfe, 0xff, 0x7e]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.x = 2;
        cpu.machine.snes.cpu.a = 0x125a;
        cpu.machine.snes.ram[0x1_0000] = 0x80;
        cpu.machine.snes.open_bus = 0x7c;
        ...
        assert_eq!(receipt.accesses[1].charged_master_cycles, 24);
    ...
    #[test]
    fn lda_absolute_long_x_m16_wraps_the_24_bit_address_and_word() {
        let rom = synthetic_rom(&[0xbf, 0xfe, 0xff, 0xff]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        cpu.machine.snes.cpu.x = 1;
        cpu.machine.snes.cpu.a = 0xa55a;
        cpu.machine.snes.ram[0] = 0x34;
        cpu.machine.snes.open_bus = 0x7c;
        ...
        assert_eq!(receipt.accesses.len(), 4);
        assert_eq!(receipt.accesses[2].address, 0xff_ffff);
        assert_eq!(receipt.accesses[3].address, 0x00_0000);
        // LoROM bank $ff:$ffff maps the synthetic ROM's final $ea byte;
        // WRAP_NONE then carries the high read through $ffffff->$000000.
        assert_eq!(cpu.machine.snes.cpu.a, 0x34ea);
        assert!(!cpu.machine.snes.cpu.n);
        assert!(!cpu.machine.snes.cpu.z);
        assert_eq!(cpu.machine.snes.open_bus, 0x34);
    }

    #[test]
    fn adc_absolute_long_x_m8_crosses_bank_without_an_internal_cycle() {
        let rom = synthetic_rom(&[0x7f, 0xfe, 0xff, 0x7e]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.x = 2;
        cpu.machine.snes.cpu.a = 0xab01;
        cpu.machine.snes.cpu.c = false;
        cpu.machine.snes.ram[0x1_0000] = 0x7f;
        cpu.machine.snes.open_bus = 0x7c;
        ...
        assert!(cpu.machine.snes.cpu.n);
        assert!(!cpu.machine.snes.cpu.z);
        assert_eq!(cpu.machine.snes.open_bus, 0x7f);
    }

    #[test]
    fn adc_absolute_long_x_m16_crosses_bank_with_none_wrapped_word() {
        let rom = synthetic_rom(&[0x7f, 0xfe, 0xff, 0x7e]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        ...
        cpu.machine.snes.cpu.a = 1;
        cpu.machine.snes.cpu.c = false;
        cpu.machine.snes.ram[0xffff] = 0xff;
        cpu.machine.snes.ram[0x1_0000] = 0x7f;
        cpu.machine.snes.open_bus = 0x7c;
        ...
        assert_eq!(receipt.accesses[2].address, 0x7e_ffff);
        assert_eq!(receipt.accesses[3].address, 0x7f_0000);
        assert_eq!(cpu.machine.snes.cpu.a, 0x8000);
        assert!(!cpu.machine.snes.cpu.c);
        assert!(cpu.machine.snes.cpu.v);
        assert!(cpu.machine.snes.cpu.n);
        assert!(!cpu.machine.snes.cpu.z);
    ...
    #[test]
    fn lda_absolute_long_x_operand_drain_failure_defers_pc_and_register_publication() {
        let rom = synthetic_rom(&[0xbf, 0x00, 0x01, 0x7e]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.x = 2;
        cpu.machine.snes.cpu.a = 0x125a;
        cpu.machine.snes.cpu.n = true;
        cpu.machine.snes.cpu.z = false;
        cpu.machine.snes.open_bus = 0x7c;
        install_hmax_smp_failure(&mut cpu, 1_334);
        ...
        assert_eq!(cpu.program_address(), 0x00_8001);
        assert_eq!(cpu.machine.snes.cpu.a, 0x125a);
        assert!(cpu.machine.snes.cpu.n);
        assert!(!cpu.machine.snes.cpu.z);
    ...
    #[test]
    fn sta_absolute_long_x_write_failure_commits_semantic_before_open_bus() {
        let rom = synthetic_rom(&[0x9f, 0x00, 0x01, 0x7e]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.x = 2;
        cpu.machine.snes.cpu.a = 0xab80;
    ...
    #[test]
    fn sta_absolute_y_m8_adds_the_mandatory_indexed_write_cycle() {
        let rom = synthetic_rom(&[0x99, 0xfe, 0x01]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.db = 0x7e;
        cpu.machine.snes.cpu.y = 2;
        cpu.machine.snes.cpu.a = 0x12a5;
        cpu.machine.snes.ram[0x0200] = 0x5a;
        cpu.machine.snes.open_bus = 0x7c;
        ...
        assert_eq!(receipt.accesses.len(), 3);
        assert_eq!(receipt.accesses[2].address, 0x7e_0200);
        assert_eq!(receipt.accesses[2].timestamp, CpuMasterTimestamp::new(228));
        assert_eq!(cpu.machine.snes.ram[0x0200], 0xa5);
        assert_eq!(cpu.machine.snes.open_bus, 0xa5);
    }

    #[test]
    fn sta_absolute_y_m16_carries_index_into_the_next_bank() {
        let rom = synthetic_rom(&[0x99, 0xff, 0xff]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        cpu.machine.snes.cpu.db = 0x7e;
        ...
        assert_eq!(cpu.machine.snes.ram[0x1_0000], 0x34);
        assert_eq!(cpu.machine.snes.ram[0x1_0001], 0x92);
        assert_eq!(cpu.machine.snes.open_bus, 0x92);
    }

    #[test]
    fn sta_absolute_y_event_failure_precedes_indexed_write_semantics() {
        let rom = synthetic_rom(&[0x99, 0xfe, 0x01]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.db = 0x7e;
        cpu.machine.snes.cpu.y = 2;
        ...
        assert_eq!(cpu.program_address(), 0x00_8003);
        assert_eq!(cpu.machine.snes.ram[0x0200], 0x5a);
        assert_eq!(cpu.machine.snes.open_bus, 0x7c);
        assert_eq!(cpu.machine.pending_completion(), None);
        assert!(cpu.is_poisoned());
    }
    ...
    #[test]
    fn ldy_absolute_x_x8_uses_x_and_skips_cycle_without_page_crossing() {
        let rom = synthetic_rom(&[0xbc, 0x00, 0x01]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.db = 0x7e;
        cpu.machine.snes.cpu.x = 2;
        cpu.machine.snes.cpu.y = 0x0055;
        cpu.machine.snes.ram[0x0102] = 0x80;
        cpu.machine.snes.open_bus = 0x5a;
    ...
    #[test]
    fn ldy_absolute_x_x16_always_adds_cycle_and_reads_bank_wrapped_word() {
        let rom = synthetic_rom(&[0xbc, 0xff, 0xff]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.xf = false;
        cpu.machine.snes.cpu.db = 0x7e;
    ...
    #[test]
    fn ldy_absolute_x_page_cross_failure_precedes_index_and_flag_mutation() {
        let rom = synthetic_rom(&[0xbc, 0xff, 0x01]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.db = 0x7e;
        cpu.machine.snes.cpu.x = 1;
        cpu.machine.snes.cpu.y = 0x0055;
        ...
        assert_eq!(cpu.program_address(), 0x00_8003);
        assert_eq!(cpu.machine.snes.cpu.y, 0x0055);
        assert!(!cpu.machine.snes.cpu.n);
        assert!(cpu.machine.snes.cpu.z);
        assert_eq!(cpu.machine.snes.open_bus, 0x01);
        assert!(cpu.is_poisoned());
    ...
    #[test]
    fn lda_absolute_x_x8_uses_x_and_skips_cycle_without_page_crossing() {
        let rom = synthetic_rom(&[0xbd, 0x00, 0x01]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.db = 0x7e;
        cpu.machine.snes.cpu.x = 2;
        cpu.machine.snes.cpu.y = 5;
        cpu.machine.snes.cpu.a = 0xab55;
        cpu.machine.snes.ram[0x0102] = 0x80;
        cpu.machine.snes.ram[0x0105] = 0x11;
        cpu.machine.snes.open_bus = 0x5a;
    ...
    #[test]
    fn lda_absolute_x_x16_always_adds_cycle_and_reads_none_wrapped_word() {
        let rom = synthetic_rom(&[0xbd, 0xff, 0xff]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        cpu.machine.snes.cpu.xf = false;
    ...
    #[test]
    fn jmp_absolute_indirect_long_reads_word_then_bank_before_publishing_target() {
        let rom = synthetic_rom(&[0xdc, 0x10, 0x00]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.ram[0x0010] = 0x34;
        cpu.machine.snes.ram[0x0011] = 0x92;
        cpu.machine.snes.ram[0x0012] = 0x7e;
        cpu.machine.snes.open_bus = 0x5a;
    ...
    #[test]
    fn jmp_absolute_indirect_long_pointer_word_follows_none_wrap_across_bank() {
        let mut rom = synthetic_rom(&[0xdc, 0xff, 0xff]);
        rom[0x7fff] = 0x34;
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.ram[0x0000] = 0x92;
        cpu.machine.snes.ram[0x0001] = 0x7e;
        cpu.machine.snes.open_bus = 0x5a;
        ...
        let reads = receipt
            ...
            .collect::<Vec<_>>();
        assert_eq!(reads, [0x00_8001, 0x00_ffff, 0x01_0000, 0x01_0001]);
        assert_eq!(cpu.program_address(), 0x7e_9234);
        assert_eq!(cpu.machine.snes.open_bus, 0x7e);
    }

    #[test]
    fn jmp_absolute_indirect_long_bank_failure_keeps_sequential_pc_and_word_bus() {
        let rom = synthetic_rom(&[0xdc, 0x10, 0x00]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.ram[0x0010] = 0x34;
        cpu.machine.snes.ram[0x0011] = 0x92;
    ...
    #[test]
    fn and_absolute_long_x_m8_crosses_bank_without_an_internal_cycle() {
        let rom = synthetic_rom(&[0x3f, 0xfe, 0xff, 0x7e]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.x = 2;
        cpu.machine.snes.cpu.a = 0xabff;
        cpu.machine.snes.ram[0x1_0000] = 0x80;
    ...
    #[test]
    fn and_absolute_long_x_m16_crosses_target_bank_with_none_wrap() {
        let rom = synthetic_rom(&[0x3f, 0xfe, 0xff, 0x7e]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        cpu.machine.snes.cpu.x = 1;
        cpu.machine.snes.cpu.a = 0xffff;
        cpu.machine.snes.ram[0xffff] = 0x01;
        cpu.machine.snes.ram[0x1_0000] = 0x80;
    ...
    #[test]
    fn and_absolute_long_x_operand_failure_defers_pc_open_bus_and_alu_state() {
        let rom = synthetic_rom(&[0x3f, 0xfe, 0xff, 0x7e]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.x = 2;
        cpu.machine.snes.cpu.a = 0xabff;
        ...
        assert_eq!(cpu.program_address(), 0x00_8001);
        assert_eq!(cpu.machine.snes.open_bus, 0x5a);
        assert_eq!(cpu.machine.snes.cpu.a, 0xabff);
        assert!(!cpu.machine.snes.cpu.n);
        assert!(cpu.machine.snes.cpu.z);
        assert!(cpu.is_poisoned());
    }
    ...
    #[test]
    fn eor_immediate_m8_preserves_accumulator_b_and_publishes_operand() {
        let rom = synthetic_rom(&[0x49, 0xc0]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.a = 0x1240;
        ...
        assert_eq!(cpu.machine.snes.cpu.a, 0x1280);
        assert_eq!(cpu.machine.snes.open_bus, 0xc0);
        assert!(cpu.machine.snes.cpu.n);
        assert!(!cpu.machine.snes.cpu.z);
    }

    #[test]
    fn eor_immediate_m16_uses_one_word_operand_and_publishes_high_byte() {
        let rom = synthetic_rom(&[0x49, 0xff, 0x80]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        cpu.machine.snes.cpu.a = 0x80ff;
    ...
    #[test]
    fn eor_immediate_failure_defers_pc_open_bus_accumulator_and_flags() {
        let rom = synthetic_rom(&[0x49, 0xc0]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.a = 0x1240;
        cpu.machine.snes.cpu.n = false;
        ...
        assert_eq!(cpu.program_address(), 0x00_8001);
        assert_eq!(cpu.machine.snes.open_bus, 0x5a);
        assert_eq!(cpu.machine.snes.cpu.a, 0x1240);
        assert!(!cpu.machine.snes.cpu.n);
        assert!(cpu.machine.snes.cpu.z);
        assert!(cpu.is_poisoned());
    }
    ...
    #[test]
    fn bit_immediate_m8_changes_only_zero_and_publishes_operand() {
        let rom = synthetic_rom(&[0x89, 0xc0]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.a = 0x1240;
        cpu.machine.snes.cpu.n = true;
        cpu.machine.snes.cpu.v = false;
        cpu.machine.snes.cpu.z = true;
    ...
    #[test]
    fn bit_immediate_m16_uses_full_accumulator_and_sets_zero() {
        let rom = synthetic_rom(&[0x89, 0xff, 0x7f]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        cpu.machine.snes.cpu.a = 0x8000;
        cpu.machine.snes.cpu.n = false;
        cpu.machine.snes.cpu.v = true;
        cpu.machine.snes.cpu.z = false;
    ...
    #[test]
    fn bit_immediate_failure_defers_pc_open_bus_and_zero_flag() {
        let rom = synthetic_rom(&[0x89, 0xc0]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.a = 0x0040;
        cpu.machine.snes.cpu.z = true;
        cpu.machine.snes.open_bus = 0x5a;
        install_hmax_smp_failure(&mut cpu, 1_350);
    ...
    #[test]
    fn eor_absolute_y_m8_skips_internal_cycle_without_page_crossing() {
        let rom = synthetic_rom(&[0x59, 0x00, 0x01]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.db = 0x7e;
        cpu.machine.snes.cpu.y = 2;
        cpu.machine.snes.cpu.a = 0xab01;
        cpu.machine.snes.ram[0x0102] = 0x81;
    ...
    #[test]
    fn eor_absolute_y_m16_always_adds_cycle_and_reads_none_wrapped_word() {
        let rom = synthetic_rom(&[0x59, 0xfe, 0xff]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        cpu.machine.snes.cpu.xf = false;
        ...
        cpu.machine.snes.cpu.y = 1;
        cpu.machine.snes.cpu.a = 0xffff;
        cpu.machine.snes.ram[0xffff] = 0xfe;
        cpu.machine.snes.ram[0x1_0000] = 0x7f;
    ...
    #[test]
    fn eor_absolute_y_page_cross_failure_keeps_operand_and_alu_state() {
        let rom = synthetic_rom(&[0x59, 0xff, 0x01]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.db = 0x7e;
        cpu.machine.snes.cpu.y = 1;
        cpu.machine.snes.cpu.a = 0xab55;
    ...
    #[test]
    fn inc_absolute_m8_reads_then_writes_after_the_internal_cycle() {
        let rom = synthetic_rom(&[0xee, 0xfe, 0x01]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.ram[0x01fe] = 0xff;
        cpu.machine.snes.cpu.c = true;
        cpu.machine.snes.open_bus = 0x5a;
        ...
        assert_eq!(receipt.accesses.len(), 4);
        assert_eq!(receipt.accesses[2].address, 0x00_01fe);
        ...
        assert_eq!(receipt.accesses[3].address, 0x00_01fe);
        assert_eq!(receipt.accesses[3].timestamp, CpuMasterTimestamp::new(236));
        assert_eq!(
            receipt.accesses[3].kind,
            SourceCpuBusAccessKind::Write { value: 0, width: 1 }
        );
        assert_eq!(cpu.machine.snes.ram[0x01fe], 0);
        assert!(cpu.machine.snes.cpu.c);
        assert!(!cpu.machine.snes.cpu.n);
        assert!(cpu.machine.snes.cpu.z);
        assert_eq!(cpu.machine.snes.open_bus, 0);
    ...
    #[test]
    fn inc_absolute_m16_wraps_none_and_writes_high_then_low() {
        let rom = synthetic_rom(&[0xee, 0xff, 0xff]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        cpu.machine.snes.cpu.db = 0x7e;
        cpu.machine.snes.ram[0xffff] = 0xff;
        cpu.machine.snes.ram[0x1_0000] = 0xff;
        cpu.machine.snes.cpu.c = false;
        cpu.machine.snes.open_bus = 0x5a;
        ...
        assert_eq!(
            receipt.accesses[4].kind,
            SourceCpuBusAccessKind::Write { value: 0, width: 1 }
        );
        assert_eq!(
            receipt.accesses[5].kind,
            SourceCpuBusAccessKind::Write { value: 0, width: 1 }
        );
        assert_eq!(cpu.machine.snes.ram[0xffff], 0);
    ...
    #[test]
    fn inc_absolute_write_failure_commits_memory_but_defers_flags_and_open_bus() {
        let rom = synthetic_rom(&[0xee, 0xfe, 0x01]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.ram[0x01fe] = 0x7f;
        cpu.machine.snes.cpu.n = false;
        cpu.machine.snes.cpu.z = true;
        cpu.machine.snes.open_bus = 0x5a;
        install_hmax_smp_failure(&mut cpu, 1_320);
        ...
        assert_eq!(cpu.program_address(), 0x00_8003);
        assert_eq!(cpu.machine.snes.ram[0x01fe], 0x80);
        assert_eq!(cpu.machine.snes.open_bus, 0x7f);
        assert!(!cpu.machine.snes.cpu.n);
        assert!(cpu.machine.snes.cpu.z);
        assert_eq!(
    ...
    #[test]
    fn inc_absolute_x_m8_owns_index_read_internal_and_write_cycles() {
        let rom = synthetic_rom(&[0xfe, 0x00, 0x01]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.db = 0x7e;
        cpu.machine.snes.cpu.x = 2;
        cpu.machine.snes.ram[0x0102] = 0xff;
        cpu.machine.snes.cpu.c = true;
        ...
        assert_eq!(receipt.accesses[2].address, 0x7e_0102);
        assert_eq!(receipt.accesses[3].address, 0x7e_0102);
        assert_eq!(cpu.machine.snes.ram[0x0102], 0);
        assert_eq!(cpu.machine.snes.open_bus, 0);
        assert!(cpu.machine.snes.cpu.c);
        assert!(!cpu.machine.snes.cpu.n);
        assert!(cpu.machine.snes.cpu.z);
    ...
    #[test]
    fn inc_absolute_x_m16_crosses_bank_and_writes_high_then_low() {
        let rom = synthetic_rom(&[0xfe, 0xfe, 0xff]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        cpu.machine.snes.cpu.xf = false;
    ...
    #[test]
    fn inc_absolute_x_index_failure_precedes_read_modify_and_flags() {
        let rom = synthetic_rom(&[0xfe, 0x10, 0x00]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.db = 0x7e;
        cpu.machine.snes.cpu.x = 2;
        cpu.machine.snes.ram[0x0012] = 0x7f;
        cpu.machine.snes.cpu.n = false;
        cpu.machine.snes.cpu.z = true;
        cpu.machine.snes.open_bus = 0x7c;
        install_hmax_smp_failure(&mut cpu, 1_334);
        ...
        assert_eq!(cpu.program_address(), 0x00_8003);
        assert_eq!(cpu.machine.snes.ram[0x0012], 0x7f);
        assert_eq!(cpu.machine.snes.open_bus, 0);
        assert!(!cpu.machine.snes.cpu.n);
        assert!(cpu.machine.snes.cpu.z);
        assert_eq!(cpu.machine.pending_completion(), None);
    ...
    #[test]
    fn dec_absolute_m8_reads_modifies_and_writes_after_internal_cycle() {
        let rom = synthetic_rom(&[0xce, 0x00, 0x01]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.db = 0x7e;
        cpu.machine.snes.ram[0x0100] = 0;
        cpu.machine.snes.cpu.c = true;
        ...
        assert_eq!(receipt.accesses[2].address, 0x7e_0100);
        assert_eq!(receipt.accesses[3].address, 0x7e_0100);
        assert_eq!(cpu.machine.snes.ram[0x0100], 0xff);
        assert_eq!(cpu.machine.snes.open_bus, 0xff);
        assert!(cpu.machine.snes.cpu.c);
        assert!(cpu.machine.snes.cpu.n);
        assert!(!cpu.machine.snes.cpu.z);
    ...
    #[test]
    fn dec_absolute_m16_wraps_none_and_writes_high_then_low() {
        let rom = synthetic_rom(&[0xce, 0xff, 0xff]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        cpu.machine.snes.cpu.db = 0x7e;
        cpu.machine.snes.ram[0xffff] = 0;
        cpu.machine.snes.ram[0x1_0000] = 0;
        ...
        assert_eq!(receipt.accesses[4].address, 0x7f_0000);
        assert_eq!(receipt.accesses[5].address, 0x7e_ffff);
        assert_eq!(cpu.machine.snes.ram[0xffff], 0xff);
        assert_eq!(cpu.machine.snes.ram[0x1_0000], 0xff);
        assert_eq!(cpu.machine.snes.open_bus, 0xff);
        assert!(cpu.machine.snes.cpu.n);
        assert!(!cpu.machine.snes.cpu.z);
    }
    ...
    #[test]
    fn dec_absolute_write_failure_commits_memory_but_defers_flags_and_open_bus() {
        let rom = synthetic_rom(&[0xce, 0xfe, 0x01]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.ram[0x01fe] = 0x80;
        cpu.machine.snes.cpu.n = true;
        cpu.machine.snes.cpu.z = false;
    ...
    #[test]
    fn dec_direct_m8_charges_nonzero_dp_then_reads_modifies_and_writes() {
        let rom = synthetic_rom(&[0xc6, 0x14]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.dp = 0x0101;
        cpu.machine.snes.ram[0x0115] = 0;
        cpu.machine.snes.cpu.c = true;
        cpu.machine.snes.open_bus = 0x5a;
        ...
        assert_eq!(receipt.accesses.len(), 4);
        assert_eq!(receipt.accesses[2].address, 0x00_0115);
        assert_eq!(receipt.accesses[2].timestamp, CpuMasterTimestamp::new(220));
        assert_eq!(receipt.accesses[3].address, 0x00_0115);
        assert_eq!(receipt.accesses[3].timestamp, CpuMasterTimestamp::new(234));
    ...
    #[test]
    fn dec_direct_m16_uses_bank_wrapped_word_and_high_low_write_order() {
        let rom = synthetic_rom(&[0xc6, 0xfe]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        cpu.machine.snes.cpu.dp = 0x0101;
        cpu.machine.snes.ram[0x01ff] = 0x00;
        cpu.machine.snes.ram[0x0200] = 0x80;
        cpu.machine.snes.open_bus = 0x5a;
        ...
        assert_eq!(receipt.accesses.len(), 4);
        assert_eq!(receipt.accesses[2].address, 0x00_01ff);
        assert_eq!(receipt.accesses[3].address, 0x00_01ff);
        assert_eq!(
            receipt.accesses[3].kind,
            SourceCpuBusAccessKind::Write {
                value: 0x7fff,
        ...
        assert_eq!(cpu.machine.snes.ram[0x01ff], 0xff);
        assert_eq!(cpu.machine.snes.ram[0x0200], 0x7f);
        assert!(!cpu.machine.snes.cpu.n);
        assert!(!cpu.machine.snes.cpu.z);
        assert_eq!(cpu.machine.snes.open_bus, 0xff);
    }
    ...
    #[test]
    fn dec_direct_write_failure_commits_memory_but_defers_flags_and_open_bus() {
        let rom = synthetic_rom(&[0xc6, 0x14]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.dp = 0x0101;
        cpu.machine.snes.ram[0x0115] = 0;
    ...
    #[test]
    fn dec_accumulator_m8_preserves_b_and_wraps_low_byte() {
        let rom = synthetic_rom(&[0x3a]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.a = 0x1200;
        ...
        assert_eq!(receipt.accesses.len(), 1);
        assert_eq!(cpu.machine.snes.cpu.a, 0x12ff);
        assert!(cpu.machine.snes.cpu.n);
        assert!(!cpu.machine.snes.cpu.z);
    }

    #[test]
    fn dec_accumulator_m16_decrements_full_word_to_zero() {
        let rom = synthetic_rom(&[0x3a]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
    ...
    #[test]
    fn dec_accumulator_event_failure_precedes_register_and_flag_mutation() {
        let rom = synthetic_rom(&[0x3a]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.a = 0x1200;
        cpu.machine.snes.cpu.n = false;
        ...
        assert_eq!(cpu.machine.snes.cpu.a, 0x1200);
        assert!(!cpu.machine.snes.cpu.n);
        assert!(cpu.machine.snes.cpu.z);
        assert_eq!(cpu.program_address(), 0x00_8001);
        assert!(cpu.is_poisoned());
    ...
    #[test]
    fn tsc_transfers_the_full_stack_pointer_and_sets_word_flags() {
        let rom = synthetic_rom(&[0x3b]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.a = 0x1234;
        cpu.machine.snes.cpu.sp = 0x8000;
        cpu.machine.snes.cpu.n = false;
        cpu.machine.snes.cpu.z = true;
    ...
    #[test]
    fn tsc_event_failure_precedes_accumulator_and_flag_mutation() {
        let rom = synthetic_rom(&[0x3b]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.a = 0x1234;
        cpu.machine.snes.cpu.sp = 0;
        cpu.machine.snes.cpu.n = true;
        cpu.machine.snes.cpu.z = false;
        install_hmax_smp_failure(&mut cpu, 1_350);
    ...
    #[test]
    fn nop_owns_only_the_source_internal_cycle() {
    ...
    #[test]
    fn nop_event_failure_is_terminal_after_the_owned_opcode_fetch() {
        let rom = synthetic_rom(&[0xea]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        install_hmax_smp_failure(&mut cpu, 1_350);
    ...
    #[test]
    fn lsr_direct_m8_reads_then_shifts_and_writes_after_internal_cycle() {
        let rom = synthetic_rom(&[0x46, 0x14]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.dp = 0x0101;
        cpu.machine.snes.ram[0x0115] = 0x81;
        cpu.machine.snes.open_bus = 0x5a;
        ...
        assert_eq!(cpu.machine.snes.ram[0x0115], 0x40);
        assert_eq!(cpu.machine.snes.open_bus, 0x40);
        assert!(cpu.machine.snes.cpu.c);
        assert!(!cpu.machine.snes.cpu.n);
        assert!(!cpu.machine.snes.cpu.z);
    ...
    #[test]
    fn lsr_direct_m16_uses_bank_word_and_high_low_write_order() {
        let rom = synthetic_rom(&[0x46, 0x14]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        cpu.machine.snes.cpu.dp = 0x0101;
        cpu.machine.snes.ram[0x0115] = 0x01;
        cpu.machine.snes.ram[0x0116] = 0x80;
        ...
        assert_eq!(&cpu.machine.snes.ram[0x0115..=0x0116], &[0x00, 0x40]);
        assert_eq!(cpu.machine.snes.open_bus, 0);
        assert!(cpu.machine.snes.cpu.c);
        assert!(!cpu.machine.snes.cpu.n);
        assert!(!cpu.machine.snes.cpu.z);
    ...
    #[test]
    fn lsr_direct_internal_failure_retains_carry_but_not_write_or_flags() {
        let rom = synthetic_rom(&[0x46, 0x14]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.dp = 0x0101;
        cpu.machine.snes.ram[0x0115] = 0x81;
        ...
        cpu.machine.snes.cpu.z = true;
        cpu.machine.snes.open_bus = 0x5a;
        install_hmax_smp_failure(&mut cpu, 1_328);
        ...
        assert_eq!(cpu.machine.snes.ram[0x0115], 0x81);
        assert_eq!(cpu.machine.snes.open_bus, 0x14);
        assert!(cpu.machine.snes.cpu.c);
        assert!(cpu.machine.snes.cpu.n);
        assert!(cpu.machine.snes.cpu.z);
    ...
    #[test]
    fn lsr_accumulator_m8_preserves_b_and_uses_bit_zero_for_carry() {
        let rom = synthetic_rom(&[0x4a]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.a = 0x1281;
        cpu.machine.snes.cpu.c = false;
        cpu.machine.snes.cpu.n = true;
    ...
    #[test]
    fn lsr_accumulator_m16_shifts_the_full_word_to_zero() {
        let rom = synthetic_rom(&[0x4a]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
    ...
    #[test]
    fn lsr_accumulator_event_failure_precedes_accumulator_and_flag_mutation() {
        let rom = synthetic_rom(&[0x4a]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.a = 0x1281;
        cpu.machine.snes.cpu.c = false;
        ...
        assert_eq!(cpu.machine.snes.cpu.a, 0x1281);
        assert!(!cpu.machine.snes.cpu.c);
        assert!(cpu.machine.snes.cpu.n);
        assert!(cpu.machine.snes.cpu.z);
        assert!(cpu.is_poisoned());
    ...
    #[test]
    fn inx_x8_wraps_and_zero_extends_after_the_internal_cycle() {
        let rom = synthetic_rom(&[0xe8]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.x = 0x12ff;
        cpu.machine.snes.cpu.n = true;
    ...
    #[test]
    fn inx_x16_increments_the_full_index_and_sets_sign() {
        let rom = synthetic_rom(&[0xe8]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.xf = false;
        cpu.machine.snes.cpu.x = 0x7fff;
        ...
        assert_eq!(cpu.machine.snes.cpu.x, 0x8000);
        assert!(cpu.machine.snes.cpu.n);
        assert!(!cpu.machine.snes.cpu.z);
    }

    #[test]
    fn inx_event_failure_precedes_index_and_flag_mutation() {
        let rom = synthetic_rom(&[0xe8]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.x = 0x007f;
        cpu.machine.snes.cpu.n = false;
        cpu.machine.snes.cpu.z = true;
        install_hmax_smp_failure(&mut cpu, 1_350);
        ...
        assert_eq!(cpu.machine.snes.cpu.x, 0x007f);
        assert!(!cpu.machine.snes.cpu.n);
        assert!(cpu.machine.snes.cpu.z);
        assert!(cpu.is_poisoned());
    }
    ...
    #[test]
    fn dey_x8_wraps_and_zero_extends_after_the_internal_cycle() {
        let rom = synthetic_rom(&[0x88]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.y = 0x1200;
        cpu.machine.snes.cpu.n = false;
        cpu.machine.snes.cpu.z = true;
        ...
        assert_eq!(cpu.machine.snes.cpu.y, 0x00ff);
        assert!(cpu.machine.snes.cpu.n);
        assert!(!cpu.machine.snes.cpu.z);
    }

    #[test]
    fn dey_x16_decrements_the_full_index_and_sets_sign() {
        let rom = synthetic_rom(&[0x88]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.xf = false;
        cpu.machine.snes.cpu.y = 0;
        cpu.machine.snes.cpu.n = false;
        cpu.machine.snes.cpu.z = true;
        ...
        assert_eq!(cpu.machine.snes.cpu.y, 0xffff);
        assert!(cpu.machine.snes.cpu.n);
        assert!(!cpu.machine.snes.cpu.z);
    }

    #[test]
    fn dey_event_failure_precedes_index_and_flag_mutation() {
        let rom = synthetic_rom(&[0x88]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
    ...
    #[test]
    fn txy_x8_transfers_low_x_into_zero_extended_index_state() {
        let rom = synthetic_rom(&[0x9b]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.x = 0x00c1;
        cpu.machine.snes.cpu.y = 0x005a;
    ...
    #[test]
    fn txy_x16_copies_full_x_and_sets_word_flags() {
        let rom = synthetic_rom(&[0x9b]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.xf = false;
        cpu.machine.snes.cpu.x = 0x8000;
        cpu.machine.snes.cpu.y = 0x1234;
    ...
    #[test]
    fn txy_event_failure_precedes_index_and_flag_mutation() {
        let rom = synthetic_rom(&[0x9b]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.x = 0x00c1;
        cpu.machine.snes.cpu.y = 0x005a;
        ...
        assert_eq!(cpu.machine.snes.cpu.x, 0x00c1);
        assert_eq!(cpu.machine.snes.cpu.y, 0x005a);
        assert!(!cpu.machine.snes.cpu.n);
        assert!(cpu.machine.snes.cpu.z);
        assert_eq!(cpu.program_address(), 0x00_8001);
    ...
    #[test]
    fn tyx_x8_transfers_low_y_into_zero_extended_index_state() {
        let rom = synthetic_rom(&[0xbb]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.y = 0x1280;
        cpu.machine.snes.cpu.x = 0x005a;
    ...
    #[test]
    fn tyx_x16_copies_full_y_and_sets_word_flags() {
        let rom = synthetic_rom(&[0xbb]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.xf = false;
        cpu.machine.snes.cpu.y = 0x8000;
        cpu.machine.snes.cpu.x = 0x1234;
    ...
    #[test]
    fn tyx_event_failure_precedes_index_and_flag_mutation() {
        let rom = synthetic_rom(&[0xbb]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.y = 0x1280;
        cpu.machine.snes.cpu.x = 0x005a;
        ...
        assert_eq!(cpu.machine.snes.cpu.y, 0x1280);
        assert_eq!(cpu.machine.snes.cpu.x, 0x005a);
        assert!(!cpu.machine.snes.cpu.n);
        assert!(cpu.machine.snes.cpu.z);
        assert_eq!(cpu.program_address(), 0x00_8001);
        assert!(cpu.is_poisoned());
    ...
    #[test]
    fn txa_m8_preserves_accumulator_b_and_sets_byte_flags() {
        let rom = synthetic_rom(&[0x8a]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.a = 0x1277;
        cpu.machine.snes.cpu.x = 0x0080;
    ...
    #[test]
    fn txa_m16_copies_full_index_register_and_sets_word_flags() {
        let rom = synthetic_rom(&[0x8a]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
    ...
    #[test]
    fn txa_event_failure_precedes_accumulator_and_flag_mutation() {
        let rom = synthetic_rom(&[0x8a]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.a = 0x1277;
        cpu.machine.snes.cpu.x = 0x0080;
        ...
        assert_eq!(cpu.machine.snes.cpu.a, 0x1277);
        assert!(!cpu.machine.snes.cpu.n);
        assert!(cpu.machine.snes.cpu.z);
        assert!(cpu.is_poisoned());
    }
    ...
    #[test]
    fn tya_m8_preserves_accumulator_b_and_sets_byte_flags() {
        let rom = synthetic_rom(&[0x98]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.a = 0x1277;
        cpu.machine.snes.cpu.y = 0x0080;
    ...
    #[test]
    fn tya_m16_copies_full_index_register_and_sets_word_flags() {
        let rom = synthetic_rom(&[0x98]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
    ...
    #[test]
    fn tya_event_failure_precedes_accumulator_and_flag_mutation() {
        let rom = synthetic_rom(&[0x98]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.a = 0x1277;
        cpu.machine.snes.cpu.y = 0x0080;
    ...
    #[test]
    fn sec_sets_carry_before_its_internal_cycle_drain() {
        let rom = synthetic_rom(&[0x38]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.c = false;
    ...
    #[test]
    fn sec_event_failure_retains_the_already_published_carry() {
        let rom = synthetic_rom(&[0x38]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.c = false;
        install_hmax_smp_failure(&mut cpu, 1_350);
    ...
    #[test]
    fn sbc_immediate_binary_m8_preserves_b_and_reports_borrow() {
        let rom = synthetic_rom(&[0xe9, 0x01]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.a = 0x1200;
        cpu.machine.snes.cpu.c = true;
    ...
    #[test]
    fn sbc_immediate_binary_m16_sets_signed_overflow_from_binary_result() {
        let rom = synthetic_rom(&[0xe9, 0x01, 0x00]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        cpu.machine.snes.cpu.a = 0x8000;
        ...
        assert_eq!(cpu.machine.snes.cpu.a, 0x7fff);
        assert!(cpu.machine.snes.cpu.c);
        assert!(!cpu.machine.snes.cpu.n);
        assert!(!cpu.machine.snes.cpu.z);
        assert!(cpu.machine.snes.cpu.v);
    ...
    #[test]
    fn sbc_immediate_decimal_matches_snes9x_nibble_correction_in_both_widths() {
        let mut m8 =
            Snes9xColdCpuExecutor::from_lorom_reset(&synthetic_rom(&[0xe9, 0x01])).unwrap();
        m8.machine.snes.cpu.a = 0x1200;
        m8.machine.snes.cpu.c = true;
        m8.machine.snes.cpu.d = true;
        m8.step().unwrap();
        assert_eq!(m8.machine.snes.cpu.a, 0x1299);
        assert!(!m8.machine.snes.cpu.c);

        let mut m16 =
            Snes9xColdCpuExecutor::from_lorom_reset(&synthetic_rom(&[0xe9, 0x01, 0x00])).unwrap();
        m16.machine.snes.cpu.e = false;
        m16.machine.snes.cpu.mf = false;
        m16.machine.snes.cpu.a = 0x1000;
        m16.machine.snes.cpu.c = true;
        m16.machine.snes.cpu.d = true;
        m16.step().unwrap();
        assert_eq!(m16.machine.snes.cpu.a, 0x0999);
        assert!(m16.machine.snes.cpu.c);
    }

    #[test]
    fn sbc_immediate_failure_defers_operand_pc_and_all_alu_state() {
        let rom = synthetic_rom(&[0xe9, 0x01]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.a = 0x1200;
        cpu.machine.snes.cpu.c = true;
    ...
    #[test]
    fn sbc_direct_m8_charges_nonzero_dp_and_preserves_accumulator_b() {
        let rom = synthetic_rom(&[0xe5, 0x10]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.dp = 1;
        cpu.machine.snes.cpu.a = 0x1200;
        cpu.machine.snes.cpu.c = true;
        cpu.machine.snes.ram[0x11] = 1;
    ...
    #[test]
    fn sbc_direct_m16_uses_bank_word_owner_and_publishes_high_byte() {
        let rom = synthetic_rom(&[0xe5, 0x10]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
    ...
    #[test]
    fn sbc_direct_operand_failure_precedes_alu_state_mutation() {
        let rom = synthetic_rom(&[0xe5, 0x10]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.a = 0x1200;
        cpu.machine.snes.cpu.c = true;
    ...
    #[test]
    fn sbc_absolute_m8_uses_data_bank_and_preserves_accumulator_b() {
        let rom = synthetic_rom(&[0xed, 0x34, 0x12]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.db = 0x7e;
        cpu.machine.snes.cpu.a = 0x1200;
        cpu.machine.snes.cpu.c = true;
        cpu.machine.snes.ram[0x1234] = 1;
    ...
    #[test]
    fn sbc_absolute_m16_reads_none_wrapped_word_and_publishes_high_byte() {
        let rom = synthetic_rom(&[0xed, 0x34, 0x12]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
    ...
    #[test]
    fn sbc_absolute_read_failure_preserves_alu_after_operand_publication() {
        let rom = synthetic_rom(&[0xed, 0x34, 0x12]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.db = 0x7e;
        cpu.machine.snes.cpu.a = 0x1200;
        ...
        assert_eq!(
            cpu.machine.pending_completion(),
            Some(CpuSynchronousCompletion::Read(1))
        );
        assert!(cpu.is_poisoned());
    }

    #[test]
    fn sbc_absolute_long_m8_uses_explicit_bank_and_preserves_accumulator_b() {
        let rom = synthetic_rom(&[0xef, 0x34, 0x12, 0x7e]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.a = 0x1200;
        cpu.machine.snes.cpu.c = true;
        cpu.machine.snes.ram[0x1234] = 1;
    ...
    #[test]
    fn sbc_absolute_long_m16_crosses_bank_with_none_wrapped_word() {
        let rom = synthetic_rom(&[0xef, 0xff, 0xff, 0x7e]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        cpu.machine.snes.cpu.a = 0x8000;
    ...
    #[test]
    fn sbc_absolute_long_operand_failure_defers_pc_open_bus_and_alu_state() {
        let rom = synthetic_rom(&[0xef, 0x34, 0x12, 0x7e]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.a = 0x1200;
        cpu.machine.snes.cpu.c = true;
    ...
    #[test]
    fn bpl_not_taken_reads_offset_without_internal_branch_cycle() {
        let rom = synthetic_rom(&[0x10, 0x7f]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.n = true;
    ...
    #[test]
    fn bpl_taken_adds_internal_cycle_then_publishes_relative_target() {
        let rom = synthetic_rom(&[0x10, 0x02]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.n = false;
    ...
    #[test]
    fn bpl_taken_event_failure_keeps_sequential_pc_and_published_offset() {
        let rom = synthetic_rom(&[0x10, 0x02]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.n = false;
        cpu.machine.snes.open_bus = 0x5a;
        install_hmax_smp_failure(&mut cpu, 1_344);
    ...
    #[test]
    fn bcc_uses_carry_clear_condition_with_shared_source_branch_timing() {
        let rom = synthetic_rom(&[0x90, 0x02]);
        let mut not_taken = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        not_taken.machine.snes.cpu.c = true;
        ...
        taken.machine.snes.cpu.c = false;
    ...
    fn adc_direct_m8_charges_nonzero_dp_then_sets_binary_flags() {
        ...
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.dp = 1;
        cpu.machine.snes.cpu.a = 0xab7f;
        cpu.machine.snes.cpu.c = false;
        cpu.machine.snes.ram[0x0011] = 1;
        cpu.machine.snes.open_bus = 0x5a;
        ...
        assert_eq!(receipt.accesses[2].address, 0x00_0011);
        assert_eq!(cpu.machine.snes.cpu.a, 0xab80);
        assert!(!cpu.machine.snes.cpu.c);
        assert!(cpu.machine.snes.cpu.v);
        assert!(cpu.machine.snes.cpu.n);
    ...
    #[test]
    fn adc_direct_m16_uses_bank_word_and_decimal_carry() {
        let rom = synthetic_rom(&[0x65, 0x20]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        cpu.machine.snes.cpu.d = true;
        cpu.machine.snes.cpu.a = 0x9999;
        cpu.machine.snes.cpu.c = false;
        cpu.machine.snes.ram[0x0020] = 1;
        cpu.machine.snes.ram[0x0021] = 0;
        cpu.machine.snes.open_bus = 0x5a;
        ...
        assert_eq!(receipt.accesses[2].address, 0x00_0020);
        assert_eq!(cpu.machine.snes.cpu.a, 0);
        assert!(cpu.machine.snes.cpu.c);
        assert!(cpu.machine.snes.cpu.z);
        assert!(!cpu.machine.snes.cpu.n);
    ...
    #[test]
    fn adc_direct_read_failure_preserves_alu_state_after_operand_publication() {
        let rom = synthetic_rom(&[0x65, 0x10]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.a = 0xab7f;
        cpu.machine.snes.cpu.c = false;
        ...
        assert_eq!(cpu.program_address(), 0x00_8002);
        assert_eq!(cpu.machine.snes.cpu.a, 0xab7f);
        assert!(!cpu.machine.snes.cpu.c);
        assert!(!cpu.machine.snes.cpu.v);
        assert!(!cpu.machine.snes.cpu.n);
        assert!(cpu.machine.snes.cpu.z);
    ...
    #[test]
    fn adc_absolute_m8_uses_data_bank_and_sets_binary_flags() {
        let rom = synthetic_rom(&[0x6d, 0x00, 0x01]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.db = 0x7e;
        cpu.machine.snes.cpu.a = 0xab7f;
        cpu.machine.snes.cpu.c = false;
        cpu.machine.snes.ram[0x0100] = 1;
        cpu.machine.snes.open_bus = 0x5a;
    ...
    #[test]
    fn adc_absolute_m16_uses_none_wrapped_word_and_decimal_carry() {
        let rom = synthetic_rom(&[0x6d, 0xff, 0xff]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        cpu.machine.snes.cpu.d = true;
    ...
    #[test]
    fn adc_absolute_read_failure_preserves_alu_after_operand_publication() {
        let rom = synthetic_rom(&[0x6d, 0x00, 0x01]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.db = 0x7e;
        cpu.machine.snes.cpu.a = 0xab7f;
        ...
        cpu.machine.snes.ram[0x0100] = 1;
        cpu.machine.snes.open_bus = 0x5a;
        install_hmax_smp_failure(&mut cpu, 1_332);
    ...
    #[test]
    fn adc_absolute_x_m8_skips_internal_cycle_without_page_crossing() {
        let rom = synthetic_rom(&[0x7d, 0x00, 0x01]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.db = 0x7e;
        cpu.machine.snes.cpu.x = 2;
        cpu.machine.snes.cpu.a = 0xab01;
        cpu.machine.snes.cpu.c = false;
        cpu.machine.snes.ram[0x0102] = 0x7f;
        cpu.machine.snes.open_bus = 0x5a;
    ...
    #[test]
    fn adc_absolute_y_m8_skips_internal_cycle_without_page_crossing() {
        let rom = synthetic_rom(&[0x79, 0x00, 0x01]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.db = 0x7e;
        cpu.machine.snes.cpu.y = 2;
        cpu.machine.snes.cpu.a = 0xab01;
    ...
    #[test]
    fn adc_absolute_y_m16_always_adds_cycle_and_reads_none_wrapped_word() {
        let rom = synthetic_rom(&[0x79, 0xfe, 0xff]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        cpu.machine.snes.cpu.xf = false;
    ...
    #[test]
    fn adc_absolute_x_m16_always_adds_cycle_and_reads_none_wrapped_word() {
        let rom = synthetic_rom(&[0x7d, 0xfe, 0xff]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        cpu.machine.snes.cpu.xf = false;
    ...
    #[test]
    fn adc_absolute_x_page_cross_failure_precedes_operand_and_alu_state() {
        let rom = synthetic_rom(&[0x7d, 0xff, 0x00]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.db = 0x7e;
        cpu.machine.snes.cpu.x = 1;
        cpu.machine.snes.cpu.a = 0xab7f;
    ...
    #[test]
    fn jmp_absolute_long_uses_one_fast_long_operand_and_preserves_open_bus() {
        let rom = synthetic_rom(&[0x5c, 0x34, 0x92, 0x7e]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.open_bus = 0x5a;
    ...
    #[test]
    fn jmp_absolute_long_operand_failure_precedes_pc_bank_and_open_bus_publication() {
        let rom = synthetic_rom(&[0x5c, 0x34, 0x92, 0x7e]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.open_bus = 0x5a;
        install_hmax_smp_failure(&mut cpu, 1_334);
    ...
    #[test]
    fn stz_absolute_x_m8_always_charges_index_cycle_before_store() {
        let rom = synthetic_rom(&[0x9e, 0xff, 0x00]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.db = 0x7e;
        cpu.machine.snes.cpu.x = 2;
        cpu.machine.snes.ram[0x0101] = 0xa5;
        cpu.machine.snes.open_bus = 0x5a;
        ...
        assert_eq!(cpu.machine.snes.ram[0x0101], 0);
        assert_eq!(cpu.machine.snes.open_bus, 0);
    }

    #[test]
    fn stz_absolute_x_m16_carries_index_and_uses_none_wrapped_word() {
        let rom = synthetic_rom(&[0x9e, 0x02, 0x00]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        cpu.machine.snes.cpu.xf = false;
        cpu.machine.snes.cpu.db = 0x7e;
        cpu.machine.snes.cpu.x = 0xffff;
        cpu.machine.snes.ram[0x10001] = 0xa5;
        cpu.machine.snes.ram[0x10002] = 0x5a;
        cpu.machine.snes.open_bus = 0x7c;
        ...
        assert_eq!(&cpu.machine.snes.ram[0x10001..0x10003], &[0, 0]);
        assert_eq!(cpu.machine.snes.open_bus, 0);
    }

    #[test]
    fn stz_absolute_x_index_cycle_failure_precedes_store_and_open_bus() {
        let rom = synthetic_rom(&[0x9e, 0x10, 0x00]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.db = 0x7e;
        cpu.machine.snes.cpu.x = 2;
        cpu.machine.snes.ram[0x0012] = 0xa5;
        cpu.machine.snes.open_bus = 0x7c;
        install_hmax_smp_failure(&mut cpu, 1_334);
        ...
        assert_eq!(cpu.program_address(), 0x00_8003);
        assert_eq!(cpu.machine.snes.ram[0x0012], 0xa5);
        assert_eq!(cpu.machine.snes.open_bus, 0x7c);
        assert_eq!(cpu.machine.pending_completion(), None);
        assert!(cpu.is_poisoned());
    }
    ...
    #[test]
    fn sta_direct_x_m8_emulation_wraps_index_inside_the_direct_page() {
        let rom = synthetic_rom(&[0x95, 0xff]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.dp = 0x1200;
        cpu.machine.snes.cpu.x = 2;
        cpu.machine.snes.cpu.a = 0xab80;
        cpu.machine.snes.ram[0x1201] = 0x5a;
        cpu.machine.snes.open_bus = 0x7c;
        ...
        assert_eq!(receipt.accesses.len(), 3);
        assert_eq!(receipt.accesses[2].address, 0x00_1201);
        assert_eq!(cpu.machine.snes.ram[0x1201], 0x80);
        assert_eq!(cpu.machine.snes.open_bus, 0x80);
    }

    #[test]
    fn sta_direct_x_m16_native_adds_full_index_after_nonzero_dp_cycle() {
        let rom = synthetic_rom(&[0x95, 0xff]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.mf = false;
        ...
        assert_eq!(receipt.accesses.len(), 3);
        assert_eq!(receipt.accesses[2].address, 0x00_0202);
        assert_eq!(&cpu.machine.snes.ram[0x0202..0x0204], &[0x34, 0x92]);
        assert_eq!(cpu.machine.snes.open_bus, 0x92);
    }

    #[test]
    fn sta_direct_x_index_cycle_failure_precedes_store_and_open_bus_publication() {
        let rom = synthetic_rom(&[0x95, 0x10]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.x = 2;
        cpu.machine.snes.cpu.a = 0xab80;
        cpu.machine.snes.ram[0x12] = 0x5a;
        cpu.machine.snes.open_bus = 0x7c;
        install_hmax_smp_failure(&mut cpu, 1_346);
        ...
        assert_eq!(cpu.program_address(), 0x00_8002);
        assert_eq!(cpu.machine.snes.ram[0x12], 0x5a);
        assert_eq!(cpu.machine.snes.open_bus, 0x7c);
        assert_eq!(cpu.machine.pending_completion(), None);
        assert!(cpu.is_poisoned());
    }
    ...
    #[test]
    fn stx_direct_x8_charges_nonzero_dp_then_publishes_x_after_store() {
        let rom = synthetic_rom(&[0x86, 0xff]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.dp = 0x0101;
        cpu.machine.snes.cpu.x = 0x0080;
        cpu.machine.snes.ram[0x0200] = 0x5a;
    ...
    #[test]
    fn stx_direct_x16_uses_bank_wrapped_word_owner_and_publishes_high_byte() {
        let rom = synthetic_rom(&[0x86, 0xff]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.xf = false;
        cpu.machine.snes.cpu.dp = 0x0101;
        cpu.machine.snes.cpu.x = 0x9234;
        cpu.machine.snes.ram[0x0200] = 0xa5;
        cpu.machine.snes.ram[0x0201] = 0x5a;
        cpu.machine.snes.open_bus = 0x7c;
    ...
    #[test]
    fn stx_direct_write_failure_commits_semantic_before_event_but_defers_open_bus() {
        let rom = synthetic_rom(&[0x86, 0xff]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.dp = 0x0101;
        cpu.machine.snes.cpu.x = 0x0080;
    ...
    #[test]
    fn cpx_direct_x8_charges_nonzero_dp_and_sets_unsigned_compare_flags() {
        let rom = synthetic_rom(&[0xe4, 0x14]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.dp = 0x0101;
        cpu.machine.snes.cpu.x = 0;
        cpu.machine.snes.ram[0x0115] = 1;
        cpu.machine.snes.cpu.c = true;
        cpu.machine.snes.cpu.n = false;
        cpu.machine.snes.cpu.z = true;
        cpu.machine.snes.open_bus = 0x5a;
    ...
    #[test]
    fn cpx_direct_x16_reads_bank_wrapped_word_and_sets_equality() {
        let rom = synthetic_rom(&[0xe4, 0xfe]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.e = false;
        cpu.machine.snes.cpu.xf = false;
        cpu.machine.snes.cpu.dp = 0x0101;
        cpu.machine.snes.cpu.x = 0x1234;
        cpu.machine.snes.ram[0x01ff] = 0x34;
        cpu.machine.snes.ram[0x0200] = 0x12;
        cpu.machine.snes.cpu.c = false;
        cpu.machine.snes.cpu.n = true;
        cpu.machine.snes.cpu.z = false;
        cpu.machine.snes.open_bus = 0x5a;
    ...
    #[test]
    fn cpx_direct_read_failure_precedes_compare_flag_mutation() {
        let rom = synthetic_rom(&[0xe4, 0x14]);
        let mut cpu = Snes9xColdCpuExecutor::from_lorom_reset(&rom).unwrap();
        cpu.machine.snes.cpu.dp = 0x0101;
        cpu.machine.snes.cpu.x = 0x0080;
        ...
        assert_eq!(
            cpu.machine.pending_completion(),
            Some(CpuSynchronousCompletion::Read(0x40))
        );
        assert!(cpu.is_poisoned());
    }
    ...
    fn local_zelda_rom_source_owner_runs_continuously_through_host_call_1000() {
        ...
        let mut last = None;
        for host_call in 0..=1_000u32 {
            last = Some(
                cpu.run_until_main_loop_return()
                    .unwrap_or_else(|error| panic!("host call {host_call}: {error:?}")),
            );
        }
        let last = last.unwrap();
        assert_eq!(last.ended_at, CpuMasterTimestamp::new(357_672_988));
        assert_eq!(cpu.program_address(), 0x00_80c9);
        assert_eq!(cpu.machine.timestamp(), last.ended_at);
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(225, 88)
        );
        assert!(!cpu.is_poisoned());
    }
    ...
    fn local_zelda_rom_matches_post_handoff_cpu_through_first_nmi_apui_read() {
        ...
        let txa = cpu.step().unwrap();
        assert_eq!(txa.origin_pc, 0x00_8482);
        assert_eq!(txa.opcode, 0x8a);

        assert_eq!(cpu.program_address(), 0x00_8483);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_934_654));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(253, 282)
        );
        let sec = cpu.step().unwrap();
        assert_eq!(sec.origin_pc, 0x00_8483);
        assert_eq!(sec.opcode, 0x38);

        assert_eq!(cpu.program_address(), 0x00_8484);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_934_668));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(253, 296)
        );
        let sbc = cpu.step().unwrap();
        assert_eq!(sbc.origin_pc, 0x00_8484);
        assert_eq!(sbc.opcode, 0xe9);

        cpu.step().unwrap();
        assert_eq!(cpu.program_address(), 0x00_8487);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_934_698));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(253, 326)
        );
        let bpl = cpu.step().unwrap();
        assert_eq!(bpl.origin_pc, 0x00_8487);
        assert_eq!(bpl.opcode, 0x10);

        for _ in 0..115 {
            cpu.step().unwrap();
        }
        assert_eq!(cpu.program_address(), 0x00_8056);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_938_812));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
        ...
        let jump = cpu.step().unwrap();
        assert_eq!(jump.origin_pc, 0x00_80c6);
        assert_eq!(jump.opcode, 0xdc);
        assert_eq!(cpu.program_address(), 0x0c_c120);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_939_154));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(256, 690)
        );

        for _ in 0..2 {
            cpu.step().unwrap();
        }
        assert_eq!(cpu.program_address(), 0x0c_c124);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_939_194));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(256, 730)
        );
        let bcc = cpu.step().unwrap();
        assert_eq!(bcc.origin_pc, 0x0c_c124);
        assert_eq!(bcc.opcode, 0x90);
        assert_eq!(cpu.program_address(), 0x0c_c133);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_939_216));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(256, 752)
        );

        for _ in 0..9 {
            cpu.step().unwrap();
        }
        assert_eq!(cpu.program_address(), 0x00_87a9);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_939_470));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(256, 1006)
        );
        let adc = cpu.step().unwrap();
        assert_eq!(adc.origin_pc, 0x00_87a9);
        assert_eq!(adc.opcode, 0x65);
        assert_eq!(cpu.program_address(), 0x00_87ab);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_939_502));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(256, 1038)
        );

        for _ in 0..21 {
            cpu.step().unwrap();
        }
        assert_eq!(cpu.program_address(), 0x00_8949);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_940_178));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(257, 350)
        );
        let rtl = cpu.step().unwrap();
        assert_eq!(rtl.origin_pc, 0x00_8949);
        assert_eq!(rtl.opcode, 0x6b);
        assert_eq!(cpu.program_address(), 0x02_8009);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_940_222));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(257, 394)
        );

        for _ in 0..39 {
            cpu.step().unwrap();
        }
        assert_eq!(cpu.program_address(), 0x00_e402);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_941_214));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(258, 22)
        );
        let lda = cpu.step().unwrap();
        assert_eq!(lda.origin_pc, 0x00_e402);
        assert_eq!(lda.opcode, 0xbd);
        assert_eq!(cpu.program_address(), 0x00_e405);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_941_246));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
        ...
        let inc = cpu.step().unwrap();
        assert_eq!(inc.origin_pc, 0x00_e434);
        assert_eq!(inc.opcode, 0xee);
        assert_eq!(cpu.program_address(), 0x00_e437);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_941_410));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(258, 218)
        );
        let lda = cpu.step().unwrap();
        assert_eq!(lda.origin_pc, 0x00_e437);
        assert_eq!(lda.opcode, 0xad);
        assert_eq!(cpu.program_address(), 0x00_e43a);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_941_442));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(258, 250)
        );
        let lsr = cpu.step().unwrap();
        assert_eq!(lsr.origin_pc, 0x00_e43a);
        assert_eq!(lsr.opcode, 0x4a);
        assert_eq!(cpu.program_address(), 0x00_e43b);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_941_456));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(258, 264)
        );

        for _ in 0..12 {
            cpu.step().unwrap();
        }
        assert_eq!(cpu.program_address(), 0x00_e458);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_941_702));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(258, 510)
        );
        let ldy = cpu.step().unwrap();
        assert_eq!(ldy.origin_pc, 0x00_e458);
        assert_eq!(ldy.opcode, 0xbc);
        assert_eq!(cpu.program_address(), 0x00_e45b);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_941_774));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(258, 582)
        );

        for _ in 0..21 {
            cpu.step().unwrap();
        }
        assert_eq!(cpu.program_address(), 0x00_e493);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_942_318));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(258, 1_126)
        );
        let lda = cpu.step().unwrap();
        assert_eq!(lda.origin_pc, 0x00_e493);
        assert_eq!(lda.opcode, 0xa7);
        assert_eq!(cpu.program_address(), 0x00_e495);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_942_374));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(258, 1_182)
        );
        let sta = cpu.step().unwrap();
        assert_eq!(sta.origin_pc, 0x00_e495);
        assert_eq!(sta.opcode, 0x9f);
        assert_eq!(cpu.program_address(), 0x00_e499);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_942_422));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(258, 1_230)
        );
        let xba = cpu.step().unwrap();
        assert_eq!(xba.origin_pc, 0x00_e499);
        assert_eq!(xba.opcode, 0xeb);
        assert_eq!(cpu.program_address(), 0x00_e49a);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_942_442));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(258, 1_250)
        );
        let ora = cpu.step().unwrap();
        assert_eq!(ora.origin_pc, 0x00_e49a);
        assert_eq!(ora.opcode, 0x07);
        assert_eq!(cpu.program_address(), 0x00_e49c);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_942_498));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(258, 1_306)
        );
        for _ in 0..8 {
            cpu.step().unwrap();
        }
        assert_eq!(cpu.program_address(), 0x00_e4b9);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_942_796));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(259, 240)
        );
        let ora = cpu.step().unwrap();
        assert_eq!(ora.origin_pc, 0x00_e4b9);
        assert_eq!(ora.opcode, 0x05);
        assert_eq!(cpu.program_address(), 0x00_e4bb);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_942_828));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(259, 272)
        );
        for _ in 0..5 {
            cpu.step().unwrap();
        }
        assert_eq!(cpu.program_address(), 0x00_e4d4);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_943_004));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(259, 448)
        );
        let inx = cpu.step().unwrap();
        assert_eq!(inx.origin_pc, 0x00_e4d4);
        assert_eq!(inx.opcode, 0xe8);
        assert_eq!(cpu.program_address(), 0x00_e4d5);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_943_018));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(259, 462)
        );
        let inx = cpu.step().unwrap();
        assert_eq!(inx.origin_pc, 0x00_e4d5);
        assert_eq!(inx.opcode, 0xe8);
        assert_eq!(cpu.program_address(), 0x00_e4d6);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_943_032));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(259, 476)
        );
        let dey = cpu.step().unwrap();
        assert_eq!(dey.origin_pc, 0x00_e4d6);
        assert_eq!(dey.opcode, 0x88);
        assert_eq!(cpu.program_address(), 0x00_e4d7);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_943_046));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(259, 490)
        );
        for _ in 0..160 {
            cpu.step().unwrap();
        }
        assert_eq!(cpu.program_address(), 0x00_e4e1);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_948_610));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(1, 598)
        );
        let dec = cpu.step().unwrap();
        assert_eq!(dec.origin_pc, 0x00_e4e1);
        assert_eq!(dec.opcode, 0xc6);
        assert_source_transaction_shape(
            &dec,
            &[
                (
        ...
        assert_eq!(cpu.program_address(), 0x00_e4e3);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(28_948_664));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(1, 652)
        );
        for _ in 0..5_980 {
            cpu.step().unwrap();
        }
        assert_eq!(cpu.program_address(), 0x0c_ed8b);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(29_153_716));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(151, 1_104)
        );
        let sta = cpu.step().unwrap();
        assert_eq!(sta.origin_pc, 0x0c_ed8b);
        assert_eq!(sta.opcode, 0x99);
        assert_source_transaction_shape(
            &sta,
            &[
                (
        ...
        assert_eq!(cpu.program_address(), 0x0c_ed8e);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(29_153_754));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(151, 1_142)
        );
        for _ in 0..67 {
            cpu.step().unwrap();
        }
        assert_eq!(cpu.program_address(), 0x0c_c179);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(29_155_458));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
        ...
        let cpx = cpu.step().unwrap();
        assert_eq!(cpx.origin_pc, 0x0c_c1e5);
        assert_eq!(cpx.opcode, 0xe4);
        assert_source_transaction_shape(
            &cpx,
            &[
                (
        ...
        assert_eq!(cpu.program_address(), 0x0c_c1e7);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(29_157_014));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(154, 310)
        );
        for _ in 0..9_816 {
            cpu.step().unwrap();
        }
        assert_eq!(cpu.program_address(), 0x0c_c1e9);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(29_582_316));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(204, 48)
        );
        let stx = cpu.step().unwrap();
        assert_eq!(stx.origin_pc, 0x0c_c1e9);
        assert_eq!(stx.opcode, 0x86);
        assert_source_transaction_shape(
            &stx,
            &[
                (
        ...
        assert_eq!(cpu.program_address(), 0x0c_c1eb);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(29_582_348));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(204, 80)
        );
        for _ in 0..8 {
            cpu.step().unwrap();
        }
        assert_eq!(cpu.program_address(), 0x00_85fe);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(29_582_560));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(204, 292)
        );
        let tya = cpu.step().unwrap();
        assert_eq!(tya.origin_pc, 0x00_85fe);
        assert_eq!(tya.opcode, 0x98);
        assert_source_transaction_shape(
            &tya,
            &[
                (
        ...
        assert_eq!(cpu.program_address(), 0x00_85ff);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(29_582_574));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(204, 306)
        );
        for _ in 0..6 {
            cpu.step().unwrap();
        }
        assert_eq!(cpu.program_address(), 0x00_8607);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(29_582_676));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(204, 408)
        );
        let ora = cpu.step().unwrap();
        assert_eq!(ora.origin_pc, 0x00_8607);
        assert_eq!(ora.opcode, 0x1d);
        assert_source_transaction_shape(
            &ora,
            &[
                (
        ...
        assert_eq!(cpu.program_address(), 0x00_860a);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(29_582_708));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(204, 440)
        );
        for _ in 0..457 {
            cpu.step().unwrap();
        }
        assert_eq!(cpu.program_address(), 0x00_86c3);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(29_593_244));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(212, 64)
        );
        let tyx = cpu.step().unwrap();
        assert_eq!(tyx.origin_pc, 0x00_86c3);
        assert_eq!(tyx.opcode, 0xbb);
        assert_source_transaction_shape(
            &tyx,
            &[
                (
        ...
        assert_eq!(cpu.program_address(), 0x00_86c4);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(29_593_258));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(212, 78)
        );
        let adc = cpu.step().unwrap();
        assert_eq!(adc.origin_pc, 0x00_86c4);
        assert_eq!(adc.opcode, 0x7d);
        assert_source_transaction_shape(
            &adc,
            &[
                (
        ...
        assert_eq!(adc.accesses[2].address, 0x00_85b2);
        assert_eq!(cpu.machine.snes.cpu.a, 0x9560);
        assert_eq!(cpu.machine.snes.open_bus, 0x00);
        assert_eq!(cpu.program_address(), 0x00_86c7);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(29_593_298));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(212, 118)
        );
        for _ in 0..11 {
            cpu.step().unwrap();
        }
        assert_eq!(cpu.program_address(), 0x00_86e3);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(29_593_636));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(212, 456)
        );
        let dec = cpu.step().unwrap();
        assert_eq!(dec.origin_pc, 0x00_86e3);
        assert_eq!(dec.opcode, 0x3a);
        assert_source_transaction_shape(
            &dec,
            &[
                (
        ...
        assert_eq!(cpu.program_address(), 0x00_86e4);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(29_593_650));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
            crate::CpuRasterPosition::new(212, 470)
        );
        for _ in 0..765 {
            cpu.step().unwrap();
        }
        assert_eq!(cpu.program_address(), 0x00_89ec);
        assert_eq!(cpu.machine.timestamp(), CpuMasterTimestamp::new(29_612_224));
        assert_eq!(
            cpu.machine.timeline.raster_position(),
