...
impl ZeldaState {
    ...
    pub(super) fn crystal_maiden_run_cutscene(&mut self, k: usize) {
        ...
        match self.sprite_slot(k).ai_state() {
            ...
            5 => {
                let mut j = i32::from(self.save_progress().palace_index_x2()) - 10;
                if j == 2 && self.save_progress().map_icons_indicator() < 7 {
                    self.save_progress_mut().set_map_icons_indicator(7);
                }
                if j == 14 && (self.player_resources().crystal_flags() & 0x7f) != 0x7f {
                    j = 16;
                }
                self.sprite_show_message_unconditional(CRYSTAL_MAIDEN_MSGS[(j >> 1) as usize]);
                self.sprite_slot_mut(k).increment_ai_state();
                if (self.player_resources().crystal_flags() & 0x7f) == 0x7f {
    ...
    pub(super) fn zelda_in_cell(&mut self, k: usize) {
        ...
        match self.sprite_slot(k).ai_state() {
            0 => {
                ...
                let j = self.sprite_slot(k).head_direction() as usize;
                self.sprite_slot_mut(k).set_x_velocity(ZELDA_XVEL[j] as u8);
                self.sprite_slot_mut(k).set_y_velocity(ZELDA_YVEL[j] as u8);
        ...
        match self.sprite_slot(k).ai_state() {
            0 => {
                if self.sprite_slot(k).delay_main() == 0 {
                    ...
                    self.sprite_slot_mut(k).set_delay_main(DELAY0[j]);
                    let dir = DIR0[j];
    ...
    pub(super) fn zelda_at_sanctuary(&mut self, k: usize) {
        let dir = self.sprite_direction_to_face_link(k, None) ^ 3;
        self.sprite_slot_mut(k).set_head_direction(dir);
        let msg = if self.player_resources().pendant_flags() & 7 == 7 {
            0x27
        } else if self.save_progress().map_icons_indicator() >= 3 {
            0x26
        } else {
            0x1e
        };
    ...
    pub(super) fn uncle_at_house(&mut self, k: usize) {
        ...
        match self.sprite_slot(k).ai_state() {
            ...
            1 => {
                ...
                self.player_state_mut().set_y(0x2157);
                self.player_state_mut().immobilize();
                self.sprite_slot_mut(k).increment_ai_state();
            }
            2 => {
            ...
            3 => {
                ...
                if self.sprite_slot(k).delay_main() == 0 {
                    ...
                    if j == 2 {
                        ...
                        self.sprite_slot_mut(k).increment_a();
                        if j == 0 {
                            let y_low = self.sprite_slot(k).y_low().wrapping_sub(2);
                            self.sprite_slot_mut(k).set_y_low(y_low);
                        }
                        self.sprite_slot_mut(k).set_delay_main(LEAVE_HOUSE_DELAY[j]);
                        let dir = usize::from(LEAVE_HOUSE_DIR[j]);
                        self.sprite_slot_mut(k).set_direction(dir as u8);
                        self.sprite_slot_mut(k)
    ...
    pub(super) fn uncle_draw(&mut self, k: usize) {
        ...
        let base = self.sprite_slot(k).direction() as usize * 12
            + self.sprite_slot(k).graphics() as usize * 6;
        ...
        if self.sprite_slot(k).direction() != 0 && self.sprite_slot(k).direction() != 3 {
    ...
    pub(super) fn sprite_sanctuary_mantle(&mut self, k: usize) {
        ...
        if self.sprite_slot(k).c() != 0 {
            self.sprite_slot_mut(k).set_a(0x40);
            collision = true;
            ...
            self.sprite_slot_mut(k).set_delay_aux1(7);
            collision = true;
        ...
        if collision {
            ...
            match self.sprite_slot(k).ai_state() {
                ...
                1 => {
                    ...
                    self.sprite_slot_mut(k).increment_ai_state();
                    self.sprite_slot_mut(k).set_delay_main(168);
                    self.sprite_slot_mut(k).set_x_velocity(3);
                    self.sprite_slot_mut(k).set_delay_aux1(2);
                }
                2 => {
                    self.sprite_move_xy(k);
                    if self.sprite_slot(k).delay_main() == 0 {
    ...
    pub(super) fn priest_spawn_mantle(&mut self, k: usize) {
        let marker_state = self.sprite_slot(15).state().wrapping_add(1);
        self.sprite_slot_mut(15).set_state(marker_state);
        let j = self.sprite_spawn_dynamically_for_dn(k, 0x73);
        self.sprite_slot_mut(15).set_state(0);
        let j = j.expect("Priest_SpawnMantle expected Sprite_SpawnDynamically to succeed");
        self.sprite_slot_mut(j).masked_or_flags2(0xf0, 0x3);
        self.sprite_slot_mut(j).set_x_low(0xF0);
        self.sprite_slot_mut(j).set_x_high(4);
        self.sprite_slot_mut(j).set_y_low(0x37);
        self.sprite_slot_mut(j).set_y_high(2);
        self.sprite_slot_mut(j).set_e(2);
        self.sprite_slot_mut(j).set_flags4(11);
    ...
    pub(super) fn priest_dying(&mut self, k: usize) {
        ...
        self.sprite_slot_mut(k).set_direction(4);
        match self.sprite_slot(k).ai_state() {
            0 => {
                if (self.sprite_show_solicited_message_for_dn(k, 0x1b) & 0x100) != 0 {
                    ...
                    self.sprite_slot_mut(k).set_delay_aux2(128);
                }
            }
            1 => {
    ...
    pub(super) fn priest_run_rescue_cutscene(&mut self, k: usize) {
        match self.sprite_slot(k).ai_state() {
            ...
            1 => {
                if self.follower_state().zelda_rescue_cutscene_state() == 2 {
            ...
            3 => {
                ...
                if (j & 0x100) != 0 {
                    let v = j as u8;
                    self.sprite_slot_mut(k).set_direction(v);
                    self.sprite_slot_mut(k).set_head_direction(v);
                }
            }
            _ => {}
        }
    ...
    pub(super) fn priest_chillin(&mut self, k: usize) {
        let head_direction = self.sprite_direction_to_face_link_for_dn(k) ^ 3;
        self.sprite_slot_mut(k).set_head_direction(head_direction);
        let m: u16 = if (self.player_resources().pendant_flags() & 7) == 7 {
            0x1a
        } else if self.save_progress().map_icons_indicator() >= 3 {
            0x19
        } else {
    ...
    pub(super) fn priest_spawn_rescued_princess(&mut self) {
        ...
        self.sprite_slot_mut(k).set_direction(layer_bits);
        self.sprite_slot_mut(k).set_head_direction(layer_bits);
    ...
    pub(super) fn sprite_prep_uncle_and_priest_bounce(&mut self, k: usize) {
        ...
        if room == 18 {
            ...
            if self.save_progress().progress_flags() & 2 != 0 {
                self.sprite_slot_mut(k).set_state(0);
                return;
            }
            self.sprite_slot_mut(k).set_e(1);
            self.sprite_slot_mut(k).masked_or_flags2(0xf0, 0x2);
            self.sprite_slot_mut(k).set_flags4(3);
            let j: usize;
            if self.inventory_items().sword_type() >= 2 {
                ...
                if self.follower_state().indicator() == 1 {
                    ...
                    self.overworld_event_info_mut().set_event_bits(0x1b, 0x20);
                    self.sprite_slot_mut(k).set_delay_main(170);
                    j = 1;
                } else {
                    j = 2;
                }
            }
            self.sprite_slot_mut(k).set_subtype2(j as u8);
            let x = self.sprite_get_x(k);
            ...
            if (self.save_progress().progress_flags() & 0x10) == 0 {
                let x_low = self.sprite_slot(k).x_low().wrapping_add(8);
                self.sprite_slot_mut(k).set_x_low(x_low);
            } else {
                self.sprite_slot_mut(k).set_state(0);
            }
        } else if (self.save_progress().progress_flags() & 1) == 0 {
    ...
    pub(super) fn priest_draw(&mut self, k: usize) {
        let j = (self.sprite_slot(k).direction() as usize) * 2
            + self.sprite_slot(k).graphics() as usize;
        let base = j * 2;
        let mut info = PrepOamCoordsRet {
            x: 0,
            y: 0,
    ...
    pub(super) fn sprite_c4_thief(&mut self, k: usize) {
        ...
        if self.sprite_slot(k).ai_state() != 3 {
            ...
            self.sprite_slot_mut(k).set_head_direction(j);
            if (j ^ self.sprite_slot(k).direction()) == 1 {
        ...
        match self.sprite_slot(k).ai_state() {
            0 => {
                ...
                let graphics = THIEF_GFX[usize::from(self.sprite_slot(k).direction())];
                self.sprite_slot_mut(k).set_graphics(graphics);
            }
            1 => {
                self.thief_check_collision_with_link(k);
            ...
            3 => {
                ...
                if self.sprite_slot(k).delay_main() == 0 {
                    ...
                    let i = 4 + self
                        .sprite_slot(k)
                        .direction()
                        .wrapping_add(self.sprite_slot(k).subtype2() & 4);
                    self.sprite_slot_mut(k)
    ...
    pub(super) fn thief_scan_for_booty(&mut self, k: usize) -> u8 {
        for j in (0..=15usize).rev() {
            if self.sprite_slot(j).state() != 0 {
                let t = self.sprite_slot(j).sprite_type();
    ...
    pub(super) fn thief_target_booty(&mut self, k: usize, j_in: usize) {
        ...
        if (k ^ fc) & 3 == 0 {
            ...
            self.sprite_slot_mut(k).set_x_velocity(pt.x as u8);
            self.sprite_slot_mut(k).set_y_velocity(pt.y as u8);
        }
        for j in (0..=15usize).rev() {
            // Note: C uses `!((j ^ fc) & 3 | sprite_delay_aux4[j])` — `|` has
            // lower precedence than `&`, so the parens evaluate to
            // `((j^fc)&3) | aux4[j]`.
            let cond = (((j ^ fc) & 3) as u8) | self.sprite_slot(j).delay_aux4();
            if cond == 0 && self.sprite_slot(j).state() != 0 {
                let t = self.sprite_slot(j).sprite_type();
                if t == 0xdc || t == 0xe1 || t == 0xd9 {
    ...
    pub(super) fn thief_grab_booty(&mut self, k: usize, j: usize) {
        ...
        if dx < 16 && dy < 24 {
            ...
            let t = self.sprite_slot(j).sprite_type().wrapping_sub(0xd8) as usize;
            ...
            self.sprite_slot_mut(k).set_delay_main(14);
        }
    }
    ...
    pub(super) fn thief_check_collision_with_link(&mut self, k: usize) {
        if self.sprite_check_damage_to_link_same_layer_for_dn(k) {
            ...
            self.sprite_slot_mut(k).set_y_recoil((pt.y as u8) ^ 0xff);
            self.sprite_slot_mut(k).set_x_recoil((pt.x as u8) ^ 0xff);
            self.player_state_mut().set_incapacitated_timer(4);
            self.sprite_slot_mut(k).set_f(12);
            self.sprite_sfx_queue_sfx2_with_pan(k, 0xb);
        }
    }
    ...
    pub(super) fn thief_spill_items(&mut self, k: usize) {
        ...
        loop {
            ...
            let count: u16 = if pick == 1 {
                self.player_resources().arrows() as u16
            } else if pick == 2 {
                self.player_resources().bombs() as u16
            } else {
                self.player_resources().rupees_goal()
            };
            if count == 0 {
                return;
            }
            ...
            if pick == 1 {
                ...
                let cur = self.player_resources().rupees_goal();
            ...
            self.sprite_slot_mut(j).set_z_velocity(0x18);
            let tc = self.temp_counter().value() as usize;
    ...
    pub(super) fn kiki_lying_inwait(&mut self, k: usize) {
        ...
        let gate = u8::from(self.player_state().is_bunny_mirror())
        ...
        if self.sprite_check_damage_to_link_same_layer_for_dn(k) {
            let features = self.enhanced_features().bits();
    ...
    pub(super) fn kiki_flee(&mut self, k: usize) {
        ...
        let d = if (px as u8) >= (py as u8) {
            (self.sprite_slot(k).x_velocity() >> 7) ^ 3
        } else {
    ...
    pub(super) fn kiki_offer_initial_service(&mut self, k: usize) {
        ...
        let s = self.sprite_slot(k).ai_state();
        ...
        match self.sprite_slot(k).ai_state() {
            ...
            4 => {
                if self.sprite_slot(k).delay_main() == 0 && self.sprite_slot(k).z() == 0 {
                    self.sprite_slot_mut(k).set_state(0);
                    self.player_state_mut().clear_immobilized();
                }
            }
        ...
        match self.sprite_slot(k).ai_state() {
            ...
            s @ (2 | 4 | 6) => {
                ...
                let dx = KIKI_LEAVE_X[j]
                    .wrapping_sub(self.sprite_slot(k).x_low() as u16)
                    .wrapping_add(2) as u8;
                let dy = KIKI_LEAVE_Y[j]
                    .wrapping_sub(self.sprite_slot(k).y_low() as u16)
                    .wrapping_add(2) as u8;
                if dx < 4 && dy < 4 {
                    self.sprite_slot_mut(k).increment_ai_state();
                    self.sprite_slot_mut(k).set_x_velocity(0);
            ...
            s @ (3 | 5) => {
                if self.sprite_slot(k).delay_aux1() == 0 {
                    ...
                    self.sprite_slot_mut(k).set_ai_state(new_state);
            ...
            7 => {
                ...
                if t >= 0 {
                    self.sprite_slot_mut(k).set_direction(t as u8);
                    self.sprite_slot_mut(k)
            ...
            8 => {
                self.sprite_slot_mut(k).set_direction(8);
                self.sprite_slot_mut(k).set_graphics(0);
            ...
            9 => {
                if (self.sprite_slot(k).z_velocity() as i8) < 0 && self.sprite_slot(k).z() == 0 {
                    self.sprite_slot_mut(k).increment_ai_state();
    ...
    pub(super) fn kiki_draw(&mut self, k: usize) -> bool {
        ...
        if self.sprite_slot(k).direction() < 8 {
            let j = (self.sprite_slot(k).direction() as usize) * 2
                + self.sprite_slot(k).graphics() as usize;
    ...
    pub(super) fn chicken_hopping(&mut self, k: usize) {
        ...
        if (self.sprite_slot(k).z() as i8) < 0 {
            ...
            self.sprite_slot_mut(k).set_z_velocity(10);
        }
    ...
    fn chicken_incr_subtype2(&mut self, k: usize, j: u8) {
        ...
        let graphics = (self.sprite_slot(k).subtype2() >> 4) & 1;
        self.sprite_slot_mut(k).set_graphics(graphics);
        self.sprite_return_if_lifted(k);
    }
    ...
    pub(super) fn smithy_main(&mut self, k: usize) {
        ...
        let e_idx = self.sprite_slot(k).e() as usize;
        let other = self.sprite_slot(e_idx).ai_state();
        let me = self.sprite_slot(k).ai_state();
        if (other == 5
            || other == 7
            || other == 9
            || me == 5
            ...
            || (me | other) == 0)
            && {
                let old = self.sprite_slot(k).b();
                self.sprite_slot_mut(k).set_b(old.wrapping_sub(1));
                old == 0
            }
        {
            let idx = self.sprite_slot(k).a() as usize;
            ...
            self.sprite_slot_mut(k).set_graphics(SMITHY_GFX[idx]);
            self.sprite_slot_mut(k).set_b(SMITHY_FRAME_DURATIONS[idx]);
            if idx == 1 {
                self.sprite_slot_mut(k).set_z_velocity(16);
            }
            if idx == 3 {
        ...
        match self.sprite_slot(k).ai_state() {
            0 => {
                self.sprite_slot_mut(k).set_c(0);
                if self.follower_state().indicator() != 8 {
                    ...
                    } else if (self.save_progress().progress_indicator_3() & 0x20) != 0 {
                ...
                } else if (self.player_state().y() as u8) < 0xc2 {
            ...
            3 => {
                ...
                if choice != 0 || rupees < 10 {
                    ...
                    let e_idx = self.sprite_slot(k).e() as usize;
                    self.sprite_slot_mut(e_idx).set_ai_state(5);
                    self.sprite_slot_mut(k).set_ai_state(5);
            ...
            6 => {
                self.sprite_slot_mut(k).set_ai_state(0);
                let e_idx = self.sprite_slot(k).e() as usize;
                self.sprite_slot_mut(e_idx).set_ai_state(0);
                self.player_state_mut().set_item_receipt_method(0);
    ...
    pub(super) fn smithy_listen_for_hammer(&mut self, k: usize) -> bool {
        ...
        if self.save_progress().hud_current_item() != HUD_ITEM_HAMMER {
            return false;
        }
    ...
    pub(super) fn smithy_spawn_dwarf_pal(&mut self, k: usize) -> i32 {
        ...
        let x_low = self.sprite_slot(j).x_low().wrapping_add(0x2C);
        self.sprite_slot_mut(j).set_x_low(x_low);
    ...
    pub(super) fn smithy_draw(&mut self, k: usize) {
        let idx = self.sprite_slot(k).graphics() as usize * 4
            + self.sprite_slot(k).direction() as usize * 2;
        let mut info = PrepOamCoordsRet {
            x: 0,
            y: 0,
            r4: 0,
    ...
    pub(super) fn smithy_spark(&mut self, k: usize) {
        ...
        self.sprite_slot_mut(k).set_graphics(g as u8);
        self.sprite_slot_mut(k)
    ...
    pub(super) fn smithy_spawn_spark(&mut self, k: usize) {
        if let Some(j) = self.sprite_spawn_dynamically_for_dn(k, 0x1a) {
            ...
            self.sprite_set_x(j, rx);
            self.sprite_set_y(j, ry);
            let delta: i8 = if self.sprite_slot(k).direction() != 0 {
                -15
            } else {
                15
            };
            let x_low = (self.sprite_slot(j).x_low() as i8).wrapping_add(delta) as u8;
            let y_low = self.sprite_slot(j).y_low().wrapping_add(2);
            self.sprite_slot_mut(j).set_x_low(x_low);
...
mod tests {
    ...
    fn priest_dying_state2_clears_sprite_state() {
        ...
        s.sprite_slot_mut(k).set_ai_state(2);
        s.priest_dying(k);
        assert_eq!(s.sprite_slot(k).state(), 0);
        // head_dir/D should still have been written to 4.
        assert_eq!(s.sprite_slot(k).head_direction(), 4);
        assert_eq!(s.sprite_slot(k).direction(), 4);
    }
    ...
    fn priest_chillin_picks_message_by_pendants_and_map() {
        ...
        s.player_state_mut().set_x(0x100);
        s.player_state_mut().set_y(0x10);
        s.player_resources_mut().set_pendant_flags(7);
        s.priest_chillin(k);
        assert_eq!(s.sprite_slot(k).head_direction(), 3);
    }
    ...
    fn priest_spawn_mantle_marks_slot_and_sets_props() {
        ...
        s.player_state_mut().set_y(0x100);
        ...
        let mut found = None;
        for j in 0..15 {
            if s.sprite_slot(j).e() == 2
                && s.sprite_slot(j).flags4() == 11
                && s.sprite_slot(j).subtype2() == 1
            {
                found = Some(j);
                break;
            }
        ...
        assert_eq!(s.sprite_slot(j).x_low(), 0xF0);
        assert_eq!(s.sprite_slot(j).x_high(), 4);
        assert_eq!(s.sprite_slot(j).y_low(), 0x37);
        assert_eq!(s.sprite_slot(j).y_high(), 2);
        assert_eq!(s.sprite_slot(j).deflection_bits() & 0x20, 0x20);
        assert_eq!(s.sprite_slot(j).c(), 1);
    }
    ...
    fn thief_grab_booty_absorbs_when_close() {
        ...
        s.sprite_slot_mut(j).set_sprite_type(0xd9); // rupee
        ...
        s.sprite_slot_mut(j).set_x_low(0x00);
        s.sprite_slot_mut(j).set_x_high(0x01);
        s.sprite_slot_mut(j).set_y_low(0x00);
        s.sprite_slot_mut(j).set_y_high(0x01);
        s.thief_grab_booty(k, j);
        assert_eq!(s.sprite_slot(j).state(), 0);
        assert_eq!(s.sprite_slot(k).delay_main(), 14);
    }
    ...
    fn dn_dynamic_spawn_ex_uses_c_inclusive_slot_bound() {
        let mut s = fresh_state();
        let parent = 12;
        s.sprite_slot_mut(parent).set_state(9);
        for slot in 8..=15 {
            s.sprite_slot_mut(slot).set_state(9);
        }
        s.sprite_slot_mut(7).set_state(0);
        ...
        assert_eq!(spawned, 7);
        assert_eq!(s.sprite_slot(7).sprite_type(), 0xd9);
        assert_eq!(s.sprite_slot(7).state(), 9);
    }
    ...
    fn cucco_calm_seeds_velocity_when_delay_zero() {
        ...
        let d = s.sprite_slot(k).delay_main();
    ...
    fn chicken_hopping_bounces_when_z_wraps_negative() {
        ...
        s.sprite_slot_mut(k).set_z_velocity((-16i8) as u8);
        s.sprite_slot_mut(k).set_delay_main(0);
        s.sprite_slot_mut(k).set_subtype2(0x0f);
        s.chicken_hopping(k);
        assert_eq!(s.sprite_slot(k).z(), 0);
        assert_eq!(s.sprite_slot(k).z_velocity(), 10);
        assert_eq!(s.sprite_slot(k).delay_main(), 32);
        assert_eq!(s.sprite_slot(k).ai_state(), 0);
        assert_eq!(s.sprite_slot(k).subtype2(), 0x13);
    ...
    fn smithy_spawn_dwarf_pal_writes_x_offset_and_dir() {
        ...
        assert_eq!(s.sprite_slot(j).x_low(), 0xAC);
        assert_eq!(s.sprite_slot(j).direction(), 1);
        assert_eq!(s.sprite_slot(j).a(), 4);
        assert_eq!(s.sprite_slot(j).ignore_projectile(), 4);
    }
    ...
    fn returning_smithy_homecoming_state1_clears_immobilized() {
        ...
        s.player_state_mut().immobilize();
        s.smithy_homecoming(k);
        assert!(!s.player_state().is_immobilized());
