...
impl ZeldaState {
    fn apply_dung_pal_info(&mut self, idx: u8) {
        ...
        self.palette_buffer_mut().set_sp5l(pal2);
        self.palette_buffer_mut().set_sp6l(pal3);
    }
    ...
    pub(super) fn credits_load_scene_overworld_prep_gfx(&mut self) {
        ...
        self.palette_filter_mut().set_color_window_selection(0x82);
    ...
    pub(super) fn credits_load_cool_background(&mut self) {
        self.sprite_system_mut().set_main_tile_theme(33);
        self.sprite_system_mut().set_aux_tile_theme(59);
    ...
    pub(super) fn credits_load_scene_dungeon(&mut self) {
        ...
        self.decompress_animated_dungeon_tiles(
            DUNG_ANIMATED_TILES[self.sprite_system().main_tile_theme() as usize] as usize,
        );
        self.sprite_system_mut()
    ...
    pub(super) fn module19_triforce_room(&mut self) {
        match self.frame_state().subsubmodule {
            ...
            3 => {
                self.sprite_system_mut().set_main_tile_theme(36);
                self.sprite_system_mut().set_graphics_index(125);
                self.sprite_system_mut().set_aux_tile_theme(81);
                self.initialize_tilesets();
            ...
            4 => {
                ...
                self.ppu_scroll_copy_mut().set_bg1_h_high(1);
                self.palette_filter_mut().set_color_window_selection(2);
                self.palette_filter_mut().set_color_math_control(50);
                ...
                self.system_signals_mut().set_music_control(32);
                self.set_main_module(25);
                self.set_submodule(0);
            }
            5 => {
                self.player_state_mut().set_direction_and_last_direction(8);
                self.player_state_mut().set_facing(0);
                if self.player_state().y_low() < 192 {
                    self.player_state_mut().set_direction_and_last_direction(0);
            ...
            8 | 10 => {
                ...
                if self.frame_state().subsubmodule == 11 {
                    self.system_signals_mut().set_music_control(33);
                    self.set_main_module(25);
                    self.player_state_mut().set_direction_and_last_direction(0);
                    self.increment_submodule();
                }
            ...
            13 => {
                ...
                if self.palette_filter().darkening_or_lightening_screen() == 255 {
                    self.increment_subsubmodule();
                }
            }
            14 => {
        ...
        self.ppu_scroll_copy_mut().copy_live_to_ppu_copy();
    ...
    pub(super) fn intro_clear1kb_blocks_of_wram(&mut self) {
        ...
        let mut i = self.ending_scratch().primary_word();
    ...
    pub(super) fn triforce_room_prep_gfx_slot_for_poly(&mut self) {
        self.sprite_system_mut().set_misc_sprites_graphics_index(8);
        ...
        match self.attract_scene().intro_step_index() {
            ...
            2 => {
                    ...
                    && self.poly_runtime().angle_a().wrapping_sub(11) >= 220
                {
                    ...
                    self.palette_buffer_mut().set_main_color(0xd7, 0x7fff);
        ...
        match self.attract_scene().intro_step_index() {
            0 => {
                ...
                if self.attract_scene().intro_frame_counter() & 7 == 0 {
                    self.intro_actor_mut(k).add_x_velocity(XACC[k] as u8);
                }
                if self.attract_scene().intro_frame_counter() & 3 == 0 {
                    self.intro_actor_mut(k).add_y_velocity(YACC[k] as u8);
                }
            }
            1 => {
            ...
            2 => {
                ...
                if XFINAL[k] == self.intro_actor(k).x_low() {
                    self.intro_actor_mut(k).set_x_velocity(0);
                }
                if YFINAL[k] == self.intro_actor(k).y_low() {
                    self.intro_actor_mut(k).set_y_velocity(0);
                }
            }
            3 | 4 => {
                ...
                if ctr == 0 {
                    self.intro_actor_mut(k).set_y_low(YFINAL2[k]);
                } else {
    ...
    pub(super) fn animate_triforce_room_triangle_handle_contracting(&mut self, k: usize) {
        ...
        let xv = self.intro_actor(k).x_velocity().wrapping_add(
            if self.intro_actor(k).x_low() <= XFINAL[k] {
                1
            } else {
                0xff
            },
        ...
        let yv = self.intro_actor(k).y_velocity().wrapping_add(
            if self.intro_actor(k).y_low() <= YFINAL[k] {
                1
            } else {
                0xff
            },
    ...
    pub(super) fn initialize_scene_sprite_credits_triangle(&mut self, k: usize) {
        ...
        self.intro_actor_mut(k).set_x(i16::from(X[k]));
        self.intro_actor_mut(k).set_y(i16::from(Y[k]));
        self.intro_actor_mut(k).increment_init_phase();
    }
    ...
    pub(super) fn credits_prep_and_load_sprites(&mut self) {
        ...
        match scene {
            2 => {
                self.sprite_slot_mut(6).set_y_velocity((-16i8) as u8);
                self.init_ending_sprites_overworld(scene);
            }
            3 => {
                self.sprite_slot_mut(5).set_a(22);
                self.sprite_slot_mut(0).set_y_velocity((-16i8) as u8);
                self.sprite_slot_mut(1).set_y_velocity(16);
                self.sprite_slot_mut(1).set_head_direction(1);
                for j in (0..=2).rev() {
                    self.sprite_slot_mut(2 + j).set_sprite_type(0x57);
                    self.sprite_slot_mut(2 + j).set_oam_flags(0x31);
                }
                self.init_ending_sprites_overworld(scene);
            }
            6 => {
                self.sprite_slot_mut(0).set_delay_main(255);
                self.sprite_slot_mut(1).set_delay_main(255);
                self.sprite_slot_mut(2).set_delay_main(255);
                self.init_ending_sprites_overworld(scene);
            }
            7 => {
                self.sprite_slot_mut(1).set_delay_main(255);
            ...
            9 => {
                for j in (0..=4).rev() {
                    self.sprite_slot_mut(j).set_delay_main((j * 19) as u8);
                    self.sprite_slot_mut(j).set_state(0);
                }
                self.sprite_slot_mut(5).set_sprite_type(0x2e);
                for j in (0..=1).rev() {
                    self.sprite_slot_mut(7 + j).set_sprite_type(0x9f);
                    self.sprite_slot_mut(9 + j).set_sprite_type(0xa0);
                    self.sprite_slot_mut(7 + j).set_flags2(1);
                    self.sprite_slot_mut(9 + j).set_flags2(2);
                    self.sprite_slot_mut(7 + j).set_flags3(0x10);
                    self.sprite_slot_mut(9 + j).set_flags3(0x10);
                }
                self.init_ending_sprites_overworld(scene);
            }
            10 => {
                self.sprite_slot_mut(1).set_delay_main(0x10);
                self.sprite_slot_mut(2).set_delay_main(0x20);
                self.sprite_slot_mut(3).set_oam_flags(8);
                self.sprite_slot_mut(4).set_oam_flags(8);
                self.init_ending_sprites_dungeon(scene);
            }
            11 => {
                self.sprite_slot_mut(4).set_oam_flags(0x79);
                self.sprite_slot_mut(5).set_oam_flags(0x39);
                self.sprite_slot_mut(1).set_direction(1);
                self.sprite_slot_mut(1).set_a(4);
                self.init_ending_sprites_dungeon(scene);
            }
            12 => {
                for j in (0..=1).rev() {
                    self.sprite_slot_mut(j + 3).set_oam_flags(0x39);
                    self.sprite_slot_mut(j + 3).set_sprite_type(0x0b);
                    self.sprite_slot_mut(j + 3).set_flags3(0x10);
                    self.sprite_slot_mut(j + 3).set_flags2(1);
                }
                self.sprite_slot_mut(5).set_sprite_type(0x2a);
                self.sprite_slot_mut(6).set_sprite_type(0x79);
                self.sprite_slot_mut(6).set_ai_state(1);
                self.sprite_slot_mut(6).set_z(5);
                self.init_ending_sprites_overworld(scene);
            }
            14 => {
                self.sprite_slot_mut(5).set_y_velocity((-16i8) as u8);
                self.sprite_slot_mut(6).set_y_velocity(16);
                self.sprite_slot_mut(6).set_head_direction(1);
                self.sprite_slot_mut(0).set_a(8);
                for j in (0..=3).rev() {
                    self.sprite_slot_mut(1 + j).set_y_velocity(4);
                }
                self.init_ending_sprites_overworld(scene);
            }
            15 => {
                self.sprite_slot_mut(4).set_c(2);
                self.sprite_slot_mut(5).set_y_velocity(8);
                self.sprite_slot_mut(1).set_delay_main(0x13);
                self.sprite_slot_mut(4).set_delay_main(0x40);
                self.init_ending_sprites_overworld(scene);
            }
    ...
    fn init_ending_sprites_overworld(&mut self, scene: usize) {
        ...
        for k in (0..num).rev() {
            ...
            self.sprite_slot_mut(k).set_x_low(x as u8);
            self.sprite_slot_mut(k).set_x_high((x >> 8) as u8);
            self.sprite_slot_mut(k).set_y_low(y as u8);
            self.sprite_slot_mut(k).set_y_high((y >> 8) as u8);
        }
    }
    ...
    fn init_ending_sprites_dungeon(&mut self, scene: usize) {
        ...
        for k in (0..num).rev() {
            self.garnish_state_mut().set_sprcoll_x_size(0xffff);
            self.garnish_state_mut().set_sprcoll_y_size(0xffff);
            let x = ((self.sprite_workspace().room_origin_x_high() as u16) << 8)
                .wrapping_add(ENDING_SPRITE_X_OFFSETS[idx + k]);
            let y = ((self.sprite_workspace().room_origin_y_high() as u16) << 8)
    ...
    pub(super) fn credits_scroll_scene_overworld(&mut self) {
        for k in (0..16).rev() {
            if self.sprite_slot(k).delay_main() != 0 {
                ...
                self.sprite_slot_mut(k).set_delay_main(delay);
            }
        }
        let i = (self.frame_state().submodule >> 1) as usize;
        self.player_state_mut().clear_movement_velocity();
        let r16 = self.ending_scratch().primary_word();
        if r16 >= 0x40 && r16 & 1 == 0 {
    ...
    pub(super) fn credits_handle_scene_fade(&mut self) {
        ...
        match i {
            0 => {
                for k in (8..=11).rev() {
                    self.sprite_slot_mut(k).set_oam_flags(CASE0_OAM_FLAGS[k]);
            ...
            1 => {
                ...
                let k = 1;
                self.sprite_slot_mut(k).set_sprite_type(0x73);
                self.sprite_slot_mut(k).set_oam_flags(0x27);
                self.sprite_slot_mut(k).set_e(2);
            ...
            2 => {
                ...
                let mut k = 6usize;
                let j = ((self.sprite_slot(k).x_velocity() >> 7) & 1) as usize;
                let oam_flags = self
                    .sprite_slot(k)
                    .x_velocity()
                ...
                k -= 1;
                self.sprite_slot_mut(k).set_oam_flags(0x31);
                if self.sprite_slot(k).delay_main() == 0 {
                    let j = self.sprite_slot(k).a() as usize;
                    let a = self.sprite_slot(k).a() ^ 1;
                    self.sprite_slot_mut(k).set_a(a);
                    self.sprite_slot_mut(k).set_delay_main(CASE2_DELAY[j]);
                    let graphics = self.sprite_slot(k).graphics().wrapping_add(1) & 3;
                ...
                loop {
                    if self.frame_state().frame_counter & 15 == 0 {
                        let graphics = self.sprite_slot(k).graphics() ^ 1;
                        self.sprite_slot_mut(k).set_graphics(graphics);
                    }
                    self.sprite_slot_mut(k).set_oam_flags(0x31);
                    self.credits_sprite_draw_single(
            ...
            3 => {
                ...
                while k < 5 {
                    if k < 2 {
                        self.sprite_slot_mut(k).set_sprite_type(1);
                        self.sprite_slot_mut(k).set_oam_flags(0x0b);
                        self.credits_sprite_draw_set_shadow_prop(k, 2);
                        self.sprite_slot_mut(k).set_z(48);
                        let j = ((self.frame_state().frame_counter.wrapping_add(if k != 0 {
                            ...
                            & 3) as usize;
                        self.sprite_slot_mut(k).set_graphics(CASE3_GFX[j]);
                        self.credits_sprite_draw_circling_birds(k);
            ...
            4 => {
                ...
                let mut k = 2usize;
                self.sprite_slot_mut(k).set_oam_flags(0x35);
                self.credits_sprite_draw_single(k, 1, 0x3c);
                k -= 1;
                loop {
                    let oam_flags =
                        self.sprite_slot(k).x_velocity().wrapping_sub(1) >> 1 & 0x40 ^ 0x71;
                    self.sprite_slot_mut(k).set_oam_flags(oam_flags);
                    let graphics = self.frame_state().frame_counter >> 3 & 1;
                    self.sprite_slot_mut(k).set_graphics(graphics);
                    if r16 >= CASE4_CTR[k] && self.sprite_slot(k).delay_main() == 0 {
                        let a = CASE4_DELAYVEL[self.sprite_slot(k).a() as usize];
                        self.sprite_slot_mut(k).set_delay_main(a & 0xf8);
            ...
            5 => {
                ...
                if r16 == 0x200 {
                    self.system_signals_mut().set_sound_effect_1(1);
                } else if r16 == 0x208 {
            ...
            6 => {
                ...
                for k in (0..num).rev() {
                    ...
                    self.sprite_slot_mut(k).set_sprite_type(SPR_TYPE[k]);
                    self.oam_allocate_from_region_a(OAM_SIZE[k]);
                    self.sprite_slot_mut(k).set_ai_state(STATE[k]);
                    let j = if r16 >= 0x26f { k + 3 } else { k };
                    if r16 == 0x26f {
                        self.system_signals_mut().set_sound_effect_2(0x21);
                    }
                    self.sprite_slot_mut(k).set_graphics(LOCAL_GRAPHICS[j]);
                    self.sprite_slot_mut(k).set_oam_flags(0x33);
            ...
            7 => {
                let mut k = 1usize;
                self.credits_sprite_draw_set_shadow_prop(k, 2);
                self.sprite_slot_mut(k).set_sprite_type(0xe9);
                self.oam_allocate_from_region_a(0x0c);
                self.sprite_slot_mut(k).set_oam_flags(0x37);
                self.sprite_get_16_bit_coords_ending(k);
                if self.frame_state().frame_counter & 15 == 0 {
                    let graphics = self.sprite_slot(k).graphics() ^ 1;
                    self.sprite_slot_mut(k).set_graphics(graphics);
                ...
                if r16 >= 0x180 {
                    ...
                    if self.sprite_slot(k).y_low() != 0x7c {
                        self.sprite_move_xy(k);
                    }
                }
                k -= 1;
                self.sprite_slot_mut(k).set_sprite_type(0x36);
                self.oam_allocate_from_region_a(0x18);
                self.sprite_slot_mut(k).set_oam_flags(0x39);
                self.sprite_get_16_bit_coords_ending(k);
                if self.sprite_slot(k).delay_main() == 0 {
            ...
            8 => {
                let k = 0usize;
                self.sprite_slot_mut(k).set_sprite_type(0x2c);
                self.oam_allocate_from_region_a(0x2c);
                self.sprite_slot_mut(k).set_oam_flags(0x3b);
                self.sprite_get_16_bit_coords_ending(k);
            ...
            9 => {
                ...
                while k < 5 {
                    if self.sprite_slot(k).delay_main() == 0 {
                        ...
                        self.sprite_slot_mut(k).set_state(96);
                        self.sprite_slot_mut(k).set_x_velocity(0);
                        self.sprite_slot_mut(k).set_x_low(238);
                        self.sprite_slot_mut(k).set_x_high(4);
                        self.sprite_slot_mut(k).set_y_low(24);
                        self.sprite_slot_mut(k).set_y_high(11);
                    }
                    if self.sprite_slot(k).state() != 0 {
                        ...
                        if self.frame_state().frame_counter & 1 == 0 {
                            ...
                            self.sprite_slot_mut(k).add_x_velocity(delta as u8);
                        }
                ...
                loop {
                    if self.sprite_slot(k).delay_main() == 0 {
                        ...
                        self.sprite_slot_mut(k).set_delay_main(delay);
                        let next_a = self.sprite_slot(k).a().wrapping_add(1) & 3;
                        self.sprite_slot_mut(k).set_a(next_a);
                        let graphics = self.sprite_slot(k).graphics() ^ 1;
                        self.sprite_slot_mut(k).set_graphics(graphics);
                    }
                    if k == 5 {
                ...
                while k != 11 {
                    ...
                    self.sprite_slot_mut(k).set_oam_flags(OAM_FLAGS[k - 7]);
                    self.sprite_slot_mut(k).set_direction(D[k - 7]);
            ...
            10 => {
                ...
                if self.sprite_slot(k).pause() == 0 {
                    let xb = WISH_POND_X[(self.get_random_number() & 7) as usize]
                        .wrapping_add(self.sprite_workspace().current_sprite_x() as u8);
                    let yb = WISH_POND_Y[(self.get_random_number() & 7) as usize]
                        .wrapping_add(self.sprite_workspace().current_sprite_y() as u8);
                    self.credits_sprite_draw_add_sparkle(3, xb, yb);
                }
                for k in 3..5 {
                    if self.sprite_slot(k).delay_aux1() != 0 {
                        let delay = self.sprite_slot(k).delay_aux1().wrapping_sub(1);
                        self.sprite_slot_mut(k).set_delay_aux1(delay);
                    }
                    self.sprite_slot_mut(k).set_sprite_type(0xe3);
                    self.credits_sprite_draw_set_shadow_prop(k, 1);
                    self.credits_sprite_draw_activate_and_run_sprite(k, 8);
                }
                self.sprite_slot_mut(k).set_sprite_type(0x72);
                self.sprite_slot_mut(k).set_oam_flags(0x3b);
                self.sprite_slot_mut(k).set_state(9);
                self.sprite_slot_mut(k).set_b(9);
            ...
            11 => {
                if r16 >= 0x170 {
                    ...
                    if r16 < 0x1c0 {
                        ...
                        self.sprite_slot_mut(k).set_delay_main(0x20);
                        let graphics = (self.sprite_slot(k).graphics() ^ 1) & 1;
                        self.sprite_slot_mut(k).set_graphics(graphics);
                    }
                    ...
                    for k in 0..2 {
                        self.sprite_slot_mut(k).set_sprite_type(0x1a);
                        self.sprite_slot_mut(k).set_oam_flags(0x39);
                        self.credits_sprite_draw_set_shadow_prop(k, 2);
                        let bak0 = self.frame_state().main_module;
                        self.credits_sprite_draw_activate_and_run_sprite(k, 0x0c);
                        self.set_main_module(bak0);
                        if self.sprite_slot(k).b() == 15 && self.sprite_slot(k).a() == 4 {
                            self.sprite_slot_mut(k + 2).set_delay_main(15);
                        }
                        let j = self.sprite_slot(k + 2).delay_main();
                        if j != 0 {
                            self.sprite_slot_mut(k + 2).set_oam_flags(2);
                            self.sprite_slot_mut(k + 2)
            ...
            12 => {
                ...
                if self.sprite_slot(k).graphics() == 0 {
                    let x_delta = if sign8(self.sprite_slot(k).x_low().wrapping_sub(0x80)) {
                        1
                    } else {
                        0xff
                    };
                    let y_delta = if sign8(self.sprite_slot(k).y_low().wrapping_sub(0xb0)) {
                        1
                    } else {
                        0xff
                    };
                    self.sprite_slot_mut(k).add_x_velocity(x_delta);
                    self.sprite_slot_mut(k).add_y_velocity(y_delta);
                    self.sprite_move_xy(k);
                }
                let oam_flags = (self.sprite_slot(k).x_velocity() >> 1 & 0x40) ^ 0x7e;
                self.sprite_slot_mut(k).set_oam_flags(oam_flags);
                self.sprite_slot_mut(k).set_flags2(1);
                self.sprite_slot_mut(k).set_flags3(0x30);
                ...
                loop {
                    ...
                    if k == 0 {
                        ...
                        if j < 0x0f {
                            self.sprite_slot_mut(k).set_z(Z[j]);
                        }
                        self.sprite_slot_mut(k)
            ...
            13 => {
                let k = 0usize;
                if r16 == 0x200 {
                    self.sprite_slot_mut(k).set_x_velocity((-4i8) as u8);
                }
                let graphics = self.frame_state().frame_counter >> 4 & 1;
                self.sprite_slot_mut(k).set_graphics(graphics);
                if self.sprite_slot(k).x_low() == 56 {
                    self.sprite_slot_mut(k).set_x_velocity(0);
                    let graphics = self.sprite_slot(k).graphics().wrapping_add(2);
                    self.sprite_slot_mut(k).set_graphics(graphics);
                }
            ...
            14 => {
                ...
                while k != 0 {
                    if k >= 5 {
                        self.sprite_slot_mut(k).set_sprite_type(0);
                        self.credits_sprite_draw_set_shadow_prop(k, 1);
                        let graphics =
                            (self.frame_state().frame_counter.wrapping_add(0x4a) & 8) >> 3;
                        self.sprite_slot_mut(k).set_graphics(graphics);
                        self.sprite_slot_mut(k).set_z(32);
                        self.credits_sprite_draw_circling_birds(k);
                        let oam_flags = (self.sprite_slot(k).x_velocity() >> 1 & 0x40) ^ 0x0f;
                        self.sprite_slot_mut(k).set_oam_flags(oam_flags);
                        self.credits_sprite_draw_preexisting_sprite_draw(k, 8);
                    } else {
                        self.sprite_slot_mut(k).set_sprite_type(0x0d);
                        if k == 1 {
                            self.sprite_slot_mut(k).set_head_direction(0x0d);
                        }
                        self.credits_sprite_draw_set_shadow_prop(k, 3);
                        self.sprite_slot_mut(k).set_oam_flags(0x2b);
                        let mut a = self.sprite_slot(k).delay_main();
                        if a == 0 {
                            a = 0xc0;
                            self.sprite_slot_mut(k).set_delay_main(a);
                        }
                        a >>= 1;
                        if a == 0 {
                        ...
                        {
                            let mut v = self.sprite_slot(k).y_velocity().wrapping_sub(1);
                            self.sprite_slot_mut(k).set_y_velocity(v);
                            v = v.wrapping_sub(4);
                            if k < 3 {
                                v = (0u8).wrapping_sub(v);
                            }
                            self.sprite_slot_mut(k).set_x_velocity(v);
                        }
                        self.sprite_move_xy(k);
            ...
            15 => {
                ...
                let mut k = 2usize;
                self.sprite_slot_mut(k).set_sprite_type(0x62);
                self.sprite_slot_mut(k).set_oam_flags(0x39);
                ...
                loop {
                    ...
                    let oam_flags =
                        (self.sprite_slot(k).x_velocity() >> 1 & 0x40) ^ OAM_FLAGS[j as usize];
                    self.sprite_slot_mut(k).set_oam_flags(oam_flags);
                    if self.sprite_slot(k).delay_main() == 0 {
                    ...
                    if self.sprite_slot(k).a() == 0 {
                        ...
                        if self.sprite_slot(k).b() == 8 {
                            self.sprite_slot_mut(k).set_b(0);
                        }
                        let b = self.sprite_slot(k).b() & 7;
                        self.sprite_slot_mut(k).set_delay_aux1(DELAY[b as usize]);
                        let graphics = (self.sprite_slot(k).graphics() & 1) ^ 1;
                        self.sprite_slot_mut(k).set_graphics(graphics);
                        let b = self.sprite_slot(k).b().wrapping_add(1);
                        self.sprite_slot_mut(k).set_b(b);
                    }
    ...
    pub(super) fn ending_func2(&mut self, k: usize, ain: u8) {
        ...
        self.sprite_slot_mut(k).set_oam_flags(ain);
        self.end_sequence_draw_shadow2(k);
        let mut j = self.sprite_slot(k).a();
        if self.sprite_slot(k).delay_main() == 0 {
            j = j.wrapping_add(1);
            if j == 8 {
                j = 6;
            ...
            self.sprite_slot_mut(k).set_a(j);
            self.sprite_slot_mut(k)
        ...
        if (j < 5 || (10..15).contains(&j)) && self.frame_state().frame_counter & 1 == 0 {
            let y_low = self.sprite_slot(k).y_low().wrapping_add(1);
    ...
    pub(super) fn credits_sprite_draw_single(&mut self, k: usize, a: u8, j: u8) {
        ...
        let start = a as usize * self.sprite_slot(k).graphics() as usize;
    ...
    pub(super) fn credits_sprite_draw_set_shadow_prop(&mut self, k: usize, a: u8) {
        self.sprite_slot_mut(k).set_flags2(a);
        self.sprite_slot_mut(k).set_flags3(16);
    }

    pub(super) fn credits_sprite_draw_add_sparkle(&mut self, j_count: usize, xb: u8, yb: u8) {
        const DELAY: [u8; 6] = [32, 4, 4, 4, 5, 6];
        self.sprite_slot_mut(0).set_c(j_count as u8);
        for k in 0..j_count {
            let mut j = self.sprite_slot(k).graphics();
            if self.sprite_slot(k).delay_main() == 0 {
                j = j.wrapping_add(1);
                if j >= 6 {
                    self.sprite_slot_mut(k).set_x_low(xb);
                    self.sprite_slot_mut(k).set_y_low(yb);
                    j = 0;
                }
                self.sprite_slot_mut(k).set_graphics(j);
                self.sprite_slot_mut(k).set_delay_main(DELAY[j as usize]);
            }
            if j != 0 {
    ...
    pub(super) fn credits_sprite_draw_walk_link_away_from_pedestal(&mut self, k: usize) {
        ...
        if self.sprite_slot(k).delay_main() == 0 {
            let graphics = self.sprite_slot(k).graphics().wrapping_add(1) & 7;
            self.sprite_slot_mut(k).set_graphics(graphics);
            self.sprite_slot_mut(k).set_delay_main(4);
        }
        let dma = DMA[self.sprite_slot(k).graphics() as usize];
        self.player_state_mut()
            .set_link_dma_graphics_index_word(dma);
        self.sprite_slot_mut(k).set_oam_flags(32);
    ...
    pub(super) fn credits_sprite_draw_move_squirrel(&mut self, k: usize) {
        ...
        if self.sprite_slot(k).delay_main() < 64 {
            let c = self.sprite_slot(k).c().wrapping_add(1) & 3;
            self.sprite_slot_mut(k).set_c(c);
            self.sprite_slot_mut(k).increment_a();
        } else {
            let j = self.sprite_slot(k).c() as usize;
            self.sprite_slot_mut(k)
    ...
    pub(super) fn credits_sprite_draw_circling_birds(&mut self, k: usize) {
        ...
        if self.sprite_slot(k).x_velocity() == TARGET_X[j as usize] as u8 {
        ...
        if self.frame_state().frame_counter & 1 == 0 {
            ...
            if self.sprite_slot(k).y_velocity() == TARGET_Y[j as usize] as u8 {
    ...
    pub(super) fn credits_handle_camera_scroll_control(&mut self) {
        if self.player_state().y_velocity() != 0 {
            ...
            self.ppu_scroll_copy_mut().add_bg2_v_copy2_signed(y_vel);
        ...
        if self.player_state().x_velocity() != 0 {
            ...
            self.ppu_scroll_copy_mut().add_bg2_h_copy2_signed(x_vel);
        ...
        if self.world_region().overlay_index() == 0x9c {
            ...
            self.ppu_scroll_copy_mut().copy_bg2_h_live_to_bg1_h_live();
    ...
    pub(super) fn end_sequence_32(&mut self) {
        ...
        self.system_signals_mut().set_music_control(0x22);
        self.palette_filter_mut().set_color_window_selection(0);
        self.palette_filter_mut().set_color_math_control(162);
        self.zelda_ppu_write(0x2108, 0x13);
        self.palette_filter_mut().set_fixed_color_red(0x3f);
        self.palette_filter_mut().set_fixed_color_green(0x5f);
        self.palette_filter_mut().set_fixed_color_blue(0x9f);
    ...
    pub(super) fn credits_fade_out_fixed_col(&mut self) {
        ...
        if self.frame_state().subsubmodule == 0 {
            ...
            if self.palette_filter().fixed_color_red() != 32 {
                self.palette_filter_mut().subtract_fixed_color_red(1);
            } else if self.palette_filter().fixed_color_green() != 64 {
                self.palette_filter_mut().subtract_fixed_color_green(1);
            } else if self.palette_filter().fixed_color_blue() != 128 {
                self.palette_filter_mut().subtract_fixed_color_blue(1);
            }
        }
    }
    ...
    pub(super) fn credits_fade_color_and_begin_animating(&mut self) {
        ...
        if self.frame_state().frame_counter & 3 == 0 {
            ...
            if self.ppu_scroll_copy().bg3_v_copy2() == 3288 {
                self.ending_scratch_mut().set_primary_word(0x80);
                self.increment_submodule();
            } else {
                self.ppu_scroll_copy_mut().add_bg3_v_copy2_signed(1);
                let bg3v = self.ppu_scroll_copy().bg3_v_copy2();
                if bg3v & 7 == 0 {
    ...
    pub(super) fn credits_add_next_attribution(&mut self) {
        ...
        let mut r16 = self.ending_scratch().primary_word();
        ...
        let r18 = self.ending_scratch().secondary_word() as usize;
        if r18 < 394 {
            ...
            if credits.should_write_digit_for_scroll_y(
                ...
                let mut deaths = self.save_progress().death_count_for_palace(palace);
                if deaths >= 1000 {
                    deaths = 999;
                }
    ...
    pub(super) fn credits_fade_and_disperse_triangles(&mut self) {
        ...
        if self.ending_scratch().secondary_low() == 0 {
            self.apply_palette_filter_bounce();
        ...
        if self.ending_scratch().primary_low() != 0 {
            self.credits_animate_the_triangles();
            return;
        }
        self.increment_submodule();
...
impl ZeldaState {
    pub(super) fn fade_music_and_reset_sram_mirror(&mut self) {
        ...
        self.player_state_mut().clear_link_state_block_for_ending();
        self.save_progress_mut().clear_dungeon_info();
        self.set_main_module(1);
        self.system_signals_mut().set_restart_check_flag(1);
        self.set_submodule(0);
    }
    ...
    pub(super) fn load_triforce_sprite_palette(&mut self) {
        ...
        for (i, color) in POLYHEDRAL_PALETTE.iter().enumerate() {
            self.palette_buffer_mut().set_main_color(0xd0 + i, *color);
        }
        self.system_signals_mut().increment_cgram_update_flag();
    }
    ...
    pub(super) fn intro_zelda_fadein(&mut self) {
        ...
        if self.palette_filter().countdown() == 0 {
            ...
            self.increment_submodule();
            self.intro_setup_sword_and_intro_flash();
        } else if self.palette_filter().countdown() == 13 {
            self.set_main_screen_layers(0x15);
            self.set_sub_screen_layers(0);
        }
    }
    ...
    pub(super) fn intro_sword_coming_down(&mut self) {
        ...
        if self.frame_state().subsubmodule == 0 {
            self.increment_submodule();
            self.palette_filter_mut().set_color_window_selection(2);
            self.palette_filter_mut().set_color_math_control(0x22);
            self.palette_filter_mut().set_countdown_word(31);
    ...
    pub(super) fn intro_fade_in_bg(&mut self) {
        ...
        } else if (((self.player_state().filtered_joypad_l() & 0xc0)
            | self.player_state().filtered_joypad_h())
            & 0xd0)
            != 0
        {
    ...
    pub(super) fn intro_wait_player(&mut self) {
        ...
        if self.frame_state().subsubmodule == 0 {
            ...
            self.set_submodule(0);
            self.player_state_mut().set_x(0);
        }
    }

    pub(super) fn intro_periodic_sword_and_intro_flash(&mut self) {
        if self.intro_sword().sparkle_timer() != 0 {
        ...
        if self.attract_scene().intro_palette_flash_count() != 0 {
            if self.attract_scene().intro_palette_flash_count() & 3 != 0 {
                let flash = if self.enhanced_features().has(FEATURE_DIM_ENDING_FLASHES) {
                    0x05
                } else {
                    0x1f
                };
                let flash_component = self.intro_sword().flash_rgb_channel();
                self.palette_filter_mut()
        ...
        let sword_y = self.intro_sword().ypos();
        for j in (0..10).rev() {
        ...
        match self.intro_sword().anim_phase() {
            0 => {
                if self.attract_scene().intro_palette_flash_count() == 0
                    && self.intro_sword().ypos() == 30
                {
            ...
            1 => {
                ...
                if self.intro_sword().sparkle_timer() == 0 {
                    ...
                    let timer =
                        INTRO_SWORD_SPARKLE_TIMERS[self.intro_sword().sparkle_step() as usize];
                ...
                self.set_oam_plain(
                    ...
                    0x44,
                    0x43,
                    SPARKLE_CHARS[self.intro_sword().sparkle_step() as usize],
                    0x25,
                    0,
                );
            }
            2 => {
                ...
                let k = self.intro_sword().sparkle_step() as usize;
                if k >= 7 {
                    return;
                }
                let y_base = self.intro_sword().sparkle_y_offset().min(0x4f);
                let y = y_base
                    .wrapping_add(self.intro_sword().ypos() as u8)
                    .wrapping_add(0x31);
                ...
                if self.intro_sword().sparkle_timer() == 0 {
                    ...
                    if matches!(
                        self.intro_sword().sparkle_y_offset(),
                        0x04 | 0x48 | 0x4c | 0x58
                    ) {
                        let step = self.intro_sword().sparkle_step().wrapping_add(2);
    ...
    pub(super) fn intro_init(&mut self) {
        ...
        self.system_signals_mut().increment_cgram_update_flag();
        self.increment_submodule();
        self.system_signals_mut().set_sound_effect_2(10);
        self.intro_init_continue();
    }
    ...
    pub(super) fn intro_setup_screen(&mut self) {
        ...
        self.set_sub_screen_layers(0);
        self.intro_initialize_background_settings();
        self.palette_filter_mut().set_color_window_selection(0x20);
        ...
        for i in 0..17 {
            self.palette_buffer_mut().set_main_color(144 + i, 0x7fff);
    ...
    pub(super) fn intro_initialize_memory_darken_finish(&mut self) {
        self.erase_tile_maps_normal();
        self.sprite_system_mut().set_main_tile_theme(35);
        self.sprite_system_mut().set_graphics_index(125);
        self.sprite_system_mut().set_aux_tile_theme(81);
        self.sprite_system_mut().set_misc_sprites_graphics_index(8);
        ...
        self.palette_buffer_mut().set_bg_tile_animation_countdown(2);
        self.set_overworld_screen(0);
        self.palette_buffer_mut().set_palette_main_indoors(0);
        ...
        match self.attract_scene().intro_step_index() {
            ...
            1 => {
                ...
                if self.poly_runtime().config1() == 113 {
                    self.system_signals_mut().set_music_control(1);
                }
            }
            2 => {
                self.attract_scene_mut().decrement_intro_step_timer();
            ...
            4 => {
                ...
                if self.attract_scene().intro_step_timer() == 0 {
                    ...
                    self.palette_filter_mut().set_color_window_selection(2);
                    self.palette_filter_mut().set_color_math_control(0x31);
                    self.set_subsubmodule(0);
                    self.system_signals_mut().increment_cgram_update_flag();
    ...
    pub(super) fn intro_sprite_type_b_0(&mut self, k: usize) {
        ...
        if self.attract_scene().intro_step_index() != 5 {
            ...
            if self.intro_actor(k).x_low() == X_LIMIT[k] {
                self.intro_actor_mut(k).set_x_velocity(0);
            }
            if self.intro_actor(k).y_low() == Y_LIMIT[k] {
                self.intro_actor_mut(k).set_y_velocity(0);
            }
        } else {
            self.intro_actor_mut(k).set_x_velocity(0);
    ...
    pub(super) fn initialize_scene_sprite_sparkle(&mut self, k: usize) {
        ...
        self.intro_actor_mut(k).set_x(i16::from(X[j]));
        self.intro_actor_mut(k).set_y(i16::from(Y[j]));
        self.intro_actor_mut(k).increment_init_phase();
    }
    ...
    pub(super) fn animate_scene_sprite_sparkle(&mut self, k: usize) {
        ...
        let j = (self.attract_scene().intro_frame_counter() >> 5 & 3) as usize;
        self.intro_actor_mut(k).set_x_low(X[j]);
        self.intro_actor_mut(k).set_y_low(Y[j]);
    }
