...
impl ZeldaState {
    ...
    pub(super) fn sprite_blind_laser(&mut self, k: usize) {
        ...
        let x = self
            .sprite_get_x(k)
            .wrapping_add_signed(i16::from(self.sprite_slot(k).x_velocity() as i8));
        let y = self
            .sprite_get_y(k)
            .wrapping_add_signed(i16::from(self.sprite_slot(k).y_velocity() as i8));
    ...
    pub(super) fn sprite_blind_head(&mut self, k: usize) {
        ...
        self.sprite_slot_mut(k).or_object_priority_bits(48);
        ...
        let new_sub = self.sprite_slot(k).subtype().wrapping_sub(1);
        self.sprite_slot_mut(k).set_subtype(new_sub);
        if (new_sub as i8) < 0 {
            let mut sprite = self.sprite_slot_mut(k);
        ...
        if j_ret >= 0 {
            let zsub = self.sprite_slot_mut(k).decrement_z_subpixel();
            if (zsub as i8) < 0 {
                self.sprite_slot_mut(k).set_z_subpixel(4);
                let pt = self.sprite_project_speed_towards_link(k, 32);
                let j = j_ret as usize;
                let mut fireball = self.sprite_slot_mut(j);
                fireball.set_x_velocity(pt.x);
                fireball.set_y_velocity(pt.y);
            }
        }
        let mut j = (self.sprite_slot(k).g() & 1) as usize;
        if self.sprite_slot(k).x_velocity() != BLIND_HEAD_X_VELOCITY_LIMITS[j] as u8 {
            let delta: i8 = if j != 0 { -1 } else { 1 };
            self.sprite_slot_mut(k).add_x_velocity(delta as u8);
        }
        if (self.sprite_slot(k).x_low() & !1) == BLIND_HEAD_X_POSITION_LIMITS[j] {
            self.sprite_slot_mut(k).increment_g();
        }
        j = (self.sprite_slot(k).anim_clock() & 1) as usize;
        if self.sprite_slot(k).y_velocity() != BLIND_HEAD_Y_VELOCITY_LIMITS[j] as u8 {
            let delta: i8 = if j != 0 { -1 } else { 1 };
            self.sprite_slot_mut(k).add_y_velocity(delta as u8);
        }
        if (self.sprite_slot(k).y_low() & !1) == BLIND_HEAD_Y_POSITION_LIMITS[j] {
    ...
    pub(super) fn sprite_blind_blind_blind(&mut self, k: usize) {
        ...
        let a = self.sprite_slot(k).f();
        if a != 0 {
            self.sprite_slot_mut(k).decrement_f();
        }
        ...
        self.sprite_slot_mut(k).increment_subtype2();
        let new_sub2 = self.sprite_slot(k).subtype2();
        if (new_sub2 & 1) == 0 {
            self.sprite_slot_mut(k).increment_delay_main();
        }
        ...
        match self.sprite_slot(k).c() {
            ...
            1 => {
                ...
                self.blind_check_bump_damage(k);
                self.sprite_slot_mut(k).set_graphics(9);
                if self.sprite_slot(k).delay_aux2() == 0 {
                    let mut sprite = self.sprite_slot_mut(k);
                    sprite.increment_c();
                    sprite.set_delay_main(255);
                    sprite.set_ignore_projectile(0);
                } else if self.sprite_slot(k).delay_aux2() < 64 {
            ...
            2 => {
                ...
                self.blind_check_bump_damage(k);
                self.blind_animate(k);
                let sub2 = self.sprite_slot(k).subtype2();
                ...
                let mut j = (self.sprite_slot(k).b() & 1) as usize;
                let delta: i8 = if j != 0 { -1 } else { 1 };
                self.sprite_slot_mut(k).add_y_velocity(delta as u8);
                if self.sprite_slot(k).y_velocity() == BLIND_OSCILLATION_Y_VELOCITY_TARGETS[j] as u8
                {
                    self.sprite_slot_mut(k).increment_b();
                }
                j = (self.sprite_slot(k).g() & 1) as usize;
                if self.sprite_slot(k).x_velocity() != BLIND_OSCILLATION_X_VELOCITY_TARGETS[j] as u8
                {
                    let delta: i8 = if j != 0 { -1 } else { 1 };
                    self.sprite_slot_mut(k).add_x_velocity(delta as u8);
                }
                if (self.sprite_slot(k).x_low() & !1) == BLIND_OSCILLATION_X_POSITION_TARGETS[j] {
                    self.sprite_slot_mut(k).increment_g();
                }
                self.sprite_move_xy(k);
                if self.sprite_slot(k).wall_collision() != 0 {
                    let wc = self.sprite_slot(k).wall_collision();
                    self.blind_fireball_flurry(k, wc);
                } else if (self.sprite_slot(k).subtype2() & 7) == 0 {
                    let hd = self.sprite_slot(k).head_direction() << 2;
            ...
            3 => {
                ...
                if self.sprite_slot(k).delay_aux2() != 0 {
                    ...
                    let j = (self.sprite_slot(k).direction().wrapping_sub(2)) as usize;
                    if self.sprite_slot(k).y_velocity()
                        != BLIND_SWITCH_WALL_Y_VELOCITY_TARGETS[j] as u8
                    {
                        let delta: i8 = if j != 0 { -2 } else { 2 };
                        self.sprite_slot_mut(k).add_y_velocity(delta as u8);
                    }
                    if (self.sprite_slot(k).y_low() & !3) == BLIND_SWITCH_WALL_Y_POSITION_TARGETS[j]
                    {
                        let b = self.sprite_slot(k).direction().wrapping_sub(1);
                        let mut sprite = self.sprite_slot_mut(k);
                        sprite.increment_c();
                        sprite.set_b(b);
                    }
            ...
            4 => {
                ...
                if (self.sprite_slot(k).subtype2() & 7) == 0 {
                    ...
                    let j = (self.sprite_slot(k).direction().wrapping_sub(2)) as usize;
                    if self.sprite_slot(k).graphics() == BLIND_WHIRL_AROUND_GRAPHICS_TARGETS[j] {
                        let g = self.sprite_slot(k).x_low() >> 7;
                        let direction = self.sprite_slot(k).direction() ^ 1;
                        let mut sprite = self.sprite_slot_mut(k);
                        ...
                        let delta: i8 = if j != 0 { 1 } else { -1 };
                        let graphics = self.sprite_slot(k).graphics().wrapping_add(delta as u8);
            ...
            6 => {
                ...
                let aux2 = self.sprite_slot(k).delay_aux2();
                if aux2 == 0 {
                    let mut sprite = self.sprite_slot_mut(k);
                    sprite.increment_c();
                    ...
                    self.sprite_slot_mut(k).set_graphics(14);
                }
            }
            7 => {
                // rerobe
                if self.sprite_slot(k).delay_aux2() == 0 {
                    let direction = (self.sprite_slot(k).y_low() >> 7).wrapping_add(2);
                    let g = (self.sprite_slot(k).x_low() << 2) | (self.sprite_slot(k).x_low() >> 7);
                    let mut sprite = self.sprite_slot_mut(k);
                    sprite.set_c(2);
                    ...
                    let idx = (self.sprite_slot(k).delay_aux2() >> 3) as usize;
                    self.sprite_slot_mut(k)
    ...
    pub(super) fn blind_fireball_flurry(&mut self, k: usize, a: u8) {
        ...
        if (sprite.subtype2() & 31) == 0 && sprite.subtype() != 5 {
            self.sprite_slot_mut(k).increment_subtype();
        }
    ...
    pub(super) fn blind_spit_fireball(&mut self, k: usize, a: u8) -> i32 {
        ...
        if (self.sprite_slot(k).subtype2() & a) != 0 {
            return -1;
        }
    ...
    pub(super) fn blind_decelerate_x(&mut self, k: usize) {
        if self.sprite_slot(k).x_velocity() != 0 {
            let delta: i8 = if (self.sprite_slot(k).x_velocity() as i8) < 0 {
                2
            } else {
                -2
            };
    ...
    pub(super) fn blind_decelerate_y(&mut self, k: usize) {
        if self.sprite_slot(k).y_velocity() != 0 {
            let delta: i8 = if (self.sprite_slot(k).y_velocity() as i8) < 0 {
                4
            } else {
                -4
            };
    ...
    pub(super) fn blind_spawn_laser(&mut self, k: usize) {
        ...
        if let Some((j, r0_x, r2_y)) = self.sprite_spawn_dynamically_for_blind(k, 0xce) {
            ...
            let mut laser = self.sprite_slot_mut(j);
    ...
    pub(super) fn blind_draw(&mut self, k: usize) {
        if self.sprite_slot(k).graphics() >= 15 {
            const BLIND_POOF_DRAW_FRAME_STARTS: [u8; 8] = [0, 1, 5, 13, 23, 30, 35, 37];
            let j = (self.sprite_slot(k).graphics() - 15) as usize;
        ...
        if self.sprite_slot(k).graphics() >= 10 {
            return;
        }
        const BLIND_HEAD_OAM_OFFSETS_BY_GRAPHICS: [u8; 10] = [4, 4, 4, 5, 5, 0, 0, 0, 0, 0];
        let oam_off = BLIND_HEAD_OAM_OFFSETS_BY_GRAPHICS[self.sprite_slot(k).graphics() as usize];
...
mod tests {
    ...
    fn prep_blind_kills_sprite_when_locked() {
        ...
        assert_eq!(s.sprite_slot(5).state(), 0);
    }

    #[test]
    fn blind_spit_fireball_returns_minus_one_when_subtype2_masks() {
        let mut s = fresh_state();
        s.sprite_slot_mut(2).set_subtype2(0xff);
    ...
    fn blind_spit_fireball_writes_velocity_table() {
        ...
        {
            let mut sprite = s.sprite_slot_mut(0);
    ...
    fn blind_decelerate_x_brings_velocity_toward_zero() {
        ...
        assert_eq!(s.sprite_slot(4).x_velocity() as i8, -3);

        // Positive velocity -> subtract 2.
        s.sprite_slot_mut(4).set_x_velocity(7);
        s.blind_decelerate_x(4);
        assert_eq!(s.sprite_slot(4).x_velocity(), 5);

        // Zero velocity stays zero.
        s.sprite_slot_mut(4).set_x_velocity(0);
        s.blind_decelerate_x(4);
        assert_eq!(s.sprite_slot(4).x_velocity(), 0);
    }
    ...
    fn blind_animate_picks_head_dir_from_table() {
        let mut s = fresh_state();
        {
            let mut sprite = s.sprite_slot_mut(1);
            sprite.set_wall_collision(0);
