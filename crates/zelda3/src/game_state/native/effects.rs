use super::sprites::AncillaSlotsState;
...
adopting_bridge!(NativeEffectAngleScratchBridgeMut, state: EffectAngleScratchState);
...
// no longer bulk-projected (see EffectsState::write_to_ram). Re-read from RAM before
// mutating so a setter composes with whichever effect wrote the window last.
adopting_bridge!(NativeQuakeSpellBridgeMut, state: QuakeSpellState);
...
// no longer bulk-projected (see EffectsState::write_to_ram). Re-read from RAM before
// mutating so a setter composes with whichever effect wrote the window last.
adopting_bridge!(NativeBombosSpellBridgeMut, state: BombosSpellState);
...
// no longer bulk-projected (see EffectsState::write_to_ram). Re-read from RAM before
// mutating so a setter composes with whichever effect wrote the window last.
adopting_bridge!(NativeTowerSealBridgeMut, state: TowerSealState);
...
adopting_bridge!(NativeSkullWoodsFireBridgeMut, state: EntranceEffectState);
...
adopting_bridge!(NativeBlastWallBridgeMut, state: EntranceEffectState);
...
adopting_bridge!(NativeDiggingGamePrizeBridgeMut, state: DiggingGamePrizeState);
...
pub(crate) struct NativeDoorDebrisBridgeMut<'a> {
    ancilla: &'a mut AncillaSlotsState,
    ram: &'a mut [u8],
}

impl<'a> NativeDoorDebrisBridgeMut<'a> {
    pub(crate) fn new(ancilla: &'a mut AncillaSlotsState, ram: &'a mut [u8]) -> Self {
        Self { ancilla, ram }
    }

    /// One byte of the door-debris arrays: written to RAM and to the owning
    /// ancilla bank cell (`$03B6..$03C7` are all bank cells).
    fn write(&mut self, address: usize, value: u8) {
        self.ram[address] = value;
        self.ancilla.set_shared_byte(address, value);
    }

    pub(crate) fn set_direction(&mut self, slot: usize, value: u8) {
        if slot < DOOR_DEBRIS_BANK_LEN {
            self.write(DOOR_DEBRIS_DIRECTION + slot, value);
        }
    }

    pub(crate) fn set_y_low_and_x_low_from_word(&mut self, slot: usize, value: u16) {
        if slot < DOOR_DEBRIS_BANK_LEN {
            self.write(DOOR_DEBRIS_Y + slot, value as u8);
            self.write(DOOR_DEBRIS_X + slot, (value >> 8) as u8);
        }
    }

    pub(crate) fn set_x_word(&mut self, slot: usize, value: u16) {
        if slot * 2 + 1 < DOOR_DEBRIS_BANK_LEN {
            self.write(DOOR_DEBRIS_X + slot * 2, value as u8);
            self.write(DOOR_DEBRIS_X + slot * 2 + 1, (value >> 8) as u8);
        }
    }

    pub(crate) fn set_y_word(&mut self, slot: usize, value: u16) {
        if slot * 2 + 1 < DOOR_DEBRIS_BANK_LEN {
            self.write(DOOR_DEBRIS_Y + slot * 2, value as u8);
            self.write(DOOR_DEBRIS_Y + slot * 2 + 1, (value >> 8) as u8);
        }
    }
}
