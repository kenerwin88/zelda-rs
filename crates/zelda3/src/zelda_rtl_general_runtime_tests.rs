...
fn dungeon_supertile_transition_resumes_at_rom_call_boundaries() {
    ...
    let stages = [
        ...
        (
            ...
            DUNGEON_SPIRAL_SPRITE_GRAPHICS_NMI_SLICES,
        ),
    ];
...
#[test]
fn room_42_staircase_34_spiral_initializer_uses_measured_short_workload() {
    assert_eq!(spiral_room_initialization_nmi_slices(0x42, 0x34), 18);
    assert!(spiral_room_initializer_returns_before_next_nmi(0x42, 0x34));
    assert_eq!(
        spiral_room_initialization_nmi_slices(0x42, 0x30),
        DUNGEON_SPIRAL_ROOM_INITIALIZATION_NMI_SLICES,
    );
    assert!(!spiral_room_initializer_returns_before_next_nmi(0x42, 0x30));
    assert_eq!(
        spiral_room_initialization_nmi_slices(0x41, 0x34),
        DUNGEON_SPIRAL_ROOM_INITIALIZATION_NMI_SLICES,
    );
    assert!(!spiral_room_initializer_returns_before_next_nmi(0x41, 0x34));
}

#[test]
fn room_21_late_aux_graphics_entry_crosses_an_eighth_nmi() {
    assert_eq!(dungeon_aux_sprite_graphics_nmi_slices(0x21), 8);
    assert_eq!(
        dungeon_aux_sprite_graphics_nmi_slices(0x22),
        DUNGEON_SUPERTILE_AUX_SPRITE_GFX_NMI_SLICES,
    );
    ...
    let mut work = ScheduledGameWork::schedule(continuation, 8);
    for _ in 0..7 {
        assert_eq!(work.advance_one_nmi_slice(), GameWorkStep::Waiting);
    }
    assert_eq!(
...
#[test]
fn room_61_room_load_caller_waits_for_its_leading_nmi_before_fresh_main() {
    ...
    let quadrant_upload = FrameState {
        subsubmodule: 4,
        ..room_load_return
    };
...
#[test]
fn four_rat_room_load_caller_returns_without_an_extra_nmi_slice() {
    let mut rat_pack = ZeldaState::new();
    rat_pack.restore_live_rom_timing_after_checkpoint();
    rat_pack.set_main_module(7);
    rat_pack.set_submodule(2);
    rat_pack.set_subsubmodule(2);
    for slot in 0..4 {
        rat_pack.sprite_slot_view_mut(slot).set_state(8);
        rat_pack.sprite_slot_view_mut(slot).set_sprite_type(0x6d);
    }

    assert!(room_load_caller_returns_before_next_nmi(&rat_pack));
    assert!(!rat_pack.begin_dungeon_supertile_transition_work(
        DungeonSupertileTransitionWork::RoomLoadCallerResume,
    ));
    assert!(rat_pack.game_execution_scheduler.is_idle());

    let mut room_21 = ZeldaState::new();
    room_21.restore_live_rom_timing_after_checkpoint();
    room_21.set_main_module(7);
    room_21.set_submodule(2);
    room_21.set_subsubmodule(2);
    room_21.set_dungeon_room_index(0x21);
    for slot in 0..11 {
        room_21.sprite_slot_view_mut(slot).set_state(8);
        room_21.sprite_slot_view_mut(slot).set_sprite_type(0x6f);
    }
    assert!(room_load_caller_returns_before_next_nmi(&room_21));
    assert!(!room_21.begin_dungeon_supertile_transition_work(
        DungeonSupertileTransitionWork::RoomLoadCallerResume,
    ));
    assert!(room_21.game_execution_scheduler.is_idle());

    let mut guard_pack = ZeldaState::new();
    guard_pack.restore_live_rom_timing_after_checkpoint();
    guard_pack.set_main_module(7);
    guard_pack.set_submodule(2);
    guard_pack.set_subsubmodule(2);
    for slot in 0..3 {
        guard_pack.sprite_slot_view_mut(slot).set_state(8);
        guard_pack.sprite_slot_view_mut(slot).set_sprite_type(0x41);
    }
    assert!(!room_load_caller_returns_before_next_nmi(&guard_pack));
    assert!(guard_pack.begin_dungeon_supertile_transition_work(
        DungeonSupertileTransitionWork::RoomLoadCallerResume,
    ));
}

#[test]
fn room_22_sprite_pack_does_not_skip_partial_sprite_main_return() {
    let mut measured = ZeldaState::new();
    measured.restore_live_rom_timing_after_checkpoint();
    measured.set_main_module(7);
    measured.set_submodule(2);
    measured.set_subsubmodule(2);
    measured.set_dungeon_room_index(0x22);
    measured.sprite_slot_view_mut(5).set_state(8);
    measured.sprite_slot_view_mut(5).set_sprite_type(0x6e);
    measured.sprite_slot_view_mut(6).set_state(8);
    measured.sprite_slot_view_mut(6).set_sprite_type(0x41);

    assert!(!room_load_caller_returns_before_next_nmi(&measured));

    measured.set_dungeon_room_index(0x23);
    assert!(!room_load_caller_returns_before_next_nmi(&measured));
}
...
#[test]
fn room_22_room_load_sprite_main_suspends_after_slot_5() {
    let mut state = ZeldaState::new();
    state.restore_live_rom_timing_after_checkpoint();
    state.set_main_module(7);
    state.set_submodule(2);
    ...
    state.dungeon_room_load_sprite_main_nmi_after_slot = Some(5);
...
#[test]
fn room_41_state_13_suspends_common_module_suffix_in_measured_early_phases() {
    let mut state = ZeldaState::new();
    state.restore_live_rom_timing_after_checkpoint();
    state.set_main_module(7);
    state.set_submodule(2);
...
#[test]
fn room_41_state_13_caller_return_accepts_post_call_state_14() {
    let mut state = ZeldaState::new();
    state.restore_live_rom_timing_after_checkpoint();
    state.set_main_module(7);
    state.set_submodule(2);
...
#[test]
fn room_41_state_13_suffix_raster_classifier_covers_measured_boundary() {
    for countdown in [11, 13, 15, 16, 18, 19, 20, 21, 23, 24, 25, 26] {
        assert!(
            crate::raster_timing::room_41_state_13_suffix_crosses_vblank(7, 2, 13, 0x41, countdown,),
            "measured palette phase {countdown} should cross vblank"
        );
    }
    for countdown in [12, 14, 17, 22, 27] {
        assert!(
            !crate::raster_timing::room_41_state_13_suffix_crosses_vblank(
                7, 2, 13, 0x41, countdown,
            ),
            "unmeasured palette phase {countdown} should remain atomic"
        );
    }
    assert!(!crate::raster_timing::room_41_state_13_suffix_crosses_vblank(7, 2, 13, 0x42, 24,));
    for countdown in [12, 14, 17, 22] {
        assert!(
            crate::raster_timing::room_41_state_13_suffix_completes_before_vblank(
                7, 2, 13, 0x41, countdown,
            ),
            "measured palette phase {countdown} should finish atomically"
        );
    }
}

#[test]
fn room_41_state_13_finishes_common_module_suffix_atomically_after_phase_23() {
    let mut state = ZeldaState::new();
    state.restore_live_rom_timing_after_checkpoint();
    state.set_main_module(7);
    state.set_submodule(2);
...
fn room_72_state_10_suspended_quadrant_build_prepares_animated_bg_on_entry() {
    ...
    assert!(!scheduled_work_entry_prepares_animated_bg_before_suspension(7, 2, 5, 0x72, work,));
    assert!(!scheduled_work_entry_prepares_animated_bg_before_suspension(7, 2, 10, 0x71, work,));
}
...
#[test]
fn room_specific_quadrant_timing_does_not_leak_into_straight_interroom_transition() {
    let mut state = ZeldaState::new();
    state.restore_live_rom_timing_after_checkpoint();
    state.set_main_module(7);
    state.set_submodule(0x12);
...
#[test]
fn room_70_lower_level_staircase_34_sprite_graphics_complete_in_one_nmi_slice() {
    let mut state = ZeldaState::new();
    state.restore_live_rom_timing_after_checkpoint();
    state.set_main_module(7);
    state.set_submodule(0x0e);
    state.set_dungeon_room_index(0x70);
    state.dungeon_stair_movement_mut().set_staircase_index(0x34);
    state
        .dungeon_stair_movement_mut()
        .set_staircase_lower_level_status(2);
    ...
    assert_eq!(
        ...
        Some(GameWorkStep::Complete(
            GameWorkContinuation::FinishDungeonSupertileTransition {
                work: DungeonSupertileTransitionWork::SpiralSpriteGraphics,
            },
        )),
    );
}
...
#[test]
fn room_70_staircase_30_sprite_graphics_complete_in_one_nmi_slice() {
    let mut state = ZeldaState::new();
    state.restore_live_rom_timing_after_checkpoint();
    state.set_main_module(7);
    state.set_submodule(0x0e);
...
#[test]
fn room_80_staircase_35_sprite_graphics_cross_five_nmi_slices() {
    let mut state = ZeldaState::new();
    state.restore_live_rom_timing_after_checkpoint();
    state.set_main_module(7);
    state.set_submodule(0x0e);
    state.set_dungeon_room_index(0x80);
    state.dungeon_stair_movement_mut().set_staircase_index(0x35);
...
#[test]
fn room_71_staircase_30_sprite_graphics_cross_two_nmi_slices() {
    let mut state = ZeldaState::new();
    state.restore_live_rom_timing_after_checkpoint();
    state.set_main_module(7);
    state.set_submodule(0x0e);
    state.set_dungeon_room_index(0x71);
    state.dungeon_stair_movement_mut().set_staircase_index(0x30);
    state
        .dungeon_stair_movement_mut()
        .set_staircase_lower_level_status(0);
...
#[test]
fn room_1_staircase_30_spiral_sprite_graphics_cross_three_nmi_slices() {
    let mut state = ZeldaState::new();
    state.restore_live_rom_timing_after_checkpoint();
    state.set_main_module(7);
    state.set_submodule(0x0e);
...
#[test]
fn room_71_and_72_filtering_returns_hold_the_first_scroll_iteration() {
    for room in [0x71, 0x72] {
        let mut state = ZeldaState::new();
        state.restore_live_rom_timing_after_checkpoint();
        state.set_main_module(7);
        state.set_submodule(2);
        ...
        assert!(state.begin_dungeon_supertile_filtering_return());
        assert_eq!(
            state.game_execution_scheduler.advance_work_one_nmi_slice(),
            Some(GameWorkStep::Complete(
        ...
        assert_eq!(
            ...
            if room == 0x72 { 7 } else { 8 }
        );
    }

    let mut state = ZeldaState::new();
    let retained = DisplayObjGeneration::RetainCapturedOam {
        oam: vec![0x1234, 0x5678],
    };
    assert_eq!(retained.retained_oam(), Some([0x1234, 0x5678].as_slice()));
    assert_eq!(retained.retained_vram(), None);
    state.next_display_obj_memory_generation = Some(retained.clone());
    state.capture_display_snapshot();
    assert_eq!(
        state.display_snapshot.as_ref().unwrap().obj_generation,
        retained
    );
    assert_eq!(state.next_display_obj_memory_generation, None);

    let live_oam = vec![0x1111; 272];
    let published_oam = vec![0x2222; 272];
    let composed = dungeon_supertile_pre_scroll_oam(&live_oam, Some(&published_oam));
    assert_eq!(&composed[204..206], &[0x2122, 0x2222]);
    ...
    assert_eq!(&composed[202..204], &[0x1111, 0x1111]);
    assert_eq!(&composed[206..208], &[0x2222, 0x2222]);
    assert_eq!(&composed[214..216], &[0x2222, 0x2222]);
    assert_eq!(&composed[224..226], &[0x1111, 0x1111]);

    let composed = dungeon_supertile_first_state8_oam(&live_oam);
    assert_eq!(composed[92 * 2], 0x1011);
    assert_eq!(composed[93 * 2], 0x1011);
    assert_eq!(composed[94 * 2], 0x1111);

    let composed = dungeon_supertile_second_state8_oam(&composed);
    assert_eq!(composed[92 * 2], 0x0e11);
    assert_eq!(composed[93 * 2], 0x0e11);
    assert_eq!(composed[94 * 2], 0x1111);

    let mut composed = live_oam.clone();
    adjust_room_72_published_sprite_oam(&mut composed, 3);
    assert_eq!(composed[92 * 2], 0x0e11);
    assert_eq!(composed[93 * 2], 0x0e11);
    assert_eq!(composed[94 * 2], 0x1111);

    let mut composed = live_oam.clone();
    adjust_room_72_state13_link_oam(&mut composed, 3);
    assert_eq!(composed[102 * 2], 0x1411);
    assert_eq!(composed[107 * 2], 0x1411);
    assert_eq!(composed[92 * 2], 0x1111);
}
...
fn room_72_held_filter_return_retains_the_pre_nmi_vram_scanout() {
    ...
    state
        .game_execution_scheduler
        .schedule_work(GameWorkContinuation::HoldDungeonSupertileFilteringReturn, 1);
...
fn resumed_dungeon_fade_waits_for_its_leading_nmi() {
    ...
    state.set_submodule(2);
    state.set_subsubmodule(2);
    // The first walk advances to 1 and is interrupted; the resumed second walk
    // advances to 2 and returns at V=18, still well before fade completion.
    state.set_countdown_word(0);
    ...
    assert_eq!(
        ...
        Some(PreMainNmiResume::DungeonFadedFilterCompletionNmi)
    );
}
...
fn completed_dungeon_fade_waits_for_leading_nmi_before_state_three() {
    ...
    assert!(!state
        ...
        .work_suspends_translated_call_stack());
    assert_eq!(
        PreMainNmiResume::DungeonFadedFilterCompletionNmi.nmi_latch_clear_phase(),
        Some(NmiPhase::AfterNmi)
    );
    assert_eq!(
        state.next_display_obj_scanout_generation,
...
#[test]
fn measured_landing_fades_suspend_their_caller_return_before_state_fifteen_main() {
    for room in [0x22, 0x41] {
        let mut state = ZeldaState::new();
        state.rom_startup_timing = true;
        state.set_main_module(7);
        state.set_submodule(2);
...
#[test]
fn completed_room_60_landing_fade_does_not_use_the_room_41_caller_return_exception() {
    let mut state = ZeldaState::new();
    state.rom_startup_timing = true;
    state.set_main_module(7);
    state.set_submodule(2);
...
#[test]
fn penultimate_landing_palette_zero_does_not_schedule_the_final_caller_return() {
    let mut state = ZeldaState::new();
    state.restore_live_rom_timing_after_checkpoint();
    state.set_main_module(7);
    state.set_submodule(2);
...
fn only_interrupted_dungeon_fade_selects_the_host_boundary_oam_shadow() {
    let dungeon_fade = Some(PreMainCallerContinuation::DungeonFadedFilterSecondPalettePass);
...
fn dungeon_transition_scroll_steps_keep_independent_dma_operand_and_scanout_generations() {
    ...
    subtile.submodule = 1;
    for subsubmodule in 1..=2 {
        subtile.subsubmodule = subsubmodule;
        let subtile_plan = rom_graphics_dma_plan_at_host_boundary(subtile);
        assert_eq!(
            subtile_plan.oam_scanout,
    ...
    assert_eq!(
        subtile_plan.link_obj_operands,
        GraphicsDmaGeneration::HostBoundaryBeforeMain
    );
    assert_eq!(
    ...
    assert!(rom_dungeon_subtile_return_publishes_live_animated_bg(
        subtile, room_load
    ));
    assert_eq!(
    ...
    let mut supertile_entry = crate::game_state::FrameState::default();
    supertile_entry.main_module = 7;
    supertile_entry.submodule = 2;
    let gameplay_entry = crate::game_state::FrameState {
        main_module: 7,
        ..Default::default()
    };
    assert_eq!(
    ...
    let supertile_steady_plan = rom_graphics_dma_plan_at_host_boundary(supertile_exit);
    assert_eq!(
        supertile_steady_plan.oam_scanout,
        OamScanoutSource::RetainResidentPpuOam
    );
    assert_eq!(
        supertile_steady_plan.link_obj_scanout,
        GraphicsDmaGeneration::HostBoundaryBeforeMain
    );

    let mut supertile_finished = supertile_exit;
    supertile_finished.subsubmodule = 2;
    assert_eq!(
        link_obj_scanout_across_main(
            supertile_exit,
            supertile_finished,
    ...
    let mut supertile_scrolling = supertile_finished;
    supertile_scrolling.subsubmodule = 8;
    ...
    let mut supertile_sprite_conversion_return = supertile_sprite_conversion;
    supertile_sprite_conversion_return.subsubmodule = 4;
    assert!(
    ...
    let mut supertile_quadrant_build = supertile_sprite_conversion_return;
    supertile_quadrant_build.subsubmodule = 5;
    let mut supertile_filter_entry = supertile_quadrant_build;
    supertile_filter_entry.subsubmodule = 6;
    assert!(
        room_82_horizontal_quadrant_filter_entry_publishes_host_boundary_oam(
            supertile_quadrant_build,
            supertile_filter_entry,
            0x82,
            2,
            OamScanoutSource::ComposePublishedShadowDma,
        )
    ...
    assert_eq!(
        rom_graphics_dma_plan_at_host_boundary(supertile_sprite_conversion).oam_scanout,
        OamScanoutSource::RetainResidentPpuOam
    );
    assert_eq!(
        link_obj_scanout_across_main(
    ...
    let mut doorway_movement = supertile_finished;
    doorway_movement.subsubmodule = 13;
    assert!(dungeon_transition_retains_presented_link_vram(
        supertile_finished,
        doorway_movement,
        0x71,
    ));
    ...
    doorway_movement.subsubmodule = 14;
    assert!(dungeon_transition_retains_presented_link_vram(
        supertile_finished,
        doorway_movement,
        0x71,
    ));
    let mut filtering_done = doorway_movement;
    filtering_done.subsubmodule = 15;
    let mut room_load = filtering_done;
    room_load.submodule = 5;
    room_load.subsubmodule = 0;
    assert!(dungeon_transition_retains_presented_link_vram(
        filtering_done,
    ...
    assert!(!dungeon_transition_retains_presented_link_vram(
        room_load, room_load, 0x71,
    ));
    let normal_gameplay = FrameState {
        main_module: 7,
        submodule: 0,
        subsubmodule: 0,
        ..FrameState::default()
    };
    let brightness_change = FrameState {
        submodule: 0x0a,
        ..normal_gameplay
    };
    assert!(dungeon_transition_retains_presented_link_vram(
    ...
    compose_published_link_oam(&mut live_oam, Some(&published_oam));
    for entry in LINK_OAM_ENTRIES {
        assert_eq!(&live_oam[entry * 2..entry * 2 + 2], &[0x2222, 0x2222]);
    }
    assert_eq!(&live_oam[202..204], &[0x1111, 0x1111]);
    assert_eq!(&live_oam[224..226], &[0x1111, 0x1111]);
}
