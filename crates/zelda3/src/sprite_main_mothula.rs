...
impl ZeldaState {
    ...
    pub(super) fn sprite_99_pengator(&mut self, k: usize) {
        ...
        match self.sprite_slot(k).ai_state() {
            ...
            1 => {
                ...
                let value = (self.frame_state().frame_counter & 4) >> 2;
            ...
            3 => {
                if ((((k as u8) ^ self.frame_state().frame_counter) & 7) | self.sprite_slot(k).z())
                    == 0
                {
    ...
    pub(super) fn sprite_9_e_haunted_grove_ostritch(&mut self, k: usize) {
        ...
        match self.sprite_slot(k).ai_state() {
            0 => {
                let value = if (self.frame_state().frame_counter & 0x18) != 0 {
                    3
                } else {
                    0
                };
                self.sprite_slot_mut(k).set_graphics(value);
                if self.garnish_state().haunted_grove_flute_event_latch() != 0 {
                    self.sprite_slot_mut(k).set_ai_state(1);
        ...
        match self.sprite_slot(k).ai_state() {
            ...
            2 => {
                ...
                if self.sprite_slot(k).delay_aux1() != 0
                    && (((k as u8) ^ self.frame_state().frame_counter) & 7) == 0
                {
                ...
                let value = FREEZOR_MOVING_GRAPHICS
                    [usize::from(((k as u8) ^ self.frame_state().frame_counter) >> 2 & 3)];
                self.sprite_slot_mut(k).set_graphics(value);
            }
            3 => {
                if self.sprite_slot(k).delay_main() == 0 {
            ...
            2 => {
                    ...
                    && self.sprite_slot(k).delay_main() < 46
                {
                    ...
                    match loot {
                        ...
                        2 => {
                            if self.player_resources().arrows() != 0 {
    ...
    pub(super) fn stalfos_skellington(&mut self, k: usize) {
            ...
            && self.sprite_slot(k).floor() == self.player_state().lower_level_state()
        {
            ...
            if !self.player_state().is_running() {
                if self.player_state().button_b_frames() >= 0x90 {
                    should_jump = true;
                } else if !sign8(self.player_state().button_b_frames().wrapping_sub(9)) {
                    may_check_dir = false;
                }
            }
            let facing = self.player_state().facing_index() & 3;
    ...
    pub(super) fn sprite_83_green_eyegore(&mut self, k: usize) {
        ...
        if self.player_state().defense_flags() != 0
            || (self.player_state().joypad1h_last() & 0x0f) == 0
        {
            self.sprite_slot_mut(k).set_a(0);
            self.sprite_check_damage_to_and_from_link(k);
            self.sprite_check_tile_collision(k);
    ...
    pub(super) fn sprite_a8_green_zirro(&mut self, k: usize) {
        ...
        if pt.x.wrapping_add(40) < 80
            ...
            && (self.player_state().is_running()
                || sign8(self.player_state().button_b_frames().wrapping_sub(9)))
        {
    ...
    pub(super) fn sprite_c3_gibo(&mut self, k: usize) {
        ...
        let value = (self.frame_state().frame_counter >> 3) & 3;
        ...
        match self.sprite_slot(k).ai_state() {
            0 => {
                ...
                if pt.x.wrapping_add(40) < 80
                    ...
                    && j != TEKITE_DIRECTIONS[self.player_state().facing_index()]
                {
                    let pt = self.sprite_project_speed_towards_link(k, 32);
                    self.sprite_slot_mut(k).set_x_velocity(pt.x.wrapping_neg());
                    self.sprite_slot_mut(k).set_y_velocity(pt.y.wrapping_neg());
    ...
    pub(super) fn sprite_d2_flopping_fish(&mut self, k: usize) {
        ...
        if self.sprite_slot(k).state() == 10 {
            self.sprite_slot_mut(k).set_ai_state(4);
            let value = ((self.frame_state().frame_counter >> 4) & 1) + 3;
            self.sprite_slot_mut(k).set_graphics(value);
        }
        if self.sprite_return_if_inactive(k) {
            return;
        ...
        match self.sprite_slot(k).ai_state() {
            ...
            1 => {
                ...
                if sign8(self.sprite_slot(k).z()) {
                    self.sprite_slot_mut(k).set_z(0);
                    if self.sprite_workspace().tile_type() == 9 {
                        self.sprite_spawn_small_splash(k);
                    } else if self.sprite_workspace().tile_type() == 8 {
                        self.sprite_slot_mut(k).set_state(0);
                        self.sprite_spawn_small_splash(k);
                    }
                ...
                let value =
                    FLOPPING_FISH_AIR_GRAPHICS[a] + ((self.frame_state().frame_counter >> 3) & 1);
                self.sprite_slot_mut(k).set_graphics(value);
            }
            2 => {
                if self.sprite_slot(k).delay_main() == 0 {
    ...
    pub(super) fn sprite_c2_boulder(&mut self, k: usize) {
        ...
        let value = (self.frame_state().frame_counter << 2) & 0xc0;
        self.sprite_slot_mut(k).set_oam_flags(value);
        self.sprite_move_xyz(k);
        if (((k as u8) ^ self.frame_state().frame_counter) & 3) != 0 {
            return;
    ...
    pub(super) fn sprite_8_c_arrghus(&mut self, k: usize) {
        ...
        self.overlord_slot_mut(4).set_x_low(1);
        ...
        match self.sprite_slot(k).ai_state() {
            ...
            1 => {
                if self.sprite_slot(k).delay_main() == 0 {
                    ...
                    if !self.sprite_check_if_screen_is_clear() {
                        ...
                        if self.overlord_slot(3).x_low() == 4 {
            ...
            3 => {
                ...
                if self.sprite_slot(k).z() >= 224 {
                    ...
                    self.sprite_slot_mut(k).set_ai_state(4);
                    self.sprite_slot_mut(k).set_z_velocity(0);
                    let value = self.player_state().x() as u8;
                    self.sprite_slot_mut(k).set_x_low(value);
                    let value = self.player_state().y() as u8;
                    self.sprite_slot_mut(k).set_y_low(value);
                }
            }
            4 => {
        ...
        match self.sprite_slot(k).ai_state() {
            0 => {
                ...
                if hit_link {
                    ...
                    self.sprite_set_x(k, self.player_state().x());
                    self.sprite_set_y(k, self.player_state().y().wrapping_add(8));
            ...
            2 => {
                if self.sprite_slot(k).delay_main() == 0 {
                    ...
                    let value = FALLING_XVEL
                        [usize::from((self.frame_state().frame_counter >> 1) & 1)]
                        as u8;
                    self.sprite_slot_mut(k).set_x_velocity(value);
                    self.sprite_move_x(k);
                }
    ...
    pub(super) fn sprite_8_e_terrorpin(&mut self, k: usize) {
        ...
        match self.sprite_slot(k).b() {
            0 => {
                ...
                let value = (self.frame_state().frame_counter
    ...
    pub(super) fn terrorpin_check_for_hammer(&mut self, k: usize) {
        if (self.sprite_slot(k).z() | self.sprite_slot(k).delay_aux2()) == 0
            && self.sprite_slot(k).floor() == self.player_state().lower_level_state()
            && !self.player_state().has_disabled_oam_offsets()
            && self.player_state().item_in_hand_has(0x0a)
        {
    ...
    pub(super) fn sprite_89_mothula_beam(&mut self, k: usize) {
        ...
        for i in (0..=14usize).rev() {
            if self.garnish_slot(i).is_empty() {
                let value = 2;
                self.garnish_slot_mut(i).set_garnish_type(value);
                self.garnish_state_mut().set_active_type(2);
                let value = self.sprite_slot(k).x_low();
                self.garnish_slot_mut(i).set_x_low(value);
                let value = self.sprite_slot(k).x_high();
                self.garnish_slot_mut(i).set_x_high(value);
                let value = self.sprite_slot(k).y_low();
                self.garnish_slot_mut(i).set_y_low(value);
                let value = self.sprite_slot(k).y_high();
                self.garnish_slot_mut(i).set_y_high(value);
                let value = 16;
                self.garnish_slot_mut(i).set_countdown(value);
                let value = k as u8;
                self.garnish_slot_mut(i).set_sprite(value);
                let value = self.sprite_slot(k).floor();
                self.garnish_slot_mut(i).set_floor(value);
                break;
            }
        }
    }
    ...
    pub(super) fn sprite_94_pirogusu(&mut self, k: usize) {
        ...
        match self.sprite_slot(k).ai_state() {
            ...
            2 => {
                ...
                let idx = ((self.frame_state().frame_counter >> 2) & 1) as usize | (j << 1);
                self.sprite_slot_mut(k)
    ...
    pub(super) fn sprite_88_mothula(&mut self, k: usize) {
        if self.enhanced_features().has(FEATURE_MISC_BUG_FIXES_MOTHULA) {
    ...
    pub(super) fn mothula_handle_spikes(&mut self, k: usize) {
        ...
        let value = self.sprite_workspace().room_origin_x_high().wrapping_add(1);
    ...
    pub(super) fn sprite_53_armos_knight(&mut self, k: usize) {
        ...
        if sign8(self.sprite_slot(k).z()) {
            self.sprite_slot_mut(k).set_z_velocity(0);
            self.sprite_slot_mut(k).set_z(0);
            if self.sprite_workspace().armos_knight_remaining_count() != 1
        ...
        } else if self.sprite_workspace().armos_knight_remaining_count() == 1 {
