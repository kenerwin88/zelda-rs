...
impl ZeldaState {
    ...
    pub(super) fn sprite_9_a_kyameron(&mut self, k: usize) {
        ...
        match self.sprite_slot_view(k).ai_state() {
            ...
            3 => {
                ...
                self.sprite_slot_view_mut(k).set_graphics(value);
                if (((k as u8) ^ self.frame_control_view().frame_counter()) & 7) == 0 {
    ...
    pub(super) fn sprite_99_pengator(&mut self, k: usize) {
        ...
        match self.sprite_slot_view(k).ai_state() {
            ...
            1 => {
                ...
                let value = (self.frame_control_view().frame_counter() & 4) >> 2;
    ...
    pub(super) fn sprite_9_e_haunted_grove_ostritch(&mut self, k: usize) {
        ...
        match self.sprite_slot_view(k).ai_state() {
            0 => {
                let value = if (self.frame_control_view().frame_counter() & 0x18) != 0 {
                    3
                } else {
                    0
                };
                self.sprite_slot_view_mut(k).set_graphics(value);
                if self.garnish_state_view().haunted_grove_flute_event_latch() != 0 {
                    self.sprite_slot_view_mut(k).set_ai_state(1);
        ...
        match self.sprite_slot_view(k).ai_state() {
            ...
            2 => {
                ...
                if self.sprite_slot_view(k).delay_aux1() != 0
                    && (((k as u8) ^ self.frame_control_view().frame_counter()) & 7) == 0
                {
                ...
                let value = FREEZOR_MOVING_GRAPHICS
                    [usize::from(((k as u8) ^ self.frame_control_view().frame_counter()) >> 2 & 3)];
                self.sprite_slot_view_mut(k).set_graphics(value);
            }
            3 => {
                if self.sprite_slot_view(k).delay_main() == 0 {
            ...
            2 => {
                    ...
                    && self.sprite_slot_view(k).delay_main() < 46
                {
                    ...
                    match loot {
                        ...
                        2 => {
                            if self.player_resources_view().arrows() != 0 {
                        ...
                        3 => {
                            ...
                            if rupees != 0 {
                                self.player_resources_view_mut().subtract_rupees_goal(1);
                            } else {
                                self.sprite_slot_view_mut(k).set_g(0);
                            }
                        }
    ...
    pub(super) fn stalfos_skellington(&mut self, k: usize) {
            ...
            && self.sprite_slot_view(k).floor() == self.player_state_view().lower_level_state()
        {
            ...
            if !self.player_state_view().is_running() {
                if self.player_state_view().button_b_frames() >= 0x90 {
                    should_jump = true;
                } else if !sign8(self.player_state_view().button_b_frames().wrapping_sub(9)) {
                    may_check_dir = false;
                }
            }
            let facing = self.player_state_view().facing_index() & 3;
    ...
    pub(super) fn sprite_zazak_main(&mut self, k: usize) {
        if self.sprite_slot_view(k).b() != 0 {
            ...
            let value = (self.frame_control_view().frame_counter() >> 1) & 1;
            self.sprite_slot_view_mut(k).set_graphics(value);
            self.sprite_check_damage_to_link(k);
            self.sprite_move_xy(k);
            if self.sprite_check_tile_collision(k) != 0 {
    ...
    pub(super) fn sprite_83_green_eyegore(&mut self, k: usize) {
        ...
        if self.player_state_view().defense_flags() != 0
            || (self.player_state_view().joypad1h_last() & 0x0f) == 0
        {
            self.sprite_slot_view_mut(k).set_a(0);
            self.sprite_check_damage_to_and_from_link(k);
            self.sprite_check_tile_collision(k);
        ...
        match self.sprite_slot_view(k).ai_state() {
            ...
            2 => {
                ...
                if self.sprite_slot_view(k).delay_main() == 0 {
                    ...
                    if ((k as u8) ^ self.frame_control_view().frame_counter()) & 31 == 0 {
                        let value = self.sprite_direction_to_face_link(k, None);
                        self.sprite_slot_view_mut(k).set_direction(value);
                    }
                    let j = usize::from(self.sprite_slot_view(k).direction());
    ...
    pub(super) fn sprite_a8_green_zirro(&mut self, k: usize) {
        ...
        if pt.x.wrapping_add(40) < 80
            ...
            && (self.player_state_view().is_running()
                || sign8(self.player_state_view().button_b_frames().wrapping_sub(9)))
        {
    ...
    pub(super) fn sprite_c3_gibo(&mut self, k: usize) {
        ...
        let value = (self.frame_control_view().frame_counter() >> 3) & 3;
        ...
        match self.sprite_slot_view(k).ai_state() {
            0 => {
                ...
                if pt.x.wrapping_add(40) < 80
                    ...
                    && self.player_state_view().lower_level_state()
                        == self.sprite_slot_view(k).floor()
                    && j != TEKITE_DIRECTIONS[self.player_state_view().facing_index()]
                {
                    let pt = self.sprite_project_speed_towards_link(k, 32);
                    self.sprite_slot_view_mut(k)
                        .set_x_velocity(pt.x.wrapping_neg());
    ...
    pub(super) fn sprite_d2_flopping_fish(&mut self, k: usize) {
        ...
        if self.sprite_slot_view(k).state() == 10 {
            self.sprite_slot_view_mut(k).set_ai_state(4);
            let value = ((self.frame_control_view().frame_counter() >> 4) & 1) + 3;
            self.sprite_slot_view_mut(k).set_graphics(value);
        }
        if self.sprite_return_if_inactive(k) {
            return;
        ...
        match self.sprite_slot_view(k).ai_state() {
            ...
            1 => {
                ...
                if sign8(self.sprite_slot_view(k).z()) {
                    self.sprite_slot_view_mut(k).set_z(0);
                    if self.sprite_workspace_view().tile_type() == 9 {
                        self.sprite_spawn_small_splash(k);
                    } else if self.sprite_workspace_view().tile_type() == 8 {
                        self.sprite_slot_view_mut(k).set_state(0);
                        self.sprite_spawn_small_splash(k);
                    }
            ...
            3 => {
                ...
                if self.sprite_slot_view(k).z_velocity() == 0 && self.sprite_slot_view(k).a() != 0 {
                    self.dialogue_message_index_view_mut().set_value(0x176);
    ...
    pub(super) fn sprite_c2_boulder(&mut self, k: usize) {
        ...
        let value = (self.frame_control_view().frame_counter() << 2) & 0xc0;
        self.sprite_slot_view_mut(k).set_oam_flags(value);
        self.sprite_move_xyz(k);
        if (((k as u8) ^ self.frame_control_view().frame_counter()) & 3) != 0 {
            return;
        ...
        match self.sprite_slot_view(k).ai_state() {
            ...
            3 => {
                ...
                if self.sprite_slot_view(k).z() >= 224 {
                    ...
                    self.sprite_slot_view_mut(k).set_ai_state(4);
                    self.sprite_slot_view_mut(k).set_z_velocity(0);
                    let value = self.player_state_view().x() as u8;
                    self.sprite_slot_view_mut(k).set_x_low(value);
                    let value = self.player_state_view().y() as u8;
                    self.sprite_slot_view_mut(k).set_y_low(value);
                }
            }
            4 => {
                ...
                if self.sprite_slot_view(k).delay_main() == 0 {
                    ...
                    if (self.frame_control_view().frame_counter() & 7) == 0 {
                        ...
                        if j >= 0 {
                            ...
                            self.garnish_state_view_mut().set_active_type(21);
                            ...
                            let value = self.sprite_slot_view(k).y_low().wrapping_add(24);
    ...
    pub(super) fn arrghus_draw(&mut self, k: usize) {
        ...
        for i in 0..4 {
            ...
            self.oam_state_view_mut().set_entry_char(addr, charnum);
        }
        if self.sprite_slot_view(k).ai_state() == 5 {
            self.oam_state_view_mut().hide_entry(oam + 4 * 4);
        }
        if (self.sprite_slot_view(k).subtype2() & 8) != 0 {
            self.oam_state_view_mut().or_entry_flags(oam + 4 * 4, 0x40);
        }
            ...
            2 => {
                if self.sprite_slot_view(k).delay_main() == 0 {
                    ...
                    let value = FALLING_XVEL
                        [usize::from((self.frame_control_view().frame_counter() >> 1) & 1)]
                        as u8;
                    self.sprite_slot_view_mut(k).set_x_velocity(value);
                    self.sprite_move_x(k);
                }
    ...
    pub(super) fn sprite_8_e_terrorpin(&mut self, k: usize) {
        ...
        match self.sprite_slot_view(k).b() {
            0 => {
                ...
                let value = (self.frame_control_view().frame_counter()
                    >> if self.sprite_slot_view(k).g() != 0 {
                        2
                    } else {
                        3
    ...
    pub(super) fn terrorpin_check_for_hammer(&mut self, k: usize) {
        if (self.sprite_slot_view(k).z() | self.sprite_slot_view(k).delay_aux2()) == 0
            && self.sprite_slot_view(k).floor() == self.player_state_view().lower_level_state()
            && !self.player_state_view().has_disabled_oam_offsets()
            && self.player_state_view().item_in_hand_has(0x0a)
        {
    ...
    pub(super) fn sprite_89_mothula_beam(&mut self, k: usize) {
        ...
        for i in (0..=14usize).rev() {
            if self.garnish_slot_view(i).is_empty() {
                let value = 2;
                self.garnish_slot_view_mut(i).set_garnish_type(value);
                self.garnish_state_view_mut().set_active_type(2);
                let value = self.sprite_slot_view(k).x_low();
                self.garnish_slot_view_mut(i).set_x_low(value);
                let value = self.sprite_slot_view(k).x_high();
                self.garnish_slot_view_mut(i).set_x_high(value);
                let value = self.sprite_slot_view(k).y_low();
                self.garnish_slot_view_mut(i).set_y_low(value);
                let value = self.sprite_slot_view(k).y_high();
                self.garnish_slot_view_mut(i).set_y_high(value);
                let value = 16;
                self.garnish_slot_view_mut(i).set_countdown(value);
                let value = k as u8;
                self.garnish_slot_view_mut(i).set_sprite(value);
                let value = self.sprite_slot_view(k).floor();
                self.garnish_slot_view_mut(i).set_floor(value);
                break;
            }
        }
    }
        ...
        match self.sprite_slot_view(k).ai_state() {
            ...
            2 => {
                ...
                if !self.sprite_check_damage_to_and_from_link(k) {
                    ...
                    self.sprite_workspace_view_mut().set_current_sprite_y(cy);
                    if self.sprite_check_tile_collision(k) == 0 {
                        self.sprite_94_tile_animate(k);
                        return;
                    }
    ...
    pub(super) fn sprite_94_pirogusu(&mut self, k: usize) {
        ...
        match self.sprite_slot_view(k).ai_state() {
            ...
            2 => {
                ...
                let idx =
                    ((self.frame_control_view().frame_counter() >> 2) & 1) as usize | (j << 1);
                self.sprite_slot_view_mut(k)
    ...
    pub(super) fn mothula_draw(&mut self, k: usize) {
        self.oam_state_view_mut().set_current_pointer(0x920);
        self.oam_state_view_mut()
            .set_current_extended_pointer(0xa68);
    ...
    pub(super) fn sprite_53_armos_knight(&mut self, k: usize) {
        ...
        if sign8(self.sprite_slot_view(k).z()) {
            self.sprite_slot_view_mut(k).set_z_velocity(0);
            self.sprite_slot_view_mut(k).set_z(0);
            if self.sprite_workspace_view().armos_knight_remaining_count() != 1
        ...
        } else if self.sprite_workspace_view().armos_knight_remaining_count() == 1 {
