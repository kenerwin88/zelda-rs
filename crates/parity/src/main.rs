...
use parity::{fingerprint_mask_offsets, FrameFingerprint, RECORD_LEN};
...
fn main() {
    ...
    match args.first().map(String::as_str) {
        ...
        _ => {
            eprintln!("usage: zparity <capture|check|drill|coverage> [options]");
            exit(2);
        }
    }
}
...
fn cmd_capture(args: &[String]) {
    ...
    if !p.c_root.join("zelda3").exists() {
        eprintln!(
            "C oracle binary missing: {:?} (build: make -C {:?} zelda3)",
            p.c_root.join("zelda3"),
            p.c_root
        );
        exit(1);
    }
    let tmp = p
        .cache_dir
        .join(format!("capture-{}.fp", std::process::id()));
    std::fs::create_dir_all(&p.cache_dir).unwrap();
    eprintln!("zparity capture: running C oracle over {frames} frames");
    let status = runner::c_capture_cmd(&p, frames, &tmp)
        .status()
        .expect("spawn C oracle");
    if !status.success() {
    ...
    if detail {
        ...
        eprintln!(
            "wrote {blocks} detail blocks to {:?}",
            p.cache_dir.join("detail")
        );
    }
    let _ = std::fs::remove_file(&tmp);
    eprintln!(
        "golden written: {:?} (root=0x{:08x})",
        p.golden_dir, merkle.root
    );
}
