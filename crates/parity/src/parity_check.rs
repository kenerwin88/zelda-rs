use std::path::Path;
use std::path::PathBuf;
use std::process::exit;
use std::sync::{Arc, Mutex};
use std::thread;

use parity::checkpoint_cache::CheckpointCacheManifest;
use parity::golden::{Manifest, RollupMap};
use parity::merkle::MerkleIndex;
use parity::runner::{self, Paths};
use parity::{FrameFingerprint, RECORD_LEN};

const SHARD_THRESHOLD: u32 = 20_000; // below this, run single-shard

pub fn run(args: &[String]) {
    let p = Paths::discover();
    if let Some(frame_idx) = frame_arg(args) {
        run_frame(&p, frame_idx);
        return;
    }

    let frames = super::parse_frames(args);
    let detail = args.iter().any(|a| a == "--detail");
    let shards = shard_count(args, frames);

    // 1. Validate manifest.
    let man = load_checked_manifest(&p);
    if frames > man.frames {
        eprintln!("requested {frames} frames > golden {} frames", man.frames);
        exit(1);
    }

    let golden = RollupMap::open(&p.golden_dir.join("rollup.bin")).unwrap();

    // 2. Boundaries + checkpoints (single shard => none).
    let bounds = shard_bounds(frames, shards);
    if shards > 1 {
        ensure_checkpoints(&p, &bounds);
    }

    // 3. Fan out shards.
    let start_time = std::time::Instant::now();
    let golden = Arc::new(golden);
    let first_diff = Arc::new(Mutex::new(None::<u64>)); // global frame index
    let mut handles = Vec::new();
    let tmp = p.cache_dir.join("shards");
    std::fs::create_dir_all(&tmp).unwrap();
    for (i, w) in bounds.windows(2).enumerate() {
        let (s, e) = (w[0], w[1]);
        let p = p.clone();
        let golden = Arc::clone(&golden);
        let first_diff = Arc::clone(&first_diff);
        let fp_out = tmp.join(format!("shard_{i}.bin"));
        let ck = if i == 0 {
            None
        } else {
            Some(p.cache_dir.join(format!("ck/{s}.sav")))
        };
        handles.push(thread::spawn(move || {
            run_shard(&p, s, e, &fp_out, ck.as_deref(), &golden, &first_diff);
        }));
    }
    for h in handles {
        h.join().unwrap();
    }

    // 4. Report.
    let dur = start_time.elapsed().as_secs_f64();
    let merkle = MerkleIndex::from_bytes(&std::fs::read(p.golden_dir.join("merkle.bin")).unwrap());
    let diff_result = *first_diff.lock().unwrap();
    match diff_result {
        None => {
            println!(
                "MATCH  {frames} frames, {shards} shards, {dur:.1}s  root=0x{:08x}",
                merkle.root
            );
            if detail {
                println!(
                    "(detail compare requested; per-region OK — see `zparity drill` on failure)"
                );
            }
        }
        Some(f) => {
            println!("DIVERGE at frame {f}  ({shards} shards, {dur:.1}s)");
            println!("  run: zparity drill {f}");
            exit(1);
        }
    }
}

fn run_frame(p: &Paths, frame_idx: usize) {
    let man = load_checked_manifest(p);
    if frame_idx >= man.frames as usize {
        eprintln!(
            "requested frame {frame_idx} >= golden frame count {}",
            man.frames
        );
        exit(1);
    }

    let golden = RollupMap::open(&p.golden_dir.join("rollup.bin")).unwrap();
    let end_frame = (frame_idx + 1) as u32;
    let tmp = p.cache_dir.join(format!("frame-{frame_idx}.fp"));
    std::fs::create_dir_all(&p.cache_dir).unwrap();

    let ck = nearest_compatible_checkpoint(p, end_frame);
    let start = ck.as_ref().map(|(frame, _)| *frame as usize).unwrap_or(0);
    let out = runner::rust_shard_cmd(
        p,
        end_frame,
        &tmp,
        ck.as_ref().map(|(_, path)| path.as_path()),
    )
    .output()
    .expect("spawn rust frame check");
    if !out.status.success() {
        eprintln!(
            "frame check failed:\n{}",
            String::from_utf8_lossy(&out.stderr)
        );
        exit(1);
    }

    let data = std::fs::read(&tmp).expect("read frame fp");
    let shard_idx = frame_idx - start;
    if (shard_idx + 1) * RECORD_LEN > data.len() {
        eprintln!(
            "rust output too short: expected shard_idx {shard_idx}, got {} records",
            data.len() / RECORD_LEN
        );
        exit(1);
    }
    let got = FrameFingerprint::from_bytes(&data[shard_idx * RECORD_LEN..]).rollup;
    let want = golden.get(frame_idx);
    if got == want {
        println!("MATCH frame {frame_idx} rollup=0x{got:08x}");
        return;
    }

    println!("DIVERGE at frame {frame_idx}  rust=0x{got:08x} golden=0x{want:08x}");
    println!("  run: zparity drill {frame_idx}");
    exit(1);
}

fn run_shard(
    p: &Paths,
    start: u32,
    end: u32,
    fp_out: &Path,
    load_state: Option<&Path>,
    golden: &RollupMap,
    first_diff: &Mutex<Option<u64>>,
) {
    let out = runner::rust_shard_cmd(p, end, fp_out, load_state)
        .output()
        .expect("spawn rust shard");
    if !out.status.success() {
        eprintln!(
            "shard [{start},{end}) failed:\n{}",
            String::from_utf8_lossy(&out.stderr)
        );
        let mut g = first_diff.lock().unwrap();
        *g = Some(g.map_or(start as u64, |v| v.min(start as u64)));
        return;
    }
    let data = std::fs::read(fp_out).expect("read shard fp");
    // A shard that resumed from a checkpoint at frame `start` produced records for
    // frames (start, end]; record k corresponds to golden index start + k.
    // Shard 0 starts at 0 with no --load-state and produces records for (0, end].
    let n = data.len() / RECORD_LEN;
    for k in 0..n {
        // golden index 0 = fingerprint for frame 1 (first emitted); record k of a
        // shard resumed at `start` is frame start+k+1, golden index start+k.
        let frame_idx = start as usize + k;
        if frame_idx >= golden.len() {
            break;
        }
        let rollup = FrameFingerprint::from_bytes(&data[k * RECORD_LEN..]).rollup;
        if rollup != golden.get(frame_idx) {
            let mut g = first_diff.lock().unwrap();
            let f = frame_idx as u64;
            *g = Some(g.map_or(f, |v| v.min(f)));
            return;
        }
    }
}

fn load_checked_manifest(p: &Paths) -> Manifest {
    let man = Manifest::load(&p.golden_dir.join("manifest.json")).unwrap_or_else(|e| {
        eprintln!("no golden manifest ({e}); run `zparity capture` first");
        exit(1);
    });
    if man.schema != parity::golden::SCHEMA {
        eprintln!("golden schema {} != {}", man.schema, parity::golden::SCHEMA);
        exit(1);
    }
    check_hash(
        "ROM",
        &runner::sha256_file(&p.rom).unwrap(),
        &man.rom_sha256,
    );
    check_hash(
        "save",
        &runner::sha256_file(&p.save).unwrap(),
        &man.save_sha256,
    );
    man
}

fn shard_count(args: &[String], frames: u32) -> usize {
    if let Some(i) = args.iter().position(|a| a == "--shards") {
        let Some(v) = args.get(i + 1) else {
            eprintln!("--shards requires a value");
            exit(2);
        };
        return v.parse().unwrap_or_else(|_| {
            eprintln!("--shards K: invalid shard count");
            exit(2);
        });
    }
    if frames < SHARD_THRESHOLD {
        return 1;
    }
    thread::available_parallelism()
        .map(|n| n.get())
        .unwrap_or(4)
}

fn frame_arg(args: &[String]) -> Option<usize> {
    let i = args.iter().position(|arg| arg == "--frame")?;
    let Some(value) = args.get(i + 1) else {
        eprintln!("--frame requires a value");
        exit(2);
    };
    Some(value.parse().unwrap_or_else(|_| {
        eprintln!("--frame N: invalid frame index");
        exit(2);
    }))
}

fn shard_bounds(frames: u32, shards: usize) -> Vec<u32> {
    let mut v = Vec::with_capacity(shards + 1);
    for i in 0..=shards {
        v.push(((frames as u64 * i as u64) / shards as u64) as u32);
    }
    v.dedup();
    v
}

fn ensure_checkpoints(p: &Paths, bounds: &[u32]) {
    let ck_dir = p.cache_dir.join("ck");
    std::fs::create_dir_all(&ck_dir).unwrap();
    let needed: Vec<u32> = bounds[1..bounds.len() - 1].to_vec();
    let expected_manifest =
        CheckpointCacheManifest::current(p).expect("compute checkpoint cache manifest");
    let manifest_path = ck_dir.join("manifest.json");
    let manifest_reason = match CheckpointCacheManifest::load(&manifest_path) {
        Ok(existing) => existing.incompatibility_reason(&expected_manifest),
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => Some("manifest missing"),
        Err(_) => Some("manifest unreadable"),
    };
    let missing: Vec<u32> = needed
        .iter()
        .copied()
        .filter(|f| !ck_dir.join(format!("{f}.sav")).exists())
        .collect();
    if manifest_reason.is_none() && missing.is_empty() {
        return;
    }
    let frames_to_seed = if let Some(reason) = manifest_reason {
        eprintln!("checkpoint cache invalidated: {reason}");
        needed.clone()
    } else {
        missing
    };
    if frames_to_seed.is_empty() {
        expected_manifest
            .save(&manifest_path)
            .expect("write checkpoint cache manifest");
        return;
    }
    eprintln!(
        "seeding {} checkpoints (one sequential pass, render-free)...",
        frames_to_seed.len()
    );
    // Single pass to the last boundary, dropping all checkpoints via --save-state-at.
    // rust_seed_cmd skips the per-frame render (the dominant cost); the binary renders
    // ONLY the boundary frames before each checkpoint to re-project display state into
    // RAM. The result is byte-identical to a fully-rendered seed (verified by shard
    // invariance) but ~100x faster.
    // Write each checkpoint to <frame>.sav.tmp and only rename to the final
    // <frame>.sav AFTER the pass succeeds. A killed (OOM/SIGKILL) seeding pass
    // then never leaves a partial file that the existence check above would
    // trust -> shards would otherwise load a corrupt checkpoint and report a
    // phantom divergence with no root-cause signal.
    let seed_end = frames_to_seed.iter().copied().max().unwrap();
    let mut cmd = runner::rust_seed_cmd(p, seed_end);
    for f in &frames_to_seed {
        cmd.args([
            "--save-state-at",
            &format!("{f}:{}", ck_dir.join(format!("{f}.sav.tmp")).display()),
        ]);
    }
    let st = cmd.status().expect("spawn checkpoint seed");
    assert!(st.success(), "checkpoint seeding failed");
    for f in &frames_to_seed {
        let final_path = ck_dir.join(format!("{f}.sav"));
        let _ = std::fs::remove_file(&final_path);
        std::fs::rename(ck_dir.join(format!("{f}.sav.tmp")), final_path)
            .expect("rename seeded checkpoint into place");
    }
    expected_manifest
        .save(&manifest_path)
        .expect("write checkpoint cache manifest");
    let _ = std::fs::remove_file(p.cache_dir.join("seed.fp"));
}

fn nearest_compatible_checkpoint(p: &Paths, end_frame: u32) -> Option<(u32, PathBuf)> {
    let ck_dir = p.cache_dir.join("ck");
    let expected = CheckpointCacheManifest::current(p).ok()?;
    let existing = CheckpointCacheManifest::load(&ck_dir.join("manifest.json")).ok()?;
    if !existing.is_compatible_with(&expected) {
        return None;
    }
    nearest_checkpoint(&ck_dir, end_frame)
}

fn nearest_checkpoint(ck_dir: &Path, end_frame: u32) -> Option<(u32, PathBuf)> {
    let mut best: Option<(u32, PathBuf)> = None;
    let entries = std::fs::read_dir(ck_dir).ok()?;
    for e in entries.flatten() {
        let path = e.path();
        if path.extension().and_then(|s| s.to_str()) != Some("sav") {
            continue;
        }
        if let Some(stem) = path.file_stem().and_then(|s| s.to_str()) {
            if let Ok(f) = stem.parse::<u32>() {
                if f < end_frame
                    && best
                        .as_ref()
                        .map_or(true, |(best_frame, _)| f > *best_frame)
                {
                    best = Some((f, path));
                }
            }
        }
    }
    best
}

fn check_hash(what: &str, got: &str, want: &str) {
    if got != want {
        eprintln!("{what} sha256 mismatch:\n  golden={want}\n  local ={got}");
        exit(1);
    }
}
